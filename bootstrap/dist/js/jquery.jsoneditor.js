...
(function( $ ) {
    ...
    $.fn.jsonEditor = function(json, options) {
        ...
        return this.each(function() {
            JSONEditor($(this), json, onchange, onpropertyclick, options.propertyElement, options.valueElement, options.matchedElements, options.matchedElements_css);
        });
        
    };
    
    function JSONEditor(target, json, onchange, onpropertyclick, propertyElement, valueElement, matchedElements, matchedElements_css) {
        var opt = {
            target: target,
            onchange: onchange,
            ...
            matchedElements_css: matchedElements_css
        };
...
//    }
    ...
    function construct(opt, json, root, path) {
        ...
        for (var key in json) {
            if (!json.hasOwnProperty(key)) continue;//(opt.matchedElements.indexOf(key) != -1) ? opt.matchedElements_css
            
            name_css = "";
            if((opt.matchedElements.indexOf(key) != -1)){
            	name_css = opt.matchedElements_css;
            }

            var item     = $('<div>',   { 'class': 'item', 'data-path': path }),
                property =   $(opt.propertyElement || '<input '+name_css+' readonly>', { 'class': 'property' }),
                value    =   $(opt.valueElement || '<input readonly>', { 'class': 'value'    });

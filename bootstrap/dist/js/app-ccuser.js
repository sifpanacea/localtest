...
$(document).ready(function() {
	...
	$('input[name="activity"]').change(function() {
		...
		loadURL(url, container,function(response)
		{
		...
		$('.easy-pie-chart').each(function() {
			...
			$this.easyPieChart({
				...
				onStep : function(value) {
					var value=this.$el.find('span').text();
...
function loadURL(url, container) {
	...
	$.ajax({
		...
		success : function(data) 
		{
	        ...
	        var url_check=split_url.length-1;
			if(split_url[url_check]=="app_history")
			{
				var applist_last=$.parseJSON(data);
					$.each(applist_last, function (i, obj) 
					{
					console.log("eachindex",i)
					var time=obj.time;
					var date=time.split(" ");
					lists=lists+'<li><span class="unread"><a href="javascript:void(0);" class="msg"><span class="from">'+obj.app_name+'<i class="icon-paperclip"></i></span><time>'+date[0]+'</time><span class="subject">'+obj.app_description+'</span><span class="msg-body"></span></a></span></li>';
					})
				
			}
			else if(split_url[url_check]=="admin_messages")
			{
				var Message_notify=$.parseJSON(data);
				$.each(Message_notify, function (i, obj) 
				{
				lists=lists+'<li><span class="unread"><a href="javascript:void(0);" class="msg"><span class="from">'+obj.subject+'<i class="icon-paperclip"></i></span><span class="subject">'+obj.message+'</span><span class="msg-body"></span></a></span></li>';
				})
			}
			else if(split_url[url_check]=="docs_history")
			{
				
				var doclist_last=$.parseJSON(data);
				
				var document_name='';
				$.each(doclist_last, function (i, obj)
				{
					var user_with_hash = obj.history.last_stage.submitted_by;
					var user = user_with_hash.replace('#','@')
					lists=lists+'<li><span class="unread"><a href="javascript:void(0);" class="msg"><span class="from">'+obj.app_properties.app_name+'<i class="icon-paperclip"></i></span><span class="msg-body">submitted by-'+user+'</span><span class="subject">Submitted time-'+obj.history.last_stage.time+'</span></a></span></li>';
				})
			}
			if(lists!='')
			{
				var final_lst='<ul class="notification-body">'+lists+'</ul>';
				container.css({
							   opacity : '0.0'
							  }).html(final_lst).delay(50).animate({opacity : '1.0'}, 300);
			}
			else
			{
				var current_link;
				if(split_url[url_check]=="app_history")
				{
					current_link = "No apps have been created";
				}
				else if(split_url[url_check]=="admin_messages")
				{
					current_link = "No messages have been received";
				}
				else if(split_url[url_check]=="docs_history")
				{
					current_link = "No docs have been submitted";
				}
				container.html('<div class="alert alert-transparent"><h4>No Recent Activities</h4>'+current_link+'</div><i class="fa fa-times fa-4x fa-border"></i>');
			}
},

$(document).ready(function() {
		...
		//
		//open application when pressed install button
		//		
		$(document).on('click','#app_open',function(e)
		{
			var to_search = $(this).parents('tr').children('td:first').text()
			$('#content').hide();
			$('<div id="content" class="app_render"><div class="table-wrap custom-scroll animated fast fadeInRight" style="height: 192px;"><table id="inbox-table" class="table table-striped table-hover"><tbody class="inbox_add"></tbody></table></div>').appendTo('#main');
			...
			$( ".device" ).load( _url, function(response)
			{
				...
				if(res!='')
				{
					...
					image_string=res;
					$('<footer><div class="row col col-12 menubotom pull-left"><button class="btn btn-default btn-labeled menubt col col-12" type="button" id="prev_pg"><span class="btn-label"><i class="glyphicon glyphicon-chevron-left"></i></span></>Previous</button><button class="btn btn-default menubt btn-labeled col col-12" type="button" id="btnsubmit" onClick="" value="Delete Page"><span class="btn-label"><i class="fa fa-check-square"></i></span></>Submit</button><button class="btn btn-default btn-labeled menubt col col-12" type="button" id="nxt_pg" value="Next">Next</><span class="btn-label btn-label-right"><i class="glyphicon glyphicon-chevron-right"></i></span></button></div></footer>').appendTo('.device');
					$('.page').hide();
					$('.navbar').hide();
					$('#submit').hide();
					$('#disapprove').hide();
					...
					if($('.indexpages').children('.page').first().css('display')=='none')
					{
						...
						$('.device').animate({'margin-left': "200px"},1000);
						//afterload();
		...
		var apd__ = "tlsdigitals201653011205399";
		var url = "http://www.paas.com/PaaS/tlsdigitals/index.php/tlsdigitals/tlsdigitals201653011205399_con/create";
		
		// opening the application 
		/* $(document).on("click",".initiate",function()
		{
			$('#content').hide();
			$('<div id="content" class="app_render"><div class="table-wrap custom-scroll animated fast fadeInRight" style="height: 192px;"><table id="inbox-table" class="table table-striped table-hover"><tbody class="inbox_add"><div class="device"></tbody></table></div>').appendTo('#main');
		}) */
		//

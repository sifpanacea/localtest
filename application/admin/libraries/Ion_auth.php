...
class Ion_auth
{
	...
	public function __construct()
	{
		...
		log_message('debug','$this->logged_in()'.print_r($this->logged_in(),true));
		
		//auto-login the user if they are remembered
		if (!$this->logged_in() && get_cookie('identity') && get_cookie('remember_code'))
		{
		    log_message('debug','login_remembered_user'.print_r($this->logged_in(),true));
	...
	public function forgotten_password($identity)    //changed $email to $identity
	{
		if ($this->ion_auth_model->forgotten_password($identity))   //changed
		{
            ...
            $customer = $this->where('active', 1)->get_customer($identity);  //changed to get_user_by_identity from email
            
			if ($customer)
			{
			   $data = array(
					'identity'		          => $customer['email'],
					'forgotten_password_code' => $customer['forgotten_password_code']
				);
                $forgotten_password_code = $customer['forgotten_password_code'];
			...
			else
			{
				...
				$user = $this->where('active', 1)->get_user($identity);
			    ...
			    else
			    {
				    // Get admin information
			  	    $admin = $this->where('active', 1)->get_admin($identity);
                    ...
                    else
                    {
				        ...
				        $subadmin = $this->where('active', 1)->get_sub_admin($identity);
				    
					    if ($subadmin)
					    {
					        $data = array(
								'identity'		          => $subadmin['email'],
								'forgotten_password_code' => $subadmin['forgotten_password_code']
							);
							$forgotten_password_code = $subadmin['forgotten_password_code'];
							$fromaddress = $this->config->item('smtp_user');
							$this->email->set_newline("\r\n");
					    ...
					    {
						   // Get support admin information
					       $supportadmin = $this->where('active', 1)->get_support_admin($identity);
						
			               if ($supportadmin)
					       {
					            $data = array(
									'identity'		          => $supportadmin['email'],
									'forgotten_password_code' => $supportadmin['forgotten_password_code']
								);
								$forgotten_password_code = $supportadmin['forgotten_password_code'];
								$fromaddress = $this->config->item('smtp_user');
								$this->email->set_newline("\r\n");
	...
	/**
	 * Helper: Master logged in
	 *  
	 * @author Mathew ( Selva )
	 */
	public function master_logged_in()
	{
        $data = $this->session->userdata("master");
		return (bool) $data;
	}
	 ...
	 * @author Mathew
	 **/
	public function check_level_for_support_admin()
	{

...
class Tswreis_sports_mgmt extends My_Controller {
	...
	public function panacea_mgmt_schools()
	{
		...
		$unique_id = "MBNR_61406_";
		$correct_id = "RR_61406_";
		//====================screening collection ==============doc_data.widget_data.page1.Student Details.Hospital Unique ID
		$query = $this->mongo_db->whereLike('doc_data.widget_data.page1.Student Details.Hospital Unique ID',$unique_id)->get('healthcare2017617145744625');
		...
		$query = $this->mongo_db->where("_id", new MongoID($doc["_id"]))->set($doc)->update('healthcare2017617145744625');
	...
	function panacea_imports_school()
	{
		...
		$this->_render_page('tswreis_sports_admins/panacea_imports_school', $this->data);
	}
	...
	function panacea_imports_students()
	{
		...
		$this->_render_page('tswreis_sports_admins/panacea_imports_students', $this->data);
	}
	...
	function import_students()
	{
		...
		$this->data = $this->panacea_common_lib->import_tswreis_sports_students($post);
		...
		{
			redirect('tswreis_sports_admins/panacea_reports_students_filter');
		}else if($this->data['error'] == 'excel_column_check_fail')
		{
			$this->_render_page('tswreis_sports_admins/panacea_imports_students', $this->data);
		}else if($this->data['error'] == 'file_upload_failed')
...
//		$user["gcm_registration_id"] = $gcm_registration_id;
	...
	function upgrade_class_schoolwise()
	{
	  ...
	  else
	  {
			...
			redirect('panacea_mgmt/panacea_imports_students');
	  }
...
// 	 	$this->index();
	...
	*/
	 ...
	 public function basic_dashboard($date = FALSE, $request_duration = "Daily", $screening_duration = "Yearly")
	{
		...
		//$this->_render_page('panacea_admins/panacea_admin_dash',$this->data);
		$this->_render_page('tswreis_sports_admins/panacea_basic_dashboard',$this->data);
 ...
 public function get_searched_student_sick_requests()
  ...
  public function panacea_covid_cases()
	...
	*/
	...
	{
		$data = $this->tswreis_sports_common_model->get_achievements_participation_wise();
	...
	{
		$data = $this->tswreis_sports_common_model->get_registered_students_data();

		$this->output->set_output(json_encode($data));

	}

	public function tswreis_electronic_sports_record()
	{
		$this->check_for_admin();
		$this->data = "";
		$this->_render_page('tswreis_sports_admins/tswreis_electronic_sports_record', $this->data);
	} 

	public function get_achievements_wise_data()
	{

		$medal_type = $this->input->post('achieve_data',true);		
		$achievement_type = $this->input->post('achievement_type',true);
		
		$this->data['medal_details'] = $this->tswreis_sports_common_model->get_achievements_wise_data($medal_type, $achievement_type);

		$this->_render_page('tswreis_sports_admins/tswreis_achievements_wise_data', $this->data);

	}

	public function get_sport_type_data()
	{

		$event_name = $this->input->post('sport_name', true);

		$this->data['event_details'] = $this->tswreis_sports_common_model->get_sport_type_data_clicking_modal($event_name);

		$this->_render_page('tswreis_sports_admins/tswreis_sport_type_data', $this->data);
	}

	public function get_zonal_wise_data()
	{
		$this->data = "";
		$this->_render_page('tswreis_sports_admins/tswreis_zonal_wise_data', $this->data);
	}

	public function get_year_wise_sports_data()
	{
		$this->data = "";
		$this->_render_page('tswreis_sports_admins/tswreis_year_wise_sports_data', $this->data);
	}

	public function get_sports_academies_list()
	{
		$this->data = "";
		$this->_render_page('tswreis_sports_admins/tswreis_sports_academies_list', $this->data);
	}


	public function get_yearwise_achievements_pie()

  {
 	...
 	$this->data = $this->tswreis_sports_common_model->get_yearwise_achievements_pie_model($academic_year);
	...
	public function get_year_wise_sports_data_drill()

	{
		
		//$this->data['medal_list'] = $this->tswreis_sports_common_model->get_all_district();
		$participated_year = $this->input->post('spot_academic', true);
		$participation_level = $this->input->post('value_level', true);

		$this->data = "";

		$this->_render_page('tswreis_sports_admins/yearwise_achievements_page', $this->data);
	}


	public function total_achievements_pie_with_filters()
	{
		$req_type = $this->input->post('req_type', true);
		$medal = $this->input->post('medal_select', true);		
		$academic = $this->input->post('req_academic', true);

		
		$this->data = $this->tswreis_sports_common_model->total_achievements_pie_with_filters_model($req_type, $medal, $academic);
	...
	public function get_requests_by_medals_list()
	{
		$academic = $this->input->post('academic', true);
		$req_type = $this->input->post('req_type', true);		
		$medal_list = $this->input->post('medal_list', true);

		echo print_r($academic,true);
		echo print_r($req_type,true);
		echo print_r($medal_list,true);
		exit();

		
		$this->data = $this->tswreis_sports_common_model->get_requests_by_medals_list_model($req_type, $medal_list, $academic);
	...
	public function get_sports_data_based_on_span()
	{

		$select_level = $this->input->post('level_type', true);
		$select_medal = $this->input->post('medal_type', true);		
		

		$this->data = $this->tswreis_sports_common_model->get_sports_data_based_on_span_modal($select_level,$select_medal);	
	...
	public function drilldown_selected_to_medals()
	{
		$data = $_POST['data'];
		$this->data = $this->tswreis_sports_common_model->drilldown_selected_to_medals($data);
		$this->output->set_output(json_encode($this->data));
	}

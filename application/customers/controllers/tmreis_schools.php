...
class Tmreis_schools extends My_Controller {
    ...
    function __construct()
	{
		...
		$this->load->model('tmreis_schools_common_model');
		$this->load->library('tmreis_schools_common_lib');
	...
	{
		redirect('tmreis_schools/to_dashboard');
	}
	
	public function initiate_request()
	{
		$this->data = "";
		$this->_render_page('tmreis_schools/application_initiate',$this->data);
	}
	
	public function access_request($id)
	{
		$this->data['doc_id'] = $id;
		$this->_render_page('tmreis_schools/hs_req_application_access',$this->data);
	}
	
	public function initiate_attendance()
	{
		$this->data = "";
		$this->_render_page('tmreis_schools/attendance_initiate',$this->data);
	}
	
	public function initiate_sanitation_report()
	{
		$this->data = "";
		$this->_render_page('tmreis_schools/sanitation_report',$this->data);
	}
	...
	function list_chronic_cases()
	{
	  ...
	  $school_info = $this->tmreis_schools_common_model->get_school_info($school_code);
	  $school_name = $school_info[0]['school_name'];
	  
	  $total_rows = $this->tmreis_schools_common_model->tmreis_chronic_cases_count($school_name);
	  ...
	  /*$this->data['cases'] =$this->tmreis_schools_common_model->get_chronic_cases_model($config['per_page'],$page,$school_name);*/
	  ...
	  $this->data['cases'] =$this->tmreis_schools_common_model->get_chronic_cases_model_for_data_table($school_name);
		
	  $this->_render_page('tmreis_schools/chronic_case_report',$this->data);
	}
	...
	function to_dashboard($date = FALSE, $request_duration = "Daily", $screening_duration = "Yearly")
	{
		...
		$this->data = $this->tmreis_schools_common_lib->to_dashboard($date, $request_duration, $screening_duration);
		 // School Code
	  	$school_code = $this->get_my_school_code();
	  	...
	  	$school_info = $this->tmreis_schools_common_model->get_school_info($school_code);
	  	$school_name = $school_info[0]['school_name'];
	  	$this->data['attendance_submitted_school_name'] = $this->tmreis_schools_common_model->get_attendance_submitted_school_name($date, $school_name);
		$this->_render_page('tmreis_schools/school_dash',$this->data);
	...
	function to_dashboard_with_date()
	{
		...
		$this->data = $this->tmreis_schools_common_lib->to_dashboard_with_date($today_date,$request_pie_span,$screening_pie_span);
	...
	function fetch_sanitation_report_against_date()
	{
	  ...
	  $sanitation_report_data = $this->tmreis_schools_common_model->get_sanitation_report_data_with_date($date,$school_name);
		
	  $sanitation_report = $this->tmreis_schools_common_lib->build_sanitation_report($sanitation_report_data);
	...
	function update_request_pie()
	{
		...
		$today_date = $_POST['today_date'];
		log_message('debug','today_date==============191'.print_r($today_date,true));
		$request_pie_span = $_POST['request_pie_span'];
		log_message('debug','request_pie_span==============193'.print_r($request_pie_span,true));
		$this->data = $this->tmreis_schools_common_lib->update_request_pie($today_date,$request_pie_span);
		log_message('debug','this->data==============195'.print_r($this->data,true));
		$this->output->set_output($this->data);
	...
	function update_screening_pie()
	{
		...
		$today_date = $_POST['today_date'];
		$screening_pie_span = $_POST['screening_pie_span'];
		$this->data = $this->tmreis_schools_common_lib->update_screening_pie($today_date,$screening_pie_span);
	...
	function refresh_screening_data()
	{
		...
		$this->tmreis_schools_common_model->update_screening_collection($today_date,$screening_pie_span,$school_name);
		$today_date = $this->tmreis_schools_common_model->get_last_screening_update();
	...
	function drilling_screening_to_abnormalities()
	{
		...
		$today_date = $_POST['today_date'];
		$screening_pie_span = $_POST['screening_pie_span'];
		$screening_report = json_encode($this->tmreis_schools_common_model->get_drilling_screenings_abnormalities($data,$today_date,$screening_pie_span));
	...
	function drill_down_screening_to_students()
	{
		...
		$this->check_for_plan('drilling_pie');
		$data = $_POST['data'];
		log_message("debug","dataaaaaaaaaa==============525".print_r($data,true));
		$today_date = $_POST['today_date'];
		$screening_pie_span = $_POST['screening_pie_span'];
		$docs = $this->tmreis_schools_common_model->get_drilling_screenings_students($data,$today_date,$screening_pie_span);
		log_message("debug","docsssssssssssssssssssssssss".print_r($docs,true));
	...
	function drill_down_screening_to_students_load_ehr()
	{
		...
		$get_docs = $this->tmreis_schools_common_model->get_drilling_screenings_students_docs($docs_id);
		...
		$this->_render_page('tmreis_schools/drill_down_screening_to_students_load_ehr',$this->data);
	}
	
	public function drill_down_screening_to_students_load_ehr_doc($_id)
	{
		
		$docs = $this->tmreis_schools_common_model->drill_down_screening_to_students_load_ehr_doc($_id);
		...
		$this->_render_page('tmreis_schools/tmreis_schools_reports_display_ehr',$this->data);
	}
	
	public function drill_down_screening_initiate_request($_id)
	{
		//$this->data['docs'] = $this->tmreis_schools_common_model->drill_down_screening_to_students_load_ehr_doc($_id);
	...
	function drill_down_absent_to_students()
	{
		...
		// Get my school code
		$school_code = $this->get_my_school_code();
		$school_data = $this->tmreis_schools_common_model->get_school_details_for_school_code($school_code);
		
		$docs = $this->tmreis_schools_common_model->get_drilling_absent_students($label,$today_date,$school_data['school_name']);
	...
	function drill_down_absent_to_students_load_ehr()
	{
		...
		$get_docs = $this->tmreis_schools_common_model->get_drilling_absent_students_docs($UI_id);
		
		$this->data['students'] = $get_docs;
		log_message('debug','drill_down_absent_to_students_load_ehr==332'.print_r($this->data['students'],true));
		//set the flash data error message if there is one
		$this->data['message'] = (validation_errors()) ? validation_errors() : $this->session->flashdata('message');
	
		$this->_render_page('tmreis_schools/drill_down_absent_to_students_load_ehr',$this->data);
	}
	...
	function drill_down_request_to_students()
	{
		...
		$docs = $this->tmreis_schools_common_model->get_drilling_request_students($request_label,$today_date,$request_pie_span,$school_name,$id_pattern);
	...
	function drill_down_request_to_students_load_ehr()
	{
		...
		$get_docs = $this->tmreis_schools_common_model->get_drilling_request_students_docs($UI_id);
		...
		$this->_render_page('tmreis_schools/drill_down_request_to_students_load_ehr',$this->data);
	}
	...
	function drill_down_identifiers_to_students()
	{
		...
		log_message('debug','identifier========='.print_r($identifier,true));
		$today_date       = $_POST['today_date'];
		log_message('debug','today_date========='.print_r($today_date,true));
		$request_pie_span = $_POST['request_pie_span'];
		log_message('debug','request_pie_span========='.print_r($request_pie_span,true));
		...
		$docs = $this->tmreis_schools_common_model->get_drilling_identifiers_students($identifier,$today_date,$request_pie_span,$school_name,$id_pattern);
	...
	function drill_down_identifiers_to_students_load_ehr()
	{
		...
		log_message('debug','temp===tmreis==424==='.print_r($temp,true));
		$UI_id = json_decode(base64_decode($_GET['ehr_data_for_identifiers']),true);
		log_message('debug','UI_id===tmreis==424==='.print_r($UI_id,true));
		$get_docs = $this->tmreis_schools_common_model->get_drilling_identifiers_students_docs($UI_id);
		log_message('debug','get_docs===tmreis==425==='.print_r($get_docs,true));
		$this->data['students'] = $get_docs;
		log_message('debug','drill_down_identifiers_to_students_load_ehr===tmreis==425'.print_r($this->data['students'],true));
		//set the flash data error message if there is one
		$this->data['message'] = (validation_errors()) ? validation_errors() : $this->session->flashdata('message');
	
		$this->_render_page('tmreis_schools/drill_down_identifiers_to_students_load_ehr',$this->data);
	}
	 ...
	 public function classes()
	{
		...
		$this->data = $this->tmreis_schools_common_lib->classes($school_code);
		$this->_render_page('tmreis_schools/classes',$this->data);
	}
	...
	public function create_class()
	{
	    ...
	    $school_code = $this->get_my_school_code();
	 	...
	 	$this->tmreis_schools_common_model->create_class($_POST, $school_code);	
	 	redirect('tmreis_schools/classes');
	}
	...
	public function delete_class($class_id)
	{
	    ...
	    $school_code = $this->get_my_school_code();
	 	...
	 	$this->tmreis_schools_common_model->delete_class($class_id, $school_code);	
	 	redirect('tmreis_schools/classes');
	}
	...
	public function sections()
	{
        ...
        $this->data = $this->tmreis_schools_common_lib->sections($school_code);
		$this->_render_page('tmreis_schools/sections',$this->data);
	}
	...
	public function create_section()
	{
	    ...
	    $school_code = $this->get_my_school_code();
	 	...
	 	$this->tmreis_schools_common_model->create_section($_POST, $school_code);
	 	redirect('tmreis_schools/sections');
	}
	...
	public function delete_section($section_id)
	{
	  ...
	  $this->tmreis_schools_common_model->delete_section($section_id, $school_code);	
	  redirect('tmreis_schools/sections');
	}
	...
	public function create_student()
	{
		...
		$school_name = $school_info[0]['school_name'];
		$dist_info = $this->tmreis_schools_common_model->get_district($district_id);
		$district_code = $dist_info[0]['dt_code'];
		
		$classes     = $this->tmreis_schools_common_model->get_all_classes("All",$school_code);
		$sections    = $this->tmreis_schools_common_model->get_all_sections("All",$school_code);
		$hunique_id  = $this->tmreis_schools_common_model->generate_new_student_hunique_id($school_code,$district_code,$school_name);
		...
		$this->_render_page('tmreis_schools/create_student',$this->data);
	...
	public function create_staff()
	{
		...
		$hunique_id  = $this->tmreis_schools_common_model->generate_new_staff_hunique_id($school_code,$district_code,$school_name);
		
		$this->data['school_name']   = $school_name;
		$this->data['huniqueid']     = $hunique_id;
		$this->data['message']       = '';
		
		$this->_render_page('tmreis_schools/create_staff',$this->data);
	...
	public function add_student_ehr()
	{
	 ...
	 /*  $history = array();
	  ...
	  $added = $this->tmreis_schools_common_model->add_student_ehr_model($doc_data,$history,$doc_properties); 
	  ...
	  {
        $classes  = $this->tmreis_schools_common_model->get_all_classes("All");
		$sections = $this->tmreis_schools_common_model->get_all_sections("All");
		$hunique_id = $this->tmreis_schools_common_model->generate_new_student_hunique_id($school_code,$district_code,$school_name);
		$this->data['classes']   = $classes;
		$this->data['sections']  = $sections;
		$this->data['huniqueid'] = $hunique_id;
	...
	public function add_staff_ehr()
	{
	  ...
	  $added = $this->tmreis_schools_common_model->add_staff_ehr_model($doc_data,$history); 
	...
	public function student_reports()
	{
		...
		$this->data['students'] = $this->tmreis_schools_common_model->get_student($per_page,$page,$school_name);
		$this->data['studentscount'] = $this->tmreis_schools_common_model->studentscount($school_name);
		$this->data['classlist'] = $this->tmreis_schools_common_model->get_classes($per_page, $page, $school_code);
		$this->_render_page('tmreis_schools/student_reports',$this->data);
	}
	...
	public function get_students_list_by_class()
	{
		...
		$school_info = $this->tmreis_schools_common_model->get_school_info($school_code);
		$school_name = $school_info[0]['school_name'];
		$students = $this->tmreis_schools_common_model->get_students_by_class($selected_class, $school_name);
	...
	public function reports_ehr()
	{
		...
		$this->_render_page('tmreis_schools/reports_ehr',$this->data);
	}
	...
	public function reports_display_ehr_uid()
	{
		...
		$username    = $this->get_my_username();
		$this->data             = $this->tmreis_schools_common_lib->reports_display_ehr_uid($student_unique_id,$school_name);
		$this->data['username'] = $username.','.$school_name;
		...
		$this->_render_page('tmreis_schools/reports_display_ehr',$this->data);
	}
	...
	function change_password()
	{
		...
		if ($this->form_validation->run() == false)
		{
			...
			//render
			$this->_render_page('tmreis_schools/change_password', $this->data);
		}
		else
		{
			...
			$change = $this->tmreis_schools_common_model->change_password($identity, $this->input->post('old'), $this->input->post('new_pwd'));
	...
	public function staff_reports()
	{
		...
		$this->_render_page('tmreis_schools/staff_reports',$this->data);
	}
	...
	function create_schedule_followup()
	{
	  ...
	  $is_created = $this->tmreis_schools_common_model->create_schedule_followup_model($unique_id,$medication_schedule,$treatment_period,$start_date,$monthNames,$case_id);
	...
	function update_schedule_followup()
	{
	  ...
	  $compliance = $this->tmreis_schools_common_model->calculate_chronic_graph_compliance_percentage($case_id,$unique_id,$medication_taken);
	  
	  //log_message('debug','update_schedule_followup==3='.print_r($compliance,true));
	  
	  $is_updated = $this->tmreis_schools_common_model->update_schedule_followup_model($unique_id,$case_id,$compliance,$selected_date);
...
//	  log_message("debug","Begindate".print_r($begin,true));
	  ...
	  $pcompl_raw_data   = $this->tmreis_schools_common_model->fetch_student_pill_compliance_data($case_id,$student_unique_id);
	...
	public function feed_bmi_student() 
	 {
		$this->data = "";
		$this->_render_page('tmreis_schools/feed_bmi',$this->data);
	}
	
	public function show_bmi_student() 
	 {
		/* $this->data = "";
		$this->_render_page('tmreis_schools/show_bmi',$this->data); */
		 $logged_in_user = $this->session->userdata("customer");
		$email    		= $logged_in_user['email'];
		$email_array    = explode(".",$email);
	...
	public function extend_request() 
	{
		...
		$this->data['requests'] = $this->tmreis_common_model->get_all_rised_req();
		$this->data['message'] = false;
		
		$this->_render_page('tmreis_schools/extend_request',$this->data);
	}
	...
	public function app_access($doc_id)
	{
		...
		$this->_render_page('tmreis_schools/extend_request_app',$this->data);
	}
	...
	function hs_req_extend(){
		...
		if($req_return['return_error']){
			...
			redirect('tmreis_schools/extend_request');
	...
	public function tmreis_update_ehr()
	{
		...
		$this->data["district_code"] = strtoupper($email_array[0]);
		$this->data["message"] = "";		
		$this->_render_page('tmreis_schools/update_personal_info',$this->data);
	}
	
	public function tmreis_update_personal_ehr_uid()
	{
		$post = $_POST['student_unique_id'];
		$this->data = $this->tmreis_schools_common_lib->tmreis_update_personal_ehr_uid($post);
		
		$this->data['docscount'] = count($this->data['docs']);
	
		$this->_render_page('tmreis_schools/tmreis_update_personal_ehr',$this->data);
	...
	public function update_student_ehr()
	{
	  ...
	  $ehr_update = $this->tmreis_schools_common_model->update_student_ehr_model($unique_id,$update_profile); 
	  log_message("debug","photo_updateqwerty".print_r($ehr_update,true));
	  // $this->data['message'] = 'Updated Successfully';
	 //$this->_render_page('tmreis_schools/update_personal_info',$this->data);
	 $this->session->set_flashdata("message","Updated Successfully");
	 redirect("tmreis_schools/tmreis_update_ehr");
	}

	public function usermanual_view()
	{
		 $this->data="";
		$this->_render_page ( 'tmreis_schools/usermanual_view', $this->data );
	}
	...
	public function bmi_pie_view()
	{
	  ...
	  //Fetch school details with school code
		...
		$this->data = $this->tmreis_schools_common_lib->bmi_pie_view_lib($current_month, $school_name);
		$this->_render_page('tmreis_schools/tmreis_schools_bmi_pie_view',$this->data);
	}
	...
	public function bmi_pie_view_month_wise()
	{
		...
		$school_info = $this->tmreis_schools_common_model->get_school_info($school_code);
	  	$school_name = $school_info[0]['school_name'];
		$this->data = $this->tmreis_schools_common_lib->bmi_pie_view_lib_month_wise($current_month,$school_name);
	...
	function drill_down_bmi_to_students()
	{
		...
		$docs = $this->tmreis_schools_common_model->get_drill_down_to_bmi_report($symptom_type ,$month ,$school_name);
	...
	function drill_down_to_bmi_report_students()
	{
		...
		$students = $this->tmreis_schools_common_model->get_drilling_bmi_students_docs($UI_id);
		...
		$this->_render_page('tmreis_schools/tmreis_schools_drill_down_to_bmi_report_view',$this->data);
	}
	...
	public function show_student_bmi_graph($hospital_unique_id){
		...
		$bmi_value = $this->tmreis_schools_common_model->get_student_bmi_graph_values($hospital_unique_id);
		...
		$this->_render_page('tmreis_schools/tmreis_schools_show_student_bmi_graph_view',$final_bmi_data);
		}
	...
	public function generate_bmi_report_to_excel()
	{
		...
		$file_path = $this->tmreis_schools_common_lib->generate_bmi_report_to_excel_lib($date, $school_name);
	...
	*/
	...
	{
		$this->data['message'] = "";
		$this->_render_page("tmreis_schools/show_contact_us",$this->data);
	}
	/*
	...
	*/
	public function initiateSanitationReport()
	  {
			...
			$this->data = "";
		  	$this->_render_page('tmreis_schools/sanitation_report_view', $this->data);
	  }
        ...
        foreach ($_FILES as $index => $value)
       {
            ...
            if(strpos($index,'hs_req_attachments_campus')!== false)
			{
                ...
                for($i=0; $i<$cpt; $i++)
                {
                     ...
                     $_FILES['hs_req_attachments_campus']['size']  = $files['hs_req_attachments_campus']['size'][$i];
              		...
              		$this->upload->initialize($this->campus_attachment_upload_options('healthcare2017121175645993_con',$index));
     		...
     		if(strpos($index,'hs_req_attachments_toilets')!== false)
			{
                ...
                for($i=0; $i<$mri; $i++)
                {
			       ...
			       $this->upload->initialize($this->toilet_attachment_upload_options('healthcare2017121175645993_con',$index));
                if ( ! $this->upload->do_upload($index))
                {
       	...
       	if(strpos($index,'hs_req_attachments_kitchen')!== false)
		 {
            ...
            for($i=0; $i<$bill; $i++)
            {
                ...
                $this->upload->initialize($this->kitchen_attachment_upload_options('healthcare2017121175645993_con',$index));
        	...
        	if(strpos($index,'hs_req_attachments_dormitory')!== false)
		 {
            ...
            for($i=0; $i<$dormitory; $i++)
            {
                ...
                $this->upload->initialize($this->dormitory_attachment_upload_options('healthcare2017121175645993_con',$index));
		...
		$added = $this->tmreis_schools_common_model->create_sanitation_report_model($doc_data, $doc_properties, $app_properties, $history);
		if($added)
		{
			redirect('tmreis_schools/to_dashboard');
		}
		...
		{
			redirect('tmreis_schools/initiateSanitationReport');
		}
	  ...
	  public function initiateAttendanceReport()
	{
		...
		$this->_render_page('tmreis_schools/attendance_report_view', $this->data);
	}
	...
	public function create_attendence_report()
	{
		...
		$added = $this->tmreis_schools_common_model->create_attendence_report_model($doc_data, $doc_properties, $app_properties, $history);
		if($added)
		{
		...
		{
			redirect('tmreis_schools/initiateAttendanceReport');
		}
	...
	public function initiateHemoglobinReport()
	{
		...
		$this->data["district_code"] = strtoupper($email_array[0]);
	  	...
	  	$this->_render_page('tmreis_schools/hb_feed_view', $this->data);
	}
 	...
 	public function create_hemoglobin_report()
  	{
		...
		$is_exists  = $this->tmreis_schools_common_model->check_if_doc_exists_in_hb($uniqueId);
		/*echo "<pre>";
		echo print_r($is_exists,true);
		echo "</pre>";
		exit;*/
		...
		if(!empty($is_exists))
		{
			...
			else 
			{
				...
				$newly_created = $this->tmreis_schools_common_model->add_student_HB_model($monthly_hb, $doc_properties, $app_properties, $history);
	...
	*/ 
	public function show_hb_student() 
	{
		...
		$this->data["school_code"]   = $email_array[1];
		$this->data["district_code"] = strtoupper($email_array[0]);
		$this->_render_page('tmreis_schools/school_wise_show_hb_graph',$this->data);
	}
	/*
	*BMI Ajax function
	...
	*/ 
	public function student_hb_graph(){
		...
		$hb_value = $this->tmreis_schools_common_model->get_student_hb_values($unique_id);
	...
	function tmreis_imports_hb_values()
	{
		$this->check_for_admin();
		$this->check_for_plan('tmreis_imports_diagnostic');
		//$this->data = $this->tswreis_schools_common_lib->tmreis_imports_hb_values();
			$this->data = $this->get_district_school_name();
			$this->data['message'] = "";
		$this->_render_page('tmreis_schools/tmreis_imports_hb_values', $this->data);
	}
	function imported_hb_xl_sheet()
	{
		$this->check_for_admin();
		$this->check_for_plan('tmreis_imports_diagnostic');
		//$this->data = $this->tmreis_schools_common_lib->tmreis_imports_hb_values();
			$this->data = $this->get_district_school_name();
	...
	function imports_hb_values()
	{
		...
		$this->data = $this->tmreis_schools_common_lib->imports_hb_values($post);

		
		if($this->data == "redirect_to_hb_fn")
		{
			redirect('tmreis_schools/imported_hb_xl_sheet');
	...
	public function initiateBmiReport()
	{
		...
		$this->data["school_code"]   = $email_array[1];
		$this->data["district_code"] = strtoupper($email_array[0]);
		$this->load->view('tmreis_schools/bmifeed_report_view', $this->data);
	 ...
	 public function feedBmiStudentReport()
	{
		...
		$monthly_bmi = array();
		$uniqueId   = $this->input->post('student_code');
		$is_exists = $this->tmreis_schools_common_model->check_if_doc_exists_in_bmi($uniqueId);
		...
		if(!empty($is_exists))
		{
			...
			$existing_update = $this->tmreis_schools_common_model->update_bmi_values($ndate,$monthly_bmi, $uniqueId);
				...
				if ($existing_update ) // the information has therefore been successfully saved in the db
				{
					...
					redirect('tmreis_schools/initiateBmiReport');
				}
				else
				{
					$this->session->set_flashdata('fail','Some thing went wrong! Try Again');
			...
			else 
			{
				...
				$class = $this->input->post('page1_StudentDetails_Class');
				$section = $this->input->post('page1_StudentDetails_Section');
				$height = $this->input->post('page1_StudentDetails_Heightcms');
				$weight = $this->input->post('page1_StudentDetails_Weightkgs');
				$bmi = $this->input->post('page1_StudentDetails_BMI');
				$month = $this->input->post('page1_StudentDetails_Date');
	          ...
	          // History
				...
				$newly_created = $this->tmreis_schools_common_model->add_student_BMI_model($monthly_bmi, $school_details, $doc_properties, $app_properties, $history);
	...
	function tmreis_imports_bmi_values()
	{
		$this->check_for_admin();
		$this->check_for_plan('tmreis_imports_bmi_values');
		$this->data = $this->get_district_school_name();
		$this->data['message'] = "";
		$this->_render_page('tmreis_schools/tmreis_imports_bmi_values', $this->data);
	}
	...
	function imports_bmi_values()
	{
		...
		//log_message('error','post=============='.print_r($post,true));exit();
		$this->data = $this->tmreis_schools_common_lib->imports_bmi_values($post);

		
		if($this->data == "redirect_to_bmi_fn")
		{

			redirect('tmreis_schools/imported_bmi_xl_sheet');
		}else if($this->data['error'] == 'excel_column_check_fail')
		{
			$this->data = $this->get_district_school_name();
			$this->_render_page('tmreis_schools/tmreis_imports_bmi_values', $this->data);
	 ...
	 public function fetch_student_info()
	{
		...
		$this->data['get_data'] = $this->tmreis_schools_common_model->fetch_student_info_model($school_name, $unique_id);
	...
	{ 
		...
		$today_date = date('Y-m-d');
		$this->data['report_count'] = $this->tmreis_schools_common_model->get_attendance_report_daily_count($school_name, $today_date);
		$docs = $this->tmreis_schools_common_model->get_requests_daily_count($district_school_code, $today_date);
		$requests_info = $this->tmreis_schools_common_model->get_request_info_by_type($district_school_code, $today_date);
		$doctor_docs = $this->tmreis_schools_common_model->get_docs_daily_count($district_school_code, $today_date);
		$doctor_docs_info = $this->tmreis_schools_common_model->get_docs_daily_requests_info($district_school_code, $today_date);
		$this->data['total_student_count'] = $this->tmreis_schools_common_model->studentscount($school_name);
       ...
       if(!empty($doctor_docs_info['doc_chronic_requests_info'])) {
		...
		$bmi_info = $this->tmreis_schools_common_model->get_bmi_report_model_count($current_month, $school_name);
		...
		$bmi_student_info = $this->tmreis_schools_common_model->get_bmi_report_model_info($current_month, $school_name);
		...
		$this->data['screening_info'] =$this->tmreis_schools_common_model->get_all_screenings_count($today_date = false, $screening_duration = "Yearly");
		

		$hb_count = $this->tmreis_schools_common_model->get_hb_report_model_count($current_month, $school_name);
		$hb_count_info = $this->tmreis_schools_common_model->get_hb_report_model_info($current_month, $school_name);
		...
		$sanitation_report = $this->tmreis_schools_common_model->get_sanitation_report_data_with_date_version_2($date,$school_name);

		$this->data['sanitation_report'] = $this->tmreis_schools_common_lib->build_sanitation_report_v2($sanitation_report);
		
		
		//$this->data['sanitation_report'] = $this->tmreis_schools_common_lib->build_sanitation_report_activities($date,$school_name);
		//$this->data['sanitation_report'] = "";
		
		$this->data['basic_dash_screening_report_yearly'] = $this->tmreis_schools_common_model->get_screened_students_count($unique_id_pattern);

		$this->data['basic_dash_yearly_request_count'] = $this->tmreis_schools_common_model->get_all_requests_yearly_count($date,$request_duration="Yearly",$unique_id_pattern);

		$this->data['basic_dash_students_count'] = $this->tmreis_schools_common_model->studentscount($school_name);
		
		$school_color_code = $this->tmreis_schools_common_model->get_school_color_code($school_code);
		$this->data['school_color_code'] = $school_color_code;

		//$hsrequests = $this->tmreis_schools_common_lib->to_dashboard_new($date, $request_duration, $screening_duration);

		//$this->data['report'] = $hsrequests;
		$this->data['all'] = $this->tmreis_schools_common_model->get_doctor_visiting_report_date_wise($school_name);
		$this->data ['news_feeds'] = $this->tmreis_schools_common_model->get_all_news_feeds();
		$this->data['today_date'] = date('Y-m-d');
		$this->_render_page('tmreis_schools/all_activities_view',$this->data);
	}
	public function my_profile_hs()
	{
		 ...
		 $school_data = $this->tmreis_schools_common_model->get_school_information_for_school_code($schoolCode);
		 ...
		 $health_supervisor = $this->tmreis_schools_common_model->get_health_supervisor_details($schoolCode);
			 	 $this->data['hs_name'] = $health_supervisor['hs_name'];
			 	 $this->data['hs_mob']  = $health_supervisor['hs_mob'];
		 
		 $this->_render_page('tmreis_schools/my_profile_hs',$this->data);
	...
	{ 
		...
		$result = $this->tmreis_schools_common_model->update_principal_hs_profile($post,$schoolCode);
		...
		if ($result ) // the information has therefore been successfully saved in the db
				{
					...
					redirect('tmreis_schools/my_profile_hs');
				}
				else
				{
					$this->session->set_flashdata('fail','Some thing went wrong! Try Again');
	...
	*/
	public function hs_request()
	{
		...
		$this->_render_page('tmreis_schools/hs_initiate_request', $this->data);
	}
	/*
	...
	*/
	public function fetch_student_information()
	{
		...
		$this->data['get_data'] = $this->tmreis_schools_common_model->fetch_student_information_model($school_name, $unique_id);
			  ...
			  if(!empty($value['name']))
			  {
					...
					if ( ! $this->upload->do_upload($index))
					{
						 $this->session->set_flashdata('message','Submission failed. Each attachment file should be less than 2 MB');
								...
								redirect('tmreis_schools/hs_request');  
        ...
        foreach ($_FILES as $index => $value)
       {
             ...
             if(strpos($index,'Prescriptions')!== false)
			{
                ...
                for($i=0; $i<$cpt; $i++)
                {
                     ...
                     $_FILES['Prescriptions']['size']  = $files['Prescriptions']['size'][$i];
              		...
              		$this->upload->initialize($this->Prescriptions_attachment_upload_options('healthcare201610114435690_con',$index));
        ...
        else
        {
		 ...
		 $username = $session_data['email'];
		  ...
		  $doc_data['stage_name'] = 'healthcare201610114435690';
		 ...
		 if(array_key_exists("user_type",$session_data))
		 {
			...
			{
				$health_supervisor = $this->ion_auth->tmreis_health_supervisors()->row();
		 ...
		 //log_message('debug','healthcare201610114435690_CON=====GET_HEALTH_SUPERVISOR_DETAILS==HSNAME==>'.print_r($hs_name,true));
		// log_message('debug','healthcare201610114435690_CON=====GET_HEALTH_SUPERVISOR_DETAILS==HSMOB==>'.print_r($hs_mob,true));
...
//		echo print_r($doc_data,true);
  	 ...
  	 // $request_type = $form_data['doc_data']['widget_data']['page2']['Review Info']['Request Type'];
		  ...
		  if($request_type === "Chronic" || $request_type === "Deficiency" || $request_type === "Defects")
		 {
		  ...
		  // log_message('error','chronic_disease=========2662'.print_r($chronic_disease,TRUE));exit();
		   $this->tmreis_schools_common_model->create_chronic_case_new($unique_id,$request_type,$chronic_disease,$disease_desc,$schoolName);
	     }

  	  $initate_submit = $this->tmreis_schools_common_model->initiate_request_model($doc_data,$doc_properties,$app_properties,$array_history); 
  	  ...
  	  if ($initate_submit ) // the information has therefore been successfully saved in the db
			{
				...
				$fcm_notification = $this->tmreis_common_lib->fcm_message_notification($request_type,$unique_id,$student_name);
	...
	*/
  	...
  	public function fetch_submited_requests_docs()
  	{
  		...
  		$collection = "healthcare201610114435690_static_html";
  		$hs_req_docs  = $this->tmreis_schools_common_model->get_hs_req_normal($collection,$unique_id_code);

		$hs_req_emergency  = $this->tmreis_schools_common_model->get_hs_req_emergency($collection,$unique_id_code);

		$hs_req_chronic  = $this->tmreis_schools_common_model->get_hs_req_chronic($collection,$unique_id_code);
		...
		if(!empty($hs_req_chronic)){
  		...
  		$this->_render_page('tmreis_schools/fetch_submited_requests_docs',$this->data);
  	}
	...
	*/
	public function access_submited_request_docs($id)
  	{
  		$doc_id = $id;
		$query = $this->tmreis_schools_common_model->access_submited_request_docs($doc_id);
		$this->data['hs_req_docs'] = $query;
		$this->_render_page('tmreis_schools/access_submited_request_docs',$this->data);
  	}
	...
	*/
		...
		// Attachments
		 /*if(isset($_FILES) && !empty($_FILES))
		 {
		   ...
		   foreach ($_FILES as $index => $value)
	       {
			...
			$doc_history = $this->tmreis_schools_common_model->get_history($unique_id,$doc_id);
		 ...
		 if(isset($_FILES) && !empty($_FILES))
    {
       ...
       $doc_history = $this->tmreis_schools_common_model->get_history($unique_id,$doc_id);
        ...
        else
        {
		 ...
		 if(array_key_exists("user_type",$session_data))
		 {
			if($session_data['user_type'] == "HS")
			{
				$health_supervisor = $this->ion_auth->tmreis_health_supervisor()->row();
				$hs_name = $health_supervisor->hs_name;
				$hs_mob  = $health_supervisor->hs_mob;
			}
			else
  	...
  	/* $history = array();
		...
		*/
		...
		//$doc_update = $this->ci->panacea_common_model->update_doc_for_disapprove($healthcare201610114435690_edit);
		$approval_data = array(
		...
		$approval_history = $this->tmreis_schools_common_model->get_approval_history($doc_id);

		array_push($approval_history,$approval_data);

  	  $existing_update = $this->tmreis_schools_common_model->update_request_submit_model($doc_data,$approval_history,$unique_id,$doc_id); 
  	  ...
  	  if ($existing_update ) // the information has therefore been successfully saved in the db
			{
		  	    ...
		  	    if(isset($chronic_identifiers) && !empty($chronic_identifiers))
				{
					...
					$send_msg = $this->tmreis_common_lib->send_message_to_doctors_update($request_type,$unique_id,$student_name,$total_diseaes);
						
				$fcm_notification = $this->tmreis_common_lib->fcm_message_notification_update($request_type,$unique_id,$student_name);
				$this->session->set_flashdata('success','Request Updated successfully !!');
				redirect('tmreis_schools/fetch_submited_requests_docs');
			}
			else
			{
				$this->session->set_flashdata('fail','Some thing went wrong! Try Again');
  	...
  	public function reports_display_ehr_uid_new_html_static_hs()
	{
        ...
        $this->data = $this->tmreis_schools_common_lib->reports_display_ehr_uid_new_html_static_hs($student_unique_id,$school_name);
		$this->data["username"] = $username.','.$school_name;
		if(isset($_POST['timee']))
		{
			$time = $_POST['timee'];
		...
		$this->_render_page('tmreis_schools/tmreis_reports_display_ehr_new_dashboard',$this->data);
	}
	public function get_students_load_ehr_doc_basic_dashboard($_id)
	{
		$docs = $this->tmreis_schools_common_model->get_students_load_ehr_doc_model($_id);
		...
		$this->_render_page('tmreis_schools/tmreis_schools_reports_display_ehr_new',$this->data);
	}
	...
	public function hb_pie_view()
	{
	  ...
	  //Fetch school details with school code
		...
		$this->data = $this->tmreis_schools_common_lib->hb_pie_view_lib($current_month, $school_name);
		$this->_render_page('tmreis_schools/hb_pie_view',$this->data);
	}
	...
	function drill_down_hb_to_students()
	{
		...
		$docs = $this->tmreis_schools_common_model->get_drill_down_to_hb_report($symptom_type ,$month ,$school_name);
	...
	function drill_down_to_hb_report_students()
	{
		...
		$students = $this->tmreis_schools_common_model->get_drilling_hb_students_docs($UI_id);
		...
		$this->_render_page('tmreis_schools/drill_down_to_hb_report_view',$this->data);
	}
	public function show_student_hb_graph($hospital_unique_id){
		...
		$hb_value = $this->tmreis_schools_common_model->get_student_hb_graph_values($hospital_unique_id);
	...
	$this->_render_page('tmreis_schools/show_student_hb_graph_view',$final_hb_data);
	...
	public function hb_pie_view_month_wise()
	{
		...
		$school_info = $this->tmreis_schools_common_model->get_school_info($school_code);
	  	$school_name = $school_info[0]['school_name'];
		$this->data = $this->tmreis_common_lib->hb_pie_view_lib_month_wise($current_month,$school_name);
		$this->output->set_output(json_encode($this->data));
	}
	...
	public function doctor_visit(){
		...
		$this->data['all'] = $this->tmreis_schools_common_model->get_doctor_visiting_report_date_wise($school_name);
	  	...
	  	$this->_render_page('tmreis_schools/doctor_visit', $this->data);
	}
	public function fetch_student_info_for_doctor_visit()
	{
		...
		$this->data['get_data'] = $this->tmreis_schools_common_model->fetch_student_info_for_doctor_visit($school_name, $unique_id);
		$this->data['get_attachments'] = $this->tmreis_schools_common_model->fetch_student_attachments($school_name, $unique_id);
 	...
 	public function create_doctor_visit_report()
  	{
	  	...
	  	$is_exists = $this->tmreis_schools_common_model->check_if_doc_exists_in_doctor_visiting_student($uniqueId);
		if(!empty($is_exists))
		{
        ...
        if(strpos($index,'hs_req_attachments')!== false)
      {
       ...
       $doc_history = $this->tmreis_schools_common_model->get_dr_attachments_history($uniqueId,$doc_id);
     
		$doc_attachments = [];
       	if(isset($doc_history[0]['doc_attachments']['Prescriptions']))
        {
               
                $prescription_merged_data = array_merge($doc_history[0]['doc_attachments']['Prescriptions'],$Prescriptions_external_final);
                $doc_attachments['Prescriptions'] = array_replace_recursive($doc_history[0]['doc_attachments']['Prescriptions'],$prescription_merged_data);
        ...
        else
        {
         ...
         if(isset($doc_history[0]['doc_attachments']['Lab_Reports']))
        {
               
                $Lab_Reports = array_merge($doc_history[0]['doc_attachments']['Lab_Reports'],$Lab_Reports_external_final);                
                $doc_attachments['Lab_Reports'] = array_replace_recursive($doc_history[0]['doc_attachments']['Lab_Reports'],$Lab_Reports);
        ...
        if(isset($doc_history[0]['doc_attachments']['Digital_Images']))
        {
                $digital_images_merged_data = array_merge($doc_history[0]['doc_attachments']['Digital_Images'],$Digital_external_final);
                $doc_attachments['Digital_Images'] = array_replace_recursive($doc_history[0]['doc_attachments']['Digital_Images'],$digital_images_merged_data); 
        ...
        if(isset($doc_history[0]['doc_attachments']['Payments_Bills']))
        {
                $payments_Bills_merged_data = array_merge($doc_history[0]['doc_attachments']['Payments_Bills'],$Payments_Bills_external_final);
                $doc_attachments['Payments_Bills'] = array_replace_recursive($doc_history[0]['doc_attachments']['Payments_Bills'],$payments_Bills_merged_data);
        }
        else
        {
         ...
         if(isset($doc_history[0]['doc_attachments']['Discharge_Summary']))
        {
                $discharge_Summary_merged_data = array_merge($doc_history[0]['doc_attachments']['Discharge_Summary'],$Discharge_Summary_external_final);
                $doc_attachments['Discharge_Summary'] = array_replace_recursive($doc_history[0]['doc_attachments']['Discharge_Summary'],$discharge_Summary_merged_data);
        }
        else
        {
                $doc_attachments['Discharge_Summary'] = $Discharge_Summary_external_final;
        }
        if(isset($doc_history[0]['doc_attachments']['external_attachments']))
        {
                $external_merged_data = array_merge($doc_history[0]['doc_attachments']['external_attachments'],$external_final);
                $doc_attachments['external_attachments'] = array_replace_recursive($doc_history[0]['doc_attachments']['external_attachments'],$external_merged_data);
        }
        else
        {
			...
			$existing_update = $this->tmreis_schools_common_model->update_doctor_treated_student($doc_attachments,$uniqueId,$doc_id);

				if ($existing_update ) // the information has therefore been successfully saved in the db
				{
					
					$this->session->set_flashdata('success','Good Job!!');
					redirect('tmreis_schools/doctor_visit');
				}
				else
				{
					$this->session->set_flashdata('fail','Some thing went wrong! Try Again');
        ...
        if(strpos($index,'hs_req_attachments')!== false)
		 {
	        ...
	        else
	        {
				...
				$app_properties['app_name'] = "TMREIS Doctor Visiting App";
				...
				$newly_created = $this->tmreis_schools_common_model->submit_doctor_visiting_report($doc_data,$doc_attachments, $doc_properties, $app_properties, $history);
	...
	public function drill_down_to_doctor_treated_list(){
		...
		$this->data['student_list'] = $this->tmreis_schools_common_model->drill_down_to_doctor_treated_list($school_name,$selectedDate);

		$this->data['doctor_visiting_date'] = $selectedDate;
	  	$this->_render_page('tmreis_schools/doctor_visit_student_list', $this->data);
	}
	public function show_doctor_treated_student(/*$uid,$doctor_visit_date*/ $doc_id)

	 {
	 	
		$this->data['unique_id'] = $this->tmreis_schools_common_model->show_doctor_treated_student(/*$uid, $doctor_visit_date*/$doc_id);
		
		
		$this->_render_page('tmreis_schools/show_doctor_visit_student',$this->data);
	}
}

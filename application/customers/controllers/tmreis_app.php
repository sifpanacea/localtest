...
class Tmreis_app extends My_Controller {
	...
	public function update_gcm_id()
    {
		...
		$response = $this->tmreis_common_model->updateGcmID($userid,$gcm_registration_id);
	...
	{
		log_message('debug','TMREIS_APP=====CHAT_ROOMS=====$user_id==360====='.print_r($user_id,true));
		log_message('debug','TMREIS_APP=====CHAT_ROOMS=====$message==361====='.print_r($message,true));
		log_message('debug','TMREIS_APP=====CHAT_ROOMS=====$message==362====='.print_r($_POST,true));
		
		if(($user_id === false) && ($message===false))
		{
			$response = array();
			// fetching all user tasks
			$result = $this->tmreis_common_model->get_all_groups();
			$response["error"] = false;
		...
		else if($message===false)
		{
			...
			$result = $this->tmreis_common_model->get_messages($user_id);
		...
		{
			$this->data = $this->tmreis_common_model->add_message($_POST,$user_id);

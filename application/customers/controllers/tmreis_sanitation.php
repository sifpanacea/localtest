...
class Tmreis_sanitation extends My_Controller {
	...
	{
		redirect('tmreis_sanitation/to_dashboard');
	}
	...
	function to_dashboard()
	{
		...
		$app_template = $this->tmreis_common_model->get_sanitation_report_app();
		...
		$this->data['sanitation_report_schools_list'] = $this->tmreis_common_model->get_sanitation_report_pie_schools_data($today_date);
		
		$this->data['today_date'] = $today_date;
		$this->data['distslist'] = $this->tmreis_common_model->get_all_district();
		
		$this->_render_page('tmreis_sanitation/admin_dash', $this->data);
	...
	function fetch_sanitation_report_against_date()
	{
	  ...
	  if(isset($_POST['selected_school']) && !empty($_POST['selected_school']))
	  {
	     ...
	     $sanitation_report_data = $this->tmreis_common_model->get_sanitation_report_data_with_date($date,$school);
		
	     $sanitation_report['report_data']  = json_encode($this->tmreis_common_lib->build_sanitation_report($sanitation_report_data));

	     $sanitation_report['schools_list'] = $this->tmreis_common_model->get_sanitation_report_pie_schools_data($date);
	  }
	  else
	  {
	  	 $sanitation_report['schools_list'] = $this->tmreis_common_model->get_sanitation_report_pie_schools_data($date);

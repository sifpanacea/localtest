<?php $current_page = "Student_Reports"; ?>
    		...
    		<div class="card">
    			...
    			<div class="header">
    				<h3 class="font-bold col-green">Total students : <span class="badge bg-teal">
						<?php if(!empty($studentscount)) {?><?php echo $studentscount;?><?php } else {?><?php echo "0";?><?php }?></span>
					</h3>
					...
					<div class="row clearfix">
						<div class="col-sm-3">
							<select id="collection_name" class="form-control">
								<option value="ttwreis_screening_report_col_2022-2023">Academic Year 2022-23</option>
								<option value="ttwreis_screening_report_col_2021-2022">Academic Year 2021-22</option>
								<option value="ttwreis_screening_report_col_2020-2021">Academic Year 2020-21</option>
								<option value="healthcare201671115519757">Academic Year 2019-20</option>
							</select>
						</div>
                        <div class="col-sm-3">
...
$(document).ready(function() {
	...
	function display_data_table(result){
		if(result.length > 0){
			data_table = '<table class="table table-bordered table-striped table-hover dataTable js-exportable" id="dt_basic">  <thead> <tr> <th>STUDENT NAME</th><th>HOSPITAL UNIQUE ID</th><th>FATHER NAME</th> <th>STUDENT SCHOOL</th><th>DATE OF BIRTH</th><th>CLASS</th><th>SECTION</th><th>MOBILE</th><th>Aadhar Card</th><th>Ration Card</th><th>Aarogya Sri</th><th>Photo</th><th>Action</th></tr> </thead> <tbody>';
			...
			$.each(result, function() {
				...
				data_table = data_table + '<td>'+this.doc_data.widget_data["page2"]['Personal Information']['Aadhar card'] + '</td>';	
				data_table = data_table + '<td>'+this.doc_data.widget_data["page2"]['Personal Information']['Ration card'] + '</td>';	
				data_table = data_table + '<td>'+this.doc_data.widget_data["page2"]['Personal Information']['Aarogya sri'] + '</td>';	
				...
				data_table = data_table + '<td><a class="btn btn-primary btn-xs" href="'+urlLink+'ttwreis_mgmt/ttwreis_reports_display_ehr_uid/?id = '+this.doc_data.widget_data["page1"]['Personal Information']['Hospital Unique ID']+'">Show EHR</a></td>';
			...
			data_table = data_table + '</tbody></table><div><button type="button" class="btn btn-primary pull-right" onclick="window.history.bacBGk();">Back</button></div>';
...
$('#get_excel').click(function(){
        ...
        $.ajax({        	
            ...
            type:'POST',
            data:{'dist_name':dist, 'school':scl ,'collection' : collection_name},
            success : function(data){               

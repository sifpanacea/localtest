					...
					<table id="datatable_fixed_column" class="table table-bordered table-striped table-hover dataTable js-exportable">
				        ...
				        <?php foreach ($news_feeds as $news_feed):?>
							<tr>
								...
								?></td>
								<td>
								<?php echo (strtotime($news_feed["display_date"]) >= strtotime(date("Y-m-d H:i:s", strtotime ( date("Y-m-d H:i:s") . '+6 hours' ))))? '<a class="btn btn-warning btn-xs" href="'. URL.'ttwreis_mgmt/edit_news_feed_view/'.$news_feed['_id'].'">Edit </a>' : "Time elapsed";?>
								 | 
								<a class="btn btn-danger btn-xs" href="<?php echo URL."ttwreis_mgmt/delete_news_feed/".$news_feed['_id'];?>">Delete</a>
								</td>
							</tr>
							<?php endforeach;?>
						</tbody>

...
<br><br>
	...
	<form style="display: hidden" action="<?php echo URL; ?>tmreis_mgmt/get_bmi_students_docs" method="POST" 
		id="bmi_report_form">
	  
	  <input type="hidden" id="bmi_type" name="bmi_type" value=""/>
	</form>
	<form style="display: hidden" action="<?php echo URL; ?>tmreis_mgmt/get_hb_students_docs" method="POST" 
		id="hb_report_form">
	  
	  <input type="hidden" id="hb_type" name="hb_type" value=""/>
	</form>
	<form style="display: hidden" action="<?php echo URL; ?>tmreis_mgmt/get_chronic_students_docs" method="POST" 
		id="chronic_report_form">
	  
	  <input type="hidden" id="chronic_type" name="chronic_type" value=""/>
	</form>
	<form style="display: hidden" action="<?php echo URL; ?>tmreis_mgmt/get_emergency_req_students_docs" method="POST" 
		id="emergency_report_form">
	  
	  <input type="hidden" id="emergencyReq" name="emergencyReq" value=""/>
	</form>
	<form style="display: hidden" action="<?php echo URL; ?>tmreis_mgmt/get_field_officer_docs" method="POST" 
...
function drill_down_request_to_districts(pie_data){
	...
	$.ajax({
		...
		type: 'POST',
		data: {"data" : JSON.stringify(pie_data.data), "today_date" : today_date,"dt_name" : dt_name, "school_name" : school_name, "request_pie_span" : request_pie_span, "request_pie_status":request_pie_status,"student_type" : stu_type,'student_age': stu_age},
		success: function (data) {
			console.log(data);
...
//			student_type = $("input[name ='student_type']:checked").val();
			...
			$.ajax({
				...
				var screened_count = '<h4>Eye Screened : '+counts.screened_count.eye_screened_count+'</h4><p></p><h4>Dental Screened : '+counts.screened_count.dental_screened_count+'</h4><p></p><h4>Attendance Submitted : '+counts.attendance_count+'</h4><p></p><h4>Sanitation Submitted : '+counts.sanitation_count+'</h4>'

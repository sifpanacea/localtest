...
$page_nav["clinic management"]["sub"]["list_clinic"]["active"] = true;
include("inc/nav.php");
				...
				<article class="col-xs-12 col-sm-12 col-md-12 col-lg-12">
					...
					<div class="jarviswidget jarviswidget-color-tlsGrey" id="wid-id-0" data-widget-editbutton="false">
						...
						<header>
							...
							<h2>All Clinics <span class="badge bg-color-greenLight"><?php if(!empty($clinicscount)) {?><?php echo $clinicscount;?><?php } else {?><?php echo "0";?><?php }?></span></h2>
					...
					<tr>
						...
						<td><?php echo ucwords($clinic["clinic_name"]) ;?></td>
						<td><?php echo $clinic["email"] ;?></td>
						<td><?php echo $clinic["mobile"] ;?></td>
						<td><?php echo $clinic["contact_person"] ;?></td>
						<td>
						    <?php echo ($clinic['active']) ? lang('index_active_link'):lang('index_inactive_link'); ?>
							
                        </td>
					
						<td disabled>
						<a class='btn btn-warning btn-xs' href='<?php echo URL."schoolhealth_admin_portal/view_clinic/".$clinic['_id'];?>'>View
                			<?php echo lang('app_list')?>
                			</a></td>
							
						<td><a class='edit' href='<?php echo URL."schoolhealth_admin_portal/edit_clinic/".$clinic['_id'];?>'>
                			<?php echo lang('app_edit')?>
                			</a></td>
							
						<td id="deleteclinic"><a class='ldelete' href='<?php echo URL."schoolhealth_admin_portal/delete_clinic/".$clinic['_id'];?>'>
					...
					<?php else: ?>
        			<p>
          				<?php echo "No clinics entered yet.";?>
        			</p>
        			<?php endif ?>
									</tbody>
									<?php if($links):?>
...
<script type="text/javascript">
	...
	// Delete My Apps 
	$('#deleteclinic a').click(function(e) {
	//get the link
	var $this = $(this);
	...
	// ask verification
	$.SmartMessageBox({
		title : "<i class='fa fa-minus-square txt-color-orangeDark'></i> Do you want to delete this clinic ?",
		buttons : '[No][Yes]'

	}, function(ButtonPressed) {
		if (ButtonPressed == "Yes") {
			setTimeout(deleteclinic, 1000)
		}

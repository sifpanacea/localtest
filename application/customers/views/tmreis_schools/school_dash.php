							...
							<div class="no-padding">
								...
								<div class="widget-body">
									...
									<div class="well well-sm well-light" id="news_feed_div" style="min-height: 245px;">
										...
										<table >
										<tr>
										<td> <li style="list-style-type:none"class="news-item"><i class="fa fa-bell" style="font-size:20px; color:red"></i>&nbsp;&nbsp; <?php echo $news_feed["display_date"].' <i class="fa fa-lg fa-fa-calendar"></i> '.((strlen($news_feed["news_feed"])>=500)? substr($news_feed["news_feed"], 0,500)." <font size='2' color='blue'><i>cont...</i></font>" : $news_feed["news_feed"]) ;?> 
						...
						<div>
							...
							<div class="widget-body no-padding smart-form" style="height:250px; overflow-y: scroll;">
								...
								<li>
										...
										<p>
											<strong><?php if(isset($doc['notification_param']['Unique ID'])):?><?php echo $doc['notification_param']['Unique ID'];?><?php else:?><?php echo "Notification Field";?><?php endIF;?></strong> - <?php echo $doc['doc_received_time'];?>&nbsp;&nbsp;&nbsp;<a href="<?php echo URL.'tmreis_schools/access_request/'.$doc['doc_id'].'';?>" class="btn bg-color-greenDark txt-color-white btn-xs">Access</a>
										</p>
									</li>
							    <?php endforEach;?>
								<?php else: ?>
...
$(document).ready(function() {
	...
	//var absent_report_submitted_school = <?php echo $absent_report; ?>;
...
$('#request_pie_span').change(function(e){
	...
	$.ajax({
		...
		success: function (data) {			
			...
			$( "#pie_request" ).empty();
			$( "#pie" ).empty();
			console.log(data,"datataatataaaa");
			data = $.parseJSON(data);
			console.log(data,"datataatataaaa");
 ...
 function draw_absent_pie()
{
	...
	if(absent_data == 1)
	{
		console.log('absent_data == 1',absent_data)
		$("#pie_absent").append('No positive values to dispaly');
	}
	else if(absent_data == 2)
	{
		$("#pie_absent").append('<strong><i style="font-size: initial";>Attendance Details submitted, but no issues found('+" "+today_date+')</i></strong>');
	}
	else
	{
	  console.log('absent_data == else part',absent_data)
...
function drill_down_request_to_students(request_label)
{
	...
	$.ajax({
		...
		error:function(XMLHttpRequest, textStatus, errorThrown)
		{
		 console.log('erroreeeeeee', errorThrown);
		}
	});
}
...
{
	console.log("in drill_down_screening_to_abnormalities-------------aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa-------------",pie_data);
	$.ajax({
...
{
console.log("in drill_down_screening_to_students-------------aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa-------------",pie_data);
	$.ajax({

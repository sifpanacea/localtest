...
<header>
	...
	<h2>Select a clinic | Total clinics : <span class="badge bg-color-greenLight">
							<?php if(!empty($clinicscount)) {?><?php echo $clinicscount;?><?php } else {?><?php echo "0";?><?php }?></span></h2>
...
<div>
	...
	<div class="widget-body">
		...
		<div id="stud_report">
		Select from drop down to display clinics.
		</div>
...
$(document).ready(function() {
	...
	else
	{
			...
			$.SmartMessageBox({
				title : "Alert !",
				content : "This will take long time to display clinics report. Are you sure you want to continue ?",
				buttons : '[No][Yes]'
	...
	$('#select_dt_name').change(function(e)
	{
		...
		else
		{
			...
			}, function(ButtonPressed) 
			{
				...
				{
					$.ajax({
						url  : 'get_all_clinics_list',
						type : 'POST',
						success: function (data) 
						{			
							result = $.parseJSON(data);
							console.log(result);
							display_all_clinics_list(result);
	...
	function display_clinics_list(result)
	{
		...
		{
			$("#stud_report").html('<h5>No clinics to display for this district</h5>');
		}
	}
	
	function display_all_clinics_list(result)
	{
		if(result.length > 0)
		{
			data_table = '<table id="datatable_fixed_column" class="table table-striped table-bordered" width="100%">  <thead> <tr> <th class="hasinput" style="width:17%"> <input type="text" class="form-control" placeholder="Filter STATE" /> </th><th class="hasinput" style="width:17%"> <input type="text" class="form-control" placeholder="Filter DISTRICT" /> </th><th class="hasinput" style="width:17%"> <input type="text" class="form-control" placeholder="Filter CLINIC NAME" /> </th> <th class="hasinput" style="width:17%"> <input type="text" class="form-control" placeholder="Filter CONTACT PERSON" /> </th> <th class="hasinput" style="width:17%"> <input type="text" class="form-control" placeholder="Filter MOBILE" /> </th> <th class="hasinput" style="width:17%"> <input type="text" class="form-control" placeholder="Filter ADDRESS" /> </th></tr> <tr> <th>STATE</th> <th>DISTRICT</th> <th>CLINIC NAME</th> <th>CONTACT PERSON</th> <th>MOBILE</th> <th>ADDRESS</th></tr> </thead> <tbody>';

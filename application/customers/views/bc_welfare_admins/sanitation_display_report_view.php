...
<br>
      ...
      <!--  <section class="content"> -->
        <div class="container-fluid">
           ...
           <div class="row clearfix">
                ...
                <input type="hidden" name="sanitation" id="sanitation" value="<?php echo $sanitaion; ?>">
                   <div class="card">
                    <div class="header">
                             <div class="row clearfix">
                                 <div class="col-sm-2">
                                    <h3 class="font-bold col-green">Sanitation Data</h3>
                                 </div>
 ...
 -->
    ...
    <form style="display: hidden" action="<?php echo URL; ?>bc_welfare_mgmt/show_sanitation_submitted_pics" method ="POST" id="more_req_students_form">
    <input type="hidden" id="photos_sclname" name="school_name" value=""/>
    <input type="hidden" id="photos_date" name="school_date" value=""/>
    </form>
...
<?php include("inc/message_status.php"); ?>
    ...
    <!-- Select Plugin Js -->
    <script src="<?php echo(MDB_PLUGINS.'bootstrap-select/js/bootstrap-select.js'); ?>"></script>
  ...
  <!-- Jquery DataTable Plugin Js -->
    ...
    <script src="<?php echo(MDB_JS.'pages/ui/tooltips-popovers.js'); ?>"></script>
   ...
   // var sanitation = $('#sanitation').val();
     ...
     /* 
      ...
      ('#sanitation_filters').hide();*/
        $.ajax({
            url : 'get_sanitation_data_schools_data',
            type : 'POST',
            data : {"today_date" : date_for, "sanitation_type": sanitation},
    ...
    $('#date_set').click(function(){
     ...
     function data_table_req(result){
        ...
        if(result){
            ...
            data_table = '<table class="table table-bordered table-striped table-hover dataTable js-exportable" id="more_requests"><thead><tr><th>School Name</th><th>Campus</th><th>Kitchen & Wellness center</th><th>Toilets</th><th>Show Photos</th></tr></thead><tbody>';
        ...
        $("#more_requests").each(function(){
            $('.ehrButton').click(function (){
                ...
                var schoolNameClik =currentRow.find("td:eq(0)").text(); // get current row 2nd TD
                var date_for = $('#date').val(); //selected date
                //alert(schoolNameClik);
                //alert(date_for);
                $("#photos_sclname").val(schoolNameClik);
                $("#photos_date").val(date_for);
    ...
    function data_table_for_filters(result, requestsCount){
      ...
      if(result){
            ...
            if(requestsCount == 'Not Submitted'){
              var scl_head = '<th>S NO</th>';
              var count_head = '<th>School Name</th>';
            }else{
              var scl_head = '<th>School Name</th>';
              var count_head = '<th>Count</th>';
            }


            data_table = '<table class="table table-bordered table-striped table-hover dataTable js-exportable" id="dt_basic"><thead><tr>'+scl_head+count_head+'<th>Respond '+'<input type="checkbox" id="select_all_chk" class="filled-in chk-col-red">'+
            ...
            if(requestsCount == 'Animals'){
              ...
              data_table = data_table+'<td>'+'<div class="demo-checkbox">'+
                                                '<input type="checkbox" name="checkboxName[]" id="'+index+'" class="filled-in chk-col-red" />'+
                                                '<label for="'+index+'">Send a message</label>'+
                                            '</div>'+'</td>';

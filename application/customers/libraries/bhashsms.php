...
class Bhashsms {
	...
	{
        $response = $this->send_sms_bsmsg($recerverNO,$message);
		...
		$url = "http://bhashsms.com/api/sendmsg.php".$curl_data;
         ...
         //"http://tswreis.telangana.gov.in/SendSMS?wsdl";
    ...
    function send_sms_bsmsg($recerverNO,$message)
    {
    //log_message('error','Mobile number:'.$recerverNO.'  Message: '.$message);
      //  $message_urlencode=rawurlencode($message);
       ...
       // $password = $this->_config['pass'];
        ...
        $url="https://www.bulksmsgateway.in/sendmessage.php?user=".urlencode($username)."&password=".urlencode($password)."&mobile=".urlencode($recerverNO)."&sender=".urlencode($sender)."&message=".urlencode($message)."&type=".urlencode('3');
       // log_message('error','Mobile number:'.$recerverNO.' url Message: '.$url);
        $chl = curl_init($url);
        curl_setopt($chl, CURLOPT_RETURNTRANSFER, true);
        $curl_scraped_page = curl_exec($chl);
        curl_close($chl); 
     //   $curl_data = "?user=".$this->_config['user']."&pass=".$this->_config['pass']."&sender=".$this->_config['sender']."&phone=".$recerverNO."&text=".$message_urlencode."&priority=".$this->_config['priority']."&stype=".$this->_config['stype'];
        
    //    $url = "http://bhashsms.com/api/sendmsg.php".$curl_data;
       // log_message('error','urlllllllllllllllllllllllllllllllllllllllllllllllllllllllll'.$url);
    //    $request =  $url; 
    //    $response = file_get_contents($request);

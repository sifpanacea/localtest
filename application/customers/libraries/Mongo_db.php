...
class Mongo_db
{
   ...
   protected $_configData = array(
	...
	// private $_config_file_mongo = 'mongodb';
    /**
     * Connection resource.
     *
     * @var null|\Mongo
	...
	protected $_config_file_mongo = 'mongodb';
    ...
    public function __construct($config = array(), $connect = true)
    {
		...
		$this->CI->config->load($this->_config_file_mongo);
		$this->_config = $this->CI->config->item('default'); 
        $this->setConfig($this->_config ,$connect); 
    ...
    public function setConfig($config = array(), $connect = true)
    {
        if (is_array($config)) {
            ...
            throw new My_Exception('No config variables passed');
        }
	...
	public function setConfigRun($config = array(), $connect = true)
    {
        if (is_array($config)) {
        ...
        log_message('debug','config data at run timeeeeeeeeeeeeeeeeeeeee'.print_r($this->_configData,true));
        $this->_connectionString();
    ...
    {
        if (empty($dsn)) {
            throw new My_Exception('To switch MongoDB databases, a
        ...
        try {
            ...
            throw new My_Exception('Unable to switch Mongo Databases: ' .
             $Exception->getMessage());
        }
    }
	
	 public function switchDatabase($dsn = '')
    {
        if (empty($dsn)) {
            throw new My_Exception('To switch MongoDB databases, a
             DSN must be specified');
        ...
        try {
            // Regenerate the connection string and reconnect
            //$this->_config['dsn'] = $dsn;
            $this->_connectionStringswitchdb($dsn);
            $this->_connect();
        } catch (MongoConnectionException $Exception) {
            throw new My_Exception('Unable to switch Mongo Databases: ' .
             $Exception->getMessage());
    ...
    */
    public function dropDb($database = '')
    {
        if (empty($database)) {
           ...
           throw new My_Exception('Failed to drop MongoDB database because
            name is empty');
            ...
            // @codeCoverageIgnoreStart
                ...
                throw new My_Exception('Unable to drop Mongo database `' .
    ...
    public function dropCollection($collection = '')
    {
        ...
        if (empty($collection)) {
            throw new My_Exception('Failed to drop MongoDB collection because
             collection name is empty', 500);
        } else {
            try {
                $this->_dbhandle->{$collection}->drop();

                return true;
            }
            // @codeCoverageIgnoreStart
            catch (Exception $Exception) {
                throw new My_Exception('Unable to drop Mongo collection `' .
    ...
    public function create_collection($collection = '')
    {
    	...
    	if (empty($collection)) {
    		throw new My_Exception('Failed to create MongoDB collection because
             collection name is empty', 500);
    	} else {
    		try {
    			$this->_dbhandle->createCollection($collection);
    
    			return true;
    		}
    		// @codeCoverageIgnoreStart
    		catch (Exception $Exception) {
    			throw new My_Exception('Unable to create Mongo collection `' .
    					$collection . '`: ' . $Exception->getMessage(), 500);
    			// @codeCoverageIgnoreEnd
    		}
    	}
    ...
    public function whereInAll($field = '', $inValues = array())
    {
    	...
    	if (is_array($field)) {
    		foreach ($field as $where => $value) {
    			$this->_whereInit($where);
    			$this->wheres[$where]['$all'] = $value;
    		}
    	} else {
    		$this->_whereInit($field);
     ...
     * @return \MongoQB\Builder
     */
    public function whereNe($field = '', $value = null)
    {
    	
    	if (is_array($field)) {
    		foreach ($field as $where => $value) {
    			$this->_whereInit($where);
    			$this->wheres[$where]['$ne'] = $value;
    		}
    	} else {
    		$this->_whereInit($field);
    ...
    */
    ...
    {
        if (empty($collection)) {
            throw new My_Exception('In order to retrieve documents from
        ...
        while ($cursor->hasNext()) {
            ...
            // @codeCoverageIgnoreStart
                ...
                throw new My_Exception($Exception->getMessage());
                // @codeCoverageIgnoreEnd
            }
        }
    ...
    */
    ...
    {
        if (empty($collection)) {
            throw new My_Exception('In order to retrieve a count of
    ...
    {
        if (empty($collection)) {
            throw new My_Exception('No Mongo collection selected to insert
             into');
        }

        if (count($insert) === 0 OR ! is_array($insert)) {
            throw new My_Exception('Nothing to insert into Mongo collection
        ...
        // @codeCoverageIgnoreStart
            ...
            throw new My_Exception('Insert of data into MongoDB failed: ' .
             $Exception->getMessage());
            // @codeCoverageIgnoreEnd
        }
    }
...
/**
     * InsertTemp to insert template json in database against appid.
     *
     * Insert a new document
     *
     * @param string $collection Name of the collection
     * @param array  $insert     The document to be inserted
	 * @param int  $_id     The largest value in _id
     * @param array  $options    Array of options
     *
     * @access public
     * @return boolean
	...
	public function insertWork($collection = '', $insert = array(), $_id, $appName,
     $options = array())
	 {
		if (empty($collection)) {
            throw new My_Exception('No Mongo collection selected to insert
    ...
    /**
     * Insert Document to insert incremented _id field.
     *
     * Insert a new document
     *
     * @param string $collection Name of the collection
	...
	public function insertDocument($collection = '', $insert = array(),$_id,$id, $appName, $ver,$options = array())
	 {
		...
		$insert['app_properties']['app_name'] = $appName;
		$insert['doc_properties']['doc_id']   = $id;
		$insert['doc_properties']['status']   = 1;
		$insert['doc_properties']['_version'] = $ver + 1;
...
/**
     * Insert Document from web stage
     *
     * Insert a new document
     *
     * @param string  $collection  Name of the collection
     ...
     * @param int     $ver         version of the document 
    ...
    public function insert_document_from_web_stage($collection = '', $insert = array(), $id, $_id = false, $appName, $ver, $appid,$total_pages = false,$options = array())
     {
        if (empty($collection)) {
            throw new My_Exception('No Mongo collection selected to insert
             into');
        ...
        $insert['app_properties']['app_name']    = $appName;
        $insert['app_properties']['app_id']      = $appid;
        $insert['doc_properties']['doc_id']      = $id;
        $insert['doc_properties']['status']      = 1;
        $insert['doc_properties']['_version']    = $ver + 1;
		$insert['doc_properties']['total_pages'] = $total_pages;
	...
	public function insertDocument_ver($collection = '', $insert = array(), $_id, $id, $appName, $ver, $options = array())
	 {
		if (empty($collection)) {
            throw new My_Exception('No Mongo collection selected to insert
             into');
	...
	public function insertTemp($collection = '',$app_id, $apptemplate, $appName,$appDescription, $apptype, $appexpiry,$appcategory,$useremail,$pagenumber,$version,$status,
     $options = array())
	 {
		if (empty($collection)) {
            throw new My_Exception('No Mongo collection selected to insert
      ...
      /*  if (count($insert) === 0 OR ! is_array($insert)) {
        ...
        $insert['_id']                 = $app_id;
		$insert['app_name']            = $appName;
		$insert['app_template']        = $apptemplate;
		$insert['app_description']     = $appDescription;
		$insert['app_type']            = $apptype;
		$insert['app_expiry']          = $appexpiry;
		$insert['app_category']        = $appcategory;
		$insert['time']                = date('Y-m-d H:i:s');
		$insert['created_by']          = $useremail;
        $insert['pages']               = $pagenumber;
		$insert['_version']            = $version;
...
/**
    ...
    public function insert_application_in_collection($collection = '',$app_id, $apptemplate, $appName,$appDescription, $apptype, $appexpiry,$appcategory,$useremail,$pagenumber,$version,$status,$print_template,$notify_param,$application_header,$headertype,$blank_app)
     {
        if (empty($collection)) {
            throw new My_Exception('No Mongo collection selected to insert
             into');
        ...
        $insert['app_description']     = $appDescription;
        $insert['app_type']            = $apptype;
		$insert['use_profile_header']  = $headertype;
		$insert['blank_app']		   = $blank_app;
        $insert['app_expiry']          = $appexpiry;
        $insert['app_category']        = $appcategory;
        $insert['time']                = date('Y-m-d H:i:s');
        $insert['created_by']          = $useremail;
        ...
        $insert['_version']            = $version;
        $insert['status']              = $status;
        $insert['print_template']      = $print_template;
        $insert['notify_parameters']   = $notify_param;
        $insert['application_header']  = $application_header;
...
/**
     * analytics data collection which contains application id,application name,pages saved and trees saved count.
     *
     * 
     *
     * @param string $collection Name of the collection
	...
	public function create_analytics_data_collection($collection = '',$appname,$version,$appid,
     $options = array())
	 {
		if (empty($collection)) {
            throw new My_Exception('No Mongo collection selected to insert
		...
		$insert['app_name']     = $appname;
		$insert['_version']     = $version;
		$insert['pages_saved']  = "0";
		$insert['trees_saved']  = "0";
       ...
       public function insert_appcreation_event_into_calendar_collection($useremail,$appname,$appexpiry)
	   {
	     $usercollection = str_replace("@","_",$useremail);
	     $collection = $usercollection.'_calendar_events';
	     if (empty($collection)) {
            throw new My_Exception('No Mongo collection selected to insert
             into');
        }
        ...
        $insert['title']           = "App Created ! !";
		$insert['description']     = $appname;
	    $insert['allDay']          = "true";
        $insert['className']       = "bg-color-blueLight txt-color-white";
        $insert['end']             = "";
        $insert['icon']            = "fa-info";
        $insert['id']              = "1";
...
/**
     * InsertStut to insert template json in database against appid.
     *
     * Insert a new document
     *
     * @param string $collection Name of the collection
	...
	public function insertStut($collection = '', $appID, $_id, $app_name, $options = array())
	 {
		if (empty($collection)) {
            throw new My_Exception('No Mongo collection selected to insert
             into');
	...
	public function insertImg($collection = '', $insert = array(), $_id, $appName,
     $options = array())
	 {
		if (empty($collection)) {
            throw new My_Exception('No Mongo collection selected to insert
    ...
    {
        if (empty($collection)) {
            throw new My_Exception('No Mongo collection selected to
             update');
        }

        if (count($this->updates) === 0) {
            throw new My_Exception('Nothing to update in Mongo collection or
        ...
        // @codeCoverageIgnoreStart
            ...
            throw new My_Exception('Update of data into MongoDB failed: ' .
             $Exception->getMessage());
            // @codeCoverageIgnoreEnd
        }
    }
    ...
    {
        if (empty($collection)) {
            throw new My_Exception('No Mongo collection selected to delete
             from');
        }
        ...
        // @codeCoverageIgnoreStart
            ...
            throw new My_Exception('Delete of data into MongoDB failed: ' .
             $Exception->getMessage());
            // @codeCoverageIgnoreEnd
        }
    }
    ...
    public function command($query = array())
    {
        ...
        // @codeCoverageIgnoreStart
            ...
            throw new My_Exception('MongoDB command failed to execute: ' .
             $Exception->getMessage());
            // @codeCoverageIgnoreEnd
        }
    }
    ...
    {
        if (empty($collection)) {
            throw new My_Exception('No MongoDB collection specified to add
             index to');
        }

        if (empty($fields) OR ! is_array($fields)) {
            throw new My_Exception('Index could not be added to MongoDB
        ...
        // @codeCoverageIgnoreStart
            ...
            throw new My_Exception('An error occurred when trying to add an
    ...
    {
        if (empty($collection)) {
            throw new My_Exception('No Mongo collection specified to remove
             index from');
        }

        if (empty($keys) OR ! is_array($keys)) {
            throw new My_Exception('Index could not be removed from MongoDB
             Collection because no keys were specified');
        }
       ...
       if ($this->_dbhandle->{$collection}->deleteIndex($keys)) {
            ...
            // @codeCoverageIgnoreStart
            throw new My_Exception('An error occurred when trying to remove
     ...
     * @return array|object
     */
    public static function dbstats($dbname)
    {
        
       return $this->_dbhandle->{$dbname}->dbstats();
    ...
    private function _connect()
    {
        ...
        // @codeCoverageIgnoreStart
                ...
                throw new My_Exception('Unable to connect to MongoDB: ' .
                 $Exception->getMessage());
                // @codeCoverageIgnoreEnd
        }
    }
    ...
    private function _connectionString()
    {
        ...
        if (empty($this->_dsn)) {
            throw new My_Exception('The DSN is empty');
        }

        $this->_persist = $this->_configData['mongo_persist'];
        $this->_persistKey = trim($this->_configData['mongo_persist_key']);
        $this->_replicaSet = $this->_configData['mongo_replica_set'];
        $this->_querySafety = trim($this->_configData['mongo_query_safety']);

        $parts = parse_url($this->_dsn);

        if ( ! isset($parts['path']) OR str_replace('/', '', $parts['path']) === '') {
            throw new My_Exception('The database name must be set in the DSN string');
        }
	 ...
	 private function _connectionStringswitchdb($x='')
    {
        $this->_dsn = trim($x);
    ...
    private function _updateInit($field = '')
    {
     ...
     @access private
     */
   /* private function _connectswitchdb($x='')
    {
        $options = array();
        ...
        try {
            // @codeCoverageIgnoreStart
            if (phpversion('Mongo') >= 50)
            {
                unset($options['persist']);
				log_message('debug','_connect if part dsnnnnnnnn'.print_r($this->_dsn,true));
				log_message('debug','_connect if part optionsssssssssss'.print_r($options,true));
				log_message('debug','_connect if part dbnameeeeeeeeee'.print_r($this->_dbname,true));
            ...
            {
			    log_message('debug','_connect else part dsnnnnnnnn'.print_r($x,true));
				log_message('debug','_connect else part optionssssssssssssss in new functionnnnnn'.print_r($options,true));
				log_message('debug','_connect else part dbnameeeeeeeeeeeeeeeee in new function'.print_r($this->_dbname,true));
				
                $this->_connection = new Mongo($x, $options);
                $this->_dbhandle = $this->_connection->{$this->_dbname};
            }
            // @codeCoverageIgnoreEnd

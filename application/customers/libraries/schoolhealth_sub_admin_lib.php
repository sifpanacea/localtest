...
class Schoolhealth_sub_admin_lib 
{
	...
	*/
	...
	public function __construct()
	{
		...
		$this->ci->load->model('schoolhealth_sub_admin_portal_model');
		$this->ci->load->model('schoolhealth_school_portal_model');
		$this->ci->load->library('paas_common_lib');
		$this->ci->load->model('healthcare/healthcare20161014212024617_mod','healthcare20161014212024617_mod_calls');
	...
	function to_dashboard($date = FALSE, $request_duration = "Monthly", $screening_duration = "Yearly")
	{
		...
		$this->data['stateslist']      = $this->ci->schoolhealth_sub_admin_portal_model->get_all_states();
		//log_message('debug','$screening_report=====160=='.print_r($this->data['stateslist'],true));
		$this->data['absent_report']   = 1;
		$this->data['request_report']  = 1;
		$this->data['symptoms_report'] = 1;
	...
	{
		$docs = $this->ci->schoolhealth_sub_admin_portal_model->get_reports_ehr_uid($post['uid']);
	...
	function update_screening_pie($date = FALSE,$screening_pie_span = "Yearly")
	{
	    ...
	    $count = 0;
		$screening_report = $this->ci->schoolhealth_sub_admin_portal_model->get_all_screenings($date,$screening_pie_span);
		foreach ($screening_report as $value){
			$count = $count + intval($value['value']);
		}
		if($count > 0){
	 ...
	 function school_screening_file_import($post)
    {
		...
		$config['upload_path'] 		= $uploaddir;
		$config['upload_mednote'] = './uploaddir/public/uploads/healthcare20161014212024617_con/MedNote';
		...
		//$_FILES['userfile'] = $this->reArrayFiles($_FILES['userfile']);
			 ...
			 if ( ! $this->ci->upload->do_upload())
			{
				$error = array('error' => $this->ci->upload->display_errors());
				 $this->ci->_render_page('schoolhealth/sub_admins/schools_view',$error);
			}
		   else
			{
				...
				if ($zip->open($file) === TRUE) {
						 $zip->extractTo('./uploaddir/public/uploads/healthcare20161014212024617_con/');
			  ...
			  $is_delete =  $this->deleteAll($config['upload_mednote']);
			   ...
			   else
			   {
				...
				return TRUE;
			   $this->data['message'] = "";
			   $this->ci->_render_page('schoolhealth/sub_admins/zipfile_import_view',$this->data);
			}
	...
	{  
			...
			$photo_status = "";
					 ...
					 $configUpload['upload_path'] = './uploaddir/public/uploads/healthcare20161014212024617_con/MedNote/Screening.xls';
					...
					//Load excel file
					  $objPHPExcel = PHPExcel_IOFactory::load("./uploaddir/public/uploads/healthcare20161014212024617_con/MedNote/Screening.xls");
					...
					for($each_row=2 ; $each_row<=$total_rows ; $each_row++)
					{
						...
						foreach ($cellIterator as $cell)
						{
							...
							if($check_col_array[$header_row] == "history")
							{
								...
								////log_message("debug","history_time=====116".print_r($history_time,true));
								...
								{ 
									if(!empty($data_value)) // phone number is not empty
									{
										 ...
										 else // phone number is not valid
										{
										 ...
										 //redirect(base_url("zipfile_controller/zipfile_home"));
										}
							...
							else if($check_col_array[$header_row] == "imagepath")
							{
								if(!empty($data_value))
								{
								$photo_folder_path = EXL_PHOTO_PATH;
								$photo_folder_upload_test = IMPORT_STUDENT_PHOTO_PATH;
							...
							else if($check_col_array[$header_row] == "general physian sign")
							{
								if(!empty($data_value))
								{
								$sign_folder_path = SIGNATURES_PRIVATE;
								$sign_folder_upload_test = DOCTORS_SIGN_PRIVATE;
							...
							else if($check_col_array[$header_row] == "dentist sign")
							{
								if(!empty($data_value))
								{
								$dentist_sign_folder_path = SIGNATURES_PRIVATE;
								$dentist_sign_folder_upload_test = DOCTORS_SIGN_PRIVATE;
							...
							else if($check_col_array[$header_row] == "opthomologist sign")
							{
								if(!empty($data_value))
								{
								$opthomologist_sign_folder_path = SIGNATURES_PRIVATE;
								$opthomologist_sign_folder_upload_test = DOCTORS_SIGN_PRIVATE;
							...
							else if($check_col_array[$header_row] == "audiologist sign")
							{
								if(!empty($data_value))
								{
								$audiologist_sign_folder_path = SIGNATURES_PRIVATE;
								$audiologist_sign_folder_upload_test = DOCTORS_SIGN_PRIVATE;
							...
							}else if($check_col_array[$header_row] == "attachments")
							{
								...
								if(!empty($data_value))
								{
								$attachments_sign_folder_path = EXL_ATTACHMENTS_PATH_PRIVATE;
								$attachments_sign_folder_upload_test = IMPORT_ATTACHMENTS_PRIVATE;
					...
					for($i=2;$i<count($arr_data)+2;$i++)
					{
					 ...
					 $LastName = ($arr_data[$i]['lastname'] )? $arr_data[$i]['lastname'] : "" ; //Excel Column 1
					  ...
					  $Address= ($arr_data[$i]['address']) ? $arr_data[$i]['address'] : "" ;  //Excel Column 4 */ 
					    $result = $this->ci->schoolhealth_sub_admin_portal_model->unique_id_check($arr_data[$i]['hospital unique id']);
						...
						$doc_data['widget_data']['page3']['Physical Exam']['BMI%'] = ($arr_data[$i]['bmi']) ? floatval($arr_data[$i]['bmi']) : "";
						...
						$doc_data['widget_data']['page4']['Doctor Check Up']['Advice'] = ($arr_data[$i]['advice']) ? $arr_data[$i]['advice'] : "";
						...
						$db_doc = $this->ci->schoolhealth_sub_admin_portal_model->get_reports_ehr_uid($arr_data[$i]['hospital unique id']);
						 ...
						 if($result == "Only personal info document")
						  {
							  //echo print_r("only personal information",true);
								//exit();
							  $this->ci->schoolhealth_sub_admin_portal_model->update_screening_details($db_doc['screening'][0]["_id"],$data_user);
							  ...
							  $company_details = $this->ci->healthcare20161014212024617_mod_calls->fetch_company_details_of_enterprise_admin($company_name);
							  
							   $student_exists = $this->ci->healthcare20161014212024617_mod_calls->check_if_student_exists($data_user['doc_data']['widget_data']['page1']['Personal Information']['Hospital Unique ID']);
							  
							  if(!$student_exists)
							  {
								$this->ci->healthcare20161014212024617_mod_calls->add_student_into_login_collection($data_user,$company_details);
							  } 
							 
								$this->ci->schoolhealth_sub_admin_portal_model->insert_screening_details($data_user);

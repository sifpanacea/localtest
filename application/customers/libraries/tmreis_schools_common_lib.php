...
class Tmreis_schools_common_lib 
{
	...
	*/
	...
	public function __construct()
	{
		...
		$this->ci->load->model('tmreis_schools_common_model');
	...
	public function classes($school_code)
	{
		...
		$total_rows = $this->ci->tmreis_schools_common_model->classescount($school_code);
		...
		$this->data['classes'] = $this->ci->tmreis_schools_common_model->get_classes($config['per_page'], $page, $school_code);
	...
	public function sections($school_code)
	{
		...
		$total_rows = $this->ci->tmreis_schools_common_model->sectionscount($school_code);
		...
		//find all the categories with paginate and save it in array to past to the view
		$this->data['sections'] = $this->ci->tmreis_schools_common_model->get_sections($config['per_page'], $page, $school_code);
		//create paginate´s links
		$this->data['links'] = $this->ci->pagination->create_links();
	...
	{
		$docs = $this->ci->tmreis_schools_common_model->get_reports_ehr_uid($unique_id,$school_name);
	...
	function to_dashboard($date = FALSE, $request_duration = "Monthly", $screening_duration = "Yearly")
	{
		...
		$school_color_code = $this->ci->tmreis_schools_common_model->get_school_color_code($school_code);		
		$this->data['school_color_code'] = $school_color_code;
		
		$school_name = $this->ci->tmreis_schools_common_model->get_school_details_for_school_code($school_code);
		
		$hs_req_docs  = $this->ci->tmreis_schools_common_model->get_hs_req_docs($collection);
		...
		$absent_report = $this->ci->tmreis_schools_common_model->get_all_absent_data($date,$school_name['school_name']);
		log_message('debug','get_attendance_submitted_school_name==absent_report==302'.print_r($absent_report,true));

		$submit_report = $this->ci->tmreis_schools_common_model->get_attendance_submitted_school_name($date,$school_name['school_name']);
		...
		//$this->data['absent_report_schools_list'] = $this->ci->tmreis_schools_common_model->get_absent_pie_schools_data($date);
		
		$this->data['request_report']   = 1;
		$this->data['screening_report'] = 1; 
		$this->data['symptoms_report']  = 1;
		
		$sanitation_infra_report = $this->ci->tmreis_schools_common_model->get_sanitation_infrastructure_model($school_name['dt_name'],$school_name['school_name']);
		...
		$symptoms_report = $this->ci->tmreis_schools_common_model->get_all_symptoms($date,$request_duration,$unique_id_pattern);
		...
		$count = 0;
		$request_report = $this->ci->tmreis_schools_common_model->get_all_requests($date,$request_duration,$unique_id_pattern);
		...
		$count = 0;
		$screening_report = $this->ci->tmreis_schools_common_model->get_all_screenings($date,$screening_duration);
		...
		$chronic_ids = $this->ci->tmreis_schools_common_model->get_all_chronic_unique_ids_model($school_name['school_name']);
		
		$this->data['chronic_ids'] = json_encode($chronic_ids);
		
		$this->data['last_screening_update'] = $this->ci->tmreis_schools_common_model->get_last_screening_update(); 

		$this->data ['news_feeds'] = $this->ci->tmreis_schools_common_model->get_all_news_feeds($date);
	...
	function to_dashboard_with_date($date = FALSE, $request_duration = "Monthly", $screening_duration = "Yearly")
	{
		...
		$this->data ['news_feeds'] = json_encode($this->ci->tmreis_schools_common_model->get_all_news_feeds($date));
	...
	function update_request_pie($date = FALSE,$request_pie_span  = "Monthly")
	{
		...
		$count = 0;
		$symptoms_report = $this->ci->tmreis_schools_common_model->get_all_symptoms($date,$request_pie_span,$unique_id_pattern);
		foreach ($symptoms_report as $value){
			$count = $count + intval($value['value']);
		}
		if($count > 0){
		...
		$count = 0;
		$request_report = $this->ci->tmreis_schools_common_model->get_all_requests($date,$request_pie_span,$unique_id_pattern);
		foreach ($request_report as $value){
			$count = $count + intval($value['value']);
		}
		if($count > 0){
	...
	$count = 0;
		$screening_report = $this->ci->tmreis_schools_common_model->get_all_screenings($date,$screening_pie_span);
		foreach ($screening_report as $value){
			$count = $count + intval($value['value']);
		}
		if($count > 0){
	...
	{
		$docs = $this->ci->tmreis_schools_common_model->get_update_personal_ehr_uid($post);
		$this->data['docs'] = $docs['screening'];
	...
	public function bmi_pie_view_lib($current_month, $school_name){
		...
		$count = 0;
		$bmi_report = $this->ci->tmreis_schools_common_model->get_bmi_report_model($current_month, $school_name);
	...
	public function generate_bmi_report_to_excel_lib($date, $school_name)
	{
		...
		$objPHPExcel->getProperties()->setDescription("BMI report of TMREIS School");
		...
		$data = $this->ci->tmreis_schools_common_model->export_bmi_reports_monthly_to_excel( $date,$school_name);
		...
		if ($this->ci->upload->do_upload("file"))
		{
			...
			if (count($check)==0) {
					...
					foreach ($cellIterator as $cell)
					{
						...
						if($check_col_array[$header_row] == "height in cms")
						{
							...
							if(preg_match('/\.+/i', $data_value) || empty($data_value))
							{
								...
								$this->data['distslist'] = $this->ci->tmreis_schools_common_model->get_all_district();
				...
				for($j=2;$j<count($arr_data)+2;$j++)
				{
					...
					$is_exists = $this->ci->tmreis_schools_common_model->check_if_doc_exists($unique_id);
					if(!empty($is_exists)){
					    ...
					    $monthly_bmi = array(
						 ...
						 $existing_update = $this->ci->tmreis_schools_common_model->tmreis_update_bmi_values($ndate,$monthly_bmi,$unique_id);
			    	...
			    	array_push($bmi_final,$monthly_bmi);
						...
						$history['last_stage']['current_stage'] = "stage1";
						$history['last_stage']['approval'] = "true";
						$history['last_stage']['submitted_by'] = "medusermw1#gmail.com";
						$history['last_stage']['time'] = date("Y-m-d H:i:s");

						$this->ci->tmreis_schools_common_model->create_bmi_values_monthly($doc_data,$doc_properties,$history);
		...
		if ($this->ci->upload->do_upload("file"))
		{
			...
			if (count($check)==0) {
				...
				for($j=2;$j<count($arr_data)+2;$j++)
				{
					...
					$is_exists = $this->ci->tmreis_schools_common_model->check_if_doc_exists_hb($unique_id);
					if(!empty($is_exists)){
						$doc_data['widget_data']['page1']['Student Details']['Hospital Unique ID'] = $arr_data[$j]['hospital unique id'];
						$doc_data['widget_data']['page1']['Student Details']['Name']['field_ref'] = $arr_data[$j]['student name'];
						$doc_data['widget_data']['page1']['Student Details']['Class']['field_ref'] = $arr_data[$j]['class'];
					    ...
					    $monthly_hb = array(
						 ...
						 $existing_update = $this->ci->tmreis_schools_common_model->tmreis_update_hb_values($ndate,$monthly_hb,$unique_id);
			    	...
			    	array_push($hb_final,$monthly_hb);
						...
						$this->ci->tmreis_schools_common_model->create_hb_values_monthly($doc_data,$doc_properties,$history);
	...
	{
		$docs = $this->ci->tmreis_common_model->get_reports_ehr_uid_new_html_static_hs($unique_id,$school_name);
	...
	public function hb_pie_view_lib($current_month, $school_name){
		...
		$count = 0;
		$hb_report = $this->ci->tmreis_schools_common_model->get_hb_report_model($current_month, $school_name);

...
class Panacea_common_lib 
{
	...
	*/
	...
	public function __construct()
	{
		...
		$this->ci->load->model('tswreis_sports_common_model');
	...
	{
		$total_rows = $this->ci->panacea_common_model->dmhonumberscount();
		...
		//find all the categories with paginate and save it in array to past to the view
		$this->data['hospitals'] = $this->ci->panacea_common_model->get_dmho_numbers($config['per_page'], $page);
		//create paginate´s links
		$this->data['links'] = $this->ci->pagination->create_links();
		...
		$this->data['dmhonumberscount'] = $total_rows;
		$this->data['distslist'] = $this->ci->panacea_common_model->get_all_district();
	...
	{
		$total_rows = $this->ci->panacea_common_model->bloodbankscount();
		...
		//find all the categories with paginate and save it in array to past to the view
		$this->data['hospitals'] = $this->ci->panacea_common_model->get_blood_banks($config['per_page'], $page);
		//create paginate´s links
		$this->data['links'] = $this->ci->pagination->create_links();
		...
		$this->data['bloodbankscount'] = $total_rows;
		$this->data['distslist'] = $this->ci->panacea_common_model->get_all_district();
	...
	function import_blood_donors($post)
	{
		$uploaddir = EXCEL;
		...
		if($this->ci->upload->do_upload("file"))
		{
			...
			$header_array = array("donor id","donor name","primary contact","secondary contact","blood group","email id","address","district","age","donor type");
			...
			if(count($check) == 0)
			{
				...
				for($j=2; $j<=count($arr_data)+2; $j++){
					...
					if(!empty($arr_data[$j]['donor id']) && $arr_data[$j]['donor id'] != null && $arr_data[$j]['donor id'] != ""){
						$doc_data['donor id'] = $arr_data[$j]['donor id'];
						//$doc_data['ad no'] = $arr_data[$j]['ad no'];
						$doc_data['donor name'] = $arr_data[$j]['donor name'];
						$doc_data['primary contact'] = $arr_data[$j]['primary contact'];
						$doc_data['secondary contact'] = $arr_data[$j]['secondary contact'];
						$doc_data['blood group'] = $arr_data[$j]['blood group'];
						$doc_data['district'] = $arr_data[$j]['district'];
						$doc_data['email id'] = $arr_data[$j]['email id'];
						$doc_data['age'] = $arr_data[$j]['age'];
						$doc_data['address'] = $arr_data[$j]['address'];
						$doc_data['donor type'] = $arr_data[$j]['donor type'];
						...
						$this->ci->panacea_common_model->import_blood_donors_counts($doc_data, $history);
	...
	{	
		...
		$this->data['blood_banks'] = $this->ci->panacea_common_model->get_blood_banks_details();
	...
	*/
	...
	function import_tswreis_sports_students($post)
	{
		$uploaddir = EXCEL;
		$config['upload_path'] = $uploaddir;
		$config['allowed_types'] = 'xlsx|xls';
		...
		if($this->ci->upload->do_upload("file"))
		{
			...
			$header_array = array("ad no","unique id",	"student name", "class", "school name",	"district", "gender",	"event", "level of participation", "year of participation", "medal");
			...
			if(count($check) == 0)
			{
				...
				for($j=2; $j<=count($arr_data)+2; $j++){
					...
					if(!empty($arr_data[$j]['unique id']) && $arr_data[$j]['unique id'] != null && $arr_data[$j]['unique id'] != ""){
						$doc_data['unique id'] = $arr_data[$j]['unique id'];
						$doc_data['ad no'] = $arr_data[$j]['ad no'];
						$doc_data['student name'] = $arr_data[$j]['student name'];
						$doc_data['school name'] = $arr_data[$j]['school name'];
						$doc_data['class'] = $arr_data[$j]['class'];
						$doc_data['gender'] = $arr_data[$j]['gender'];
						$doc_data['district'] = $arr_data[$j]['district'];
						$doc_data['event'] = $arr_data[$j]['event'];
						$doc_data['medal'] = $arr_data[$j]['medal'];
						$doc_data['level of participation'] = $arr_data[$j]['level of participation'];
						$doc_data['year of participation'] = $arr_data[$j]['year of participation'];

						
						$hey = array($arr_data[$j]['year of participation'] => array($arr_data[$j]['level of participation']=>array("Sport"=>$arr_data[$j]['event'], "medal"=>$arr_data[$j]['medal'])));
						
						$doc_data['Records'] = $hey;
						
						$history['last_stage']['doc_id'] = get_unique_id();
						$history['last_stage']['submitted_by'] = "tswreissports.user#gmail.com";
						$history['last_stage']['time'] = date("Y-m-d H:i:s");

						$this->ci->tswreis_sports_common_model->import_tswreis_sports_students($doc_data, $history);
				...
				if (count($check)==0) {
					...
					for($each_row=2 ; $each_row<=$total_rows ; $each_row++){
						...
						//$sheet->setCellValue('C1', 'Status Failed')->getStyle('C1');
							...
							$sheet->getStyle('A1:K1')->getFont()->setBold(false)->setSize(12);
					...
					$last_unique_id = $this->ci->panacea_common_model->check_last_unique_id_exists_in_school($school_name);
	...
	public function panacea_passedouts_students()
	{
		...
		$total_rows = $this->ci->panacea_common_model->passedouts_studentscount();
	...
	public function generate_excel_for_absent_pie($date, $dt_name = "All", $school_name = "All")
	{
		...
		// Set properties
		$objPHPExcel->getProperties()->setCreator("Havik Healthcare IT Solutions Pvt. Ltd.");
		$objPHPExcel->getProperties()->setLastModifiedBy("Yoga Nrasimha Reddy");
	...
	public function generate_excel_for_request_pie($date,$request_pie_span, $dt_name = "All", $school_name = "All")
	{
		...
		// Set properties
		$objPHPExcel->getProperties()->setCreator("HAVIK HEALTHCARE Pvt. Ltd.");
	...
	public function add_news_feed() {
		...
		if ((file_exists ( $_FILES ['file'] ['tmp_name'] [0] ) || is_uploaded_file ( $_FILES ['file'] ['tmp_name'] [0] ))) {
			...
			$token = $this->ci->panacea_common_model->add_news_feed ( $news_data );
			if ($token) {
	...
	public function get_initaite_requests_count_today_date($today_date)
		{
			...
			$hospitalised_count = $this->ci->panacea_common_model->get_hospitalised_count_today_date();
			...
			$this->data['hospitalised_count'] = $hospitalised_count['hospitalised'];
			$this->data['review_count'] = $hospitalised_count['review'];
			$this->data['out_patient_count'] = $hospitalised_count['out_patient'];
	...
	public function get_symptoms_monthly_tracking_excel($selectedSchool, $selectedDistrict,$start_date,$end_date)
			{
				$this->ci->load->library('excel');
				...
				$objPHPExcel->getProperties()->setTitle($start_date ."to". $end_date.$title."Monthly Symptoms List");
				$objPHPExcel->getProperties()->setSubject($start_date ."to". $end_date.$title."Monthly Symptoms List");
	    ...
	    $objWorkSheet->getColumnDimension("B")->setAutoSize(true);
			...
			$data = $this->ci->panacea_common_model->get_data($start_date,$end_date, $selectedDistrict, $selectedSchool);
			...
			$file_save = BASEDIR.TENANT.'/'.$start_date ."to". $end_date."-Mothly-Symptoms-List.xlsx";
			$file_name = URLCustomer.$start_date ."to". $end_date."-Mothly-Symptoms-List.xlsx";
			$objWriter->save($file_save);
			//$this->secure_file_download($file_name);
			//unlink($file_name);
			return $file_name;
	...
	public function get_excel_for_students_nos($district, $scl)
{
	$this->ci->load->library('excel');
	...
	$objPHPExcel->getProperties()->setTitle($title."Screening Information");
	$objPHPExcel->getProperties()->setSubject($title."Screening Information");
	$objPHPExcel->getProperties()->setDescription("Screening Report");

	//Add new sheet
	$objWorkSheet = $objPHPExcel->createSheet(0); //select sheet index you want to print
	$objWorkSheet->setTitle("Student Numbers Report");
	$objWorkSheet->getRowDimension(1)->setRowHeight(44);
	...
	$objWorkSheet->setCellValue('E1', 'Father Name')->getStyle('E1')->applyFromArray($styleArray);
	$objWorkSheet->setCellValue('F1', 'School Name')->getStyle('F1')->applyFromArray($styleArray);
	$objWorkSheet->setCellValue('G1', 'District')->getStyle('G1')->applyFromArray($styleArray);
	...
	$screening_data = $this->ci->panacea_common_model->get_excel_for_phone_nos($district, $scl);
	...
	foreach ($screening_data as $screening) {
		...
		$objWorkSheet->setCellValue('E'.$cell_index, $screening ["doc_data"] ['widget_data'] ['page2'] ['Personal Information'] ['Father Name']);
		$objWorkSheet->setCellValue('F'.$cell_index, $screening ["doc_data"] ['widget_data'] ['page2'] ['Personal Information'] ['School Name']);
		$objWorkSheet->setCellValue('G'.$cell_index, $screening ["doc_data"] ['widget_data'] ['page2'] ['Personal Information'] ['District']);
		//$objWorkSheet->setCellValue('E'.$cell_index, $screening ["doc_data"] ['widget_data'] ['page1'] ['Personal Information'] ['Gender']);
		...
		if((!empty($screening ["doc_data"] ['widget_data'] ['page1'] ['Personal Information'] ['Mobile']['mob_num'])) || ($screening ["doc_data"] ['widget_data'] ['page1'] ['Personal Information'] ['Mobile']['mob_num'] == 'Nil'))
				...
				$objWorkSheet->setCellValue('H'.$cell_index, $screening ["doc_data"] ['widget_data'] ['page1'] ['Personal Information'] ['Mobile']['mob_num']);
				}else
				{
	...
	$file_save = BASEDIR.TENANT.'/'.$district."-Screening-Report.xlsx";
	$file_name = URLCustomer.$district."-Screening-Report.xlsx";
	$objWriter->save($file_save);
	//$this->secure_file_download($file_name);
	//unlink($file_name);
	return $file_name;
...
public function get_excel_for_students_reports_lib($district, $scl ,$collection_year)
{
    ...
    $objWorkSheet->setCellValue('M1', 'Photo')->getStyle('M1')->applyFromArray($styleArray);*/

    $objWorkSheet->setCellValue('A1', 's.no')->getStyle('A1')->applyFromArray($styleArray);
    ...
    $objWorkSheet->setCellValue('D1', 'student name')->getStyle('D1')->applyFromArray($styleArray);  
    $objWorkSheet->setCellValue('E1', 'Unique id')->getStyle('E1')->applyFromArray($styleArray);
    $objWorkSheet->setCellValue('F1', 'Class')->getStyle('F1')->applyFromArray($styleArray);
    $objWorkSheet->setCellValue('G1', 'Section')->getStyle('G1')->applyFromArray($styleArray);  
    $objWorkSheet->setCellValue('H1', 'Date of birth')->getStyle('H1')->applyFromArray($styleArray);  
    ...
    $objWorkSheet->setCellValue('L1', 'Submission date')->getStyle('L1')->applyFromArray($styleArray);  
    $objWorkSheet->setCellValue('M1', 'HB')->getStyle('M1')->applyFromArray($styleArray);  
    $objWorkSheet->setCellValue('N1', 'Mild')->getStyle('N1')->applyFromArray($styleArray);  
    $objWorkSheet->setCellValue('O1', 'Moderate')->getStyle('O1')->applyFromArray($styleArray);  
    $objWorkSheet->setCellValue('P1', 'Severe')->getStyle('P1')->applyFromArray($styleArray);  
    $objWorkSheet->setCellValue('Q1', 'Submission date')->getStyle('Q1')->applyFromArray($styleArray);  
    $objWorkSheet->setCellValue('R1', 'Request')->getStyle('R1')->applyFromArray($styleArray);  
    $objWorkSheet->setCellValue('S1', 'Problem')->getStyle('S1')->applyFromArray($styleArray);  
    $objWorkSheet->setCellValue('T1', 'Date of submission')->getStyle('T1')->applyFromArray($styleArray);  
    $objWorkSheet->setCellValue('U1', 'SCREENING PRBLM')->getStyle('U1')->applyFromArray($styleArray);  
    ...
    $reports_data = $this->ci->panacea_common_model->get_excel_for_students_reports_model($district, $scl,$collection_year);
    ...
    foreach ($reports_data as $student_data) {
    	...
    	//$getHBData = $this->ci->panacea_common_model->get_hb_bmi_latest_for_student($student_data ["doc_data"]["widget_data"]["page1"]["Personal Information"]["Hospital Unique ID"], $collection_year);

    	$getRequestData = $this->ci->panacea_common_model->get_request_count_for_student($student_data ["doc_data"]["widget_data"]["page1"]["Personal Information"]["Hospital Unique ID"], $collection_year);

    	$scount += 1;
   
        $objWorkSheet->setCellValue('A'.$cell_index, $scount);
        $objWorkSheet->setCellValue('B'.$cell_index, $student_data ["doc_data"]["widget_data"]["page2"]["Personal Information"]["District"]);
        $objWorkSheet->setCellValue('C'.$cell_index, $student_data ["doc_data"]["widget_data"]["page2"]["Personal Information"]["School Name"]);
        $objWorkSheet->setCellValue('D'.$cell_index, $student_data ["doc_data"]["widget_data"]["page1"]["Personal Information"]["Name"]);
        $objWorkSheet->setCellValue('E'.$cell_index, $student_data ["doc_data"]["widget_data"]["page1"]["Personal Information"]["Hospital Unique ID"]);
        $objWorkSheet->setCellValue('F'.$cell_index, $student_data ["doc_data"]["widget_data"]["page2"]["Personal Information"]["Class"]);
        $objWorkSheet->setCellValue('G'.$cell_index, $student_data ["doc_data"]["widget_data"]["page2"]["Personal Information"]["Section"]);
        $objWorkSheet->setCellValue('H'.$cell_index, $student_data ["doc_data"]["widget_data"]["page1"]["Personal Information"]["Date of Birth"]);
        ...
        $objWorkSheet->setCellValue('L'.$cell_index, isset($student_data ["doc_data"]["widget_data"]["page3"]["Physical Exam"]["BMI%"]) ? $student_data ["doc_data"]["widget_data"]["page3"]["Physical Exam"]["BMI%"] :"Nil");
        
        $objWorkSheet->setCellValue('M'.$cell_index, isset($student_data ["doc_data"]["widget_data"]["page3"]["Physical Exam"]["H B"]) ? $student_data ["doc_data"]["widget_data"]["page3"]["Physical Exam"]["H B"] :"Nil");
        $objWorkSheet->setCellValue('N'.$cell_index, "Nil");
        $objWorkSheet->setCellValue('O'.$cell_index, "Nil");
        $objWorkSheet->setCellValue('P'.$cell_index, "Nil");
        $objWorkSheet->setCellValue('Q'.$cell_index, "Nil");

        /*if(isset($getHBData['bmi'][0]) && !empty($getHBData['bmi'][0])){
        	$objWorkSheet->setCellValue('I'.$cell_index, $getHBData['bmi'][0]["doc_data"]["widget_data"]["page1"]["Student Details"]["BMI_latest"]["height"]);
        	$objWorkSheet->setCellValue('J'.$cell_index, $getHBData['bmi'][0]["doc_data"]["widget_data"]["page1"]["Student Details"]["BMI_latest"]["weight"]);
        	$objWorkSheet->setCellValue('K'.$cell_index, $getHBData['bmi'][0]["doc_data"]["widget_data"]["page1"]["Student Details"]["BMI_latest"]["bmi"]);
        	$objWorkSheet->setCellValue('L'.$cell_index, $getHBData['bmi'][0]["doc_data"]["widget_data"]["page1"]["Student Details"]["BMI_latest"]["month"]);
        }else{
        	$objWorkSheet->setCellValue('I'.$cell_index, "Nil");
        	$objWorkSheet->setCellValue('J'.$cell_index, "Nil");
        	$objWorkSheet->setCellValue('K'.$cell_index, "Nil");
        ...
        /*if(isset($getHBData['hb'][0]) && !empty($getHBData['hb'][0])){
        	$objWorkSheet->setCellValue('M'.$cell_index,isset($getHBData['hb'][0]["doc_data"]["widget_data"]["page1"]["Student Details"]["HB_latest"]["hb"]) ? ($getHBData['hb'][0]["doc_data"]["widget_data"]["page1"]["Student Details"]["HB_latest"]["hb"]) : "Nil" );
        	if($getHBData['hb'][0]["doc_data"]["widget_data"]["page1"]["Student Details"]["HB_latest"]["hb"] >=10.1 && $getHBData['hb'][0]["doc_data"]["widget_data"]["page1"]["Student Details"]["HB_latest"]["hb"] <= 12){

        		$objWorkSheet->setCellValue('N'.$cell_index, "Yes");
        		$objWorkSheet->setCellValue('O'.$cell_index, "Nil");
        		$objWorkSheet->setCellValue('P'.$cell_index, "Nil");
        	}elseif($getHBData['hb'][0]["doc_data"]["widget_data"]["page1"]["Student Details"]["HB_latest"]["hb"] >=8.1 && $getHBData['hb'][0]["doc_data"]["widget_data"]["page1"]["Student Details"]["HB_latest"]["hb"] <= 10){

        		$objWorkSheet->setCellValue('N'.$cell_index, "Nil");
        		$objWorkSheet->setCellValue('O'.$cell_index, "Yes");
        		$objWorkSheet->setCellValue('P'.$cell_index, "Nil");
        	}elseif($getHBData['hb'][0]["doc_data"]["widget_data"]["page1"]["Student Details"]["HB_latest"]["hb"] <=8){

        		$objWorkSheet->setCellValue('N'.$cell_index, "Nil");
        		$objWorkSheet->setCellValue('O'.$cell_index, "Nil");
        		$objWorkSheet->setCellValue('P'.$cell_index, "Yes");
        	}else{
        		$objWorkSheet->setCellValue('N'.$cell_index, "Nil");
        		$objWorkSheet->setCellValue('O'.$cell_index, "Nil");
        		$objWorkSheet->setCellValue('P'.$cell_index, "Nil");
        	...
        	$objWorkSheet->setCellValue('Q'.$cell_index, $getHBData['hb'][0]["doc_data"]["widget_data"]["page1"]["Student Details"]["HB_latest"]["month"]);

        }else{
        	$objWorkSheet->setCellValue('M'.$cell_index, "Nil");
        	$objWorkSheet->setCellValue('N'.$cell_index, "Nil");
        	$objWorkSheet->setCellValue('O'.$cell_index, "Nil");
        	$objWorkSheet->setCellValue('P'.$cell_index, "Nil");
        	$objWorkSheet->setCellValue('Q'.$cell_index, "Nil");
        ...
        if(!empty($getRequestData)){
        	...
        	$objWorkSheet->setCellValue('R'.$cell_index, count($getRequestData));
        	if(is_array($getRequestData)){
        		foreach ($getRequestData as $requests) {
        			array_push($emptyReq, $requests["doc_data"]["widget_data"]["page2"]["Problem Info"]["Description"]);
        		}
        	}

        	if(!empty($emptyReq)){
        		$objWorkSheet->setCellValue('S'.$cell_index, implode(",", $emptyReq));
        	}else{
        		$objWorkSheet->setCellValue('S'.$cell_index, "Nil");
        	}
        }else{
        	$objWorkSheet->setCellValue('R'.$cell_index, 0);
        	$objWorkSheet->setCellValue('S'.$cell_index, "Nil");
        }
        ...
        if(isset($student_data["doc_data"]["widget_data"]["page4"]["Doctor Check Up"]["Ortho"]) && !empty($student_data["doc_data"]["widget_data"]["page4"]["Doctor Check Up"]["Ortho"]) && is_array($student_data["doc_data"]["widget_data"]["page4"]["Doctor Check Up"]["Ortho"])){
        	foreach ($student_data["doc_data"]["widget_data"]["page4"]["Doctor Check Up"]["Ortho"] as $ortho) {
        		array_push($docCheckValues, $ortho);
        	}

        }

        if(isset($student_data["doc_data"]["widget_data"]["page4"]["Doctor Check Up"]["Postural"]) && !empty($student_data["doc_data"]["widget_data"]["page4"]["Doctor Check Up"]["Postural"]) && is_array($student_data["doc_data"]["widget_data"]["page4"]["Doctor Check Up"]["Postural"])){
        	foreach ($student_data["doc_data"]["widget_data"]["page4"]["Doctor Check Up"]["Postural"] as $postural) {
        		array_push($docCheckValues, $postural);
        	}

        }

        if(isset($student_data["doc_data"]["widget_data"]["page4"]["Doctor Check Up"]["Skin conditions"]) && !empty($student_data["doc_data"]["widget_data"]["page4"]["Doctor Check Up"]["Skin conditions"]) && is_array($student_data["doc_data"]["widget_data"]["page4"]["Doctor Check Up"]["Skin conditions"])){
        	foreach ($student_data["doc_data"]["widget_data"]["page4"]["Doctor Check Up"]["Skin conditions"] as $Skin_conditions) {
        		array_push($docCheckValues, $Skin_conditions);
        	}

        }

        if(isset($student_data["doc_data"]["widget_data"]["page5"]["Doctor Check Up"]["Defects at Birth"]) && !empty($student_data["doc_data"]["widget_data"]["page5"]["Doctor Check Up"]["Defects at Birth"]) && is_array($student_data["doc_data"]["widget_data"]["page5"]["Doctor Check Up"]["Defects at Birth"])){
        	foreach ($student_data["doc_data"]["widget_data"]["page5"]["Doctor Check Up"]["Defects at Birth"] as $Defects) {
        		array_push($docCheckValues, $Defects);
        	}

        }

        if(isset($student_data["doc_data"]["widget_data"]["page5"]["Doctor Check Up"]["Deficencies"]) && !empty($student_data["doc_data"]["widget_data"]["page5"]["Doctor Check Up"]["Deficencies"]) && is_array($student_data["doc_data"]["widget_data"]["page5"]["Doctor Check Up"]["Deficencies"])){
        	foreach ($student_data["doc_data"]["widget_data"]["page5"]["Doctor Check Up"]["Deficencies"] as $Deficencie) {
        		array_push($docCheckValues, $Deficencie);
        	}

        }

        if(isset($student_data["doc_data"]["widget_data"]["page5"]["Doctor Check Up"]["Childhood Diseases"]) && !empty($student_data["doc_data"]["widget_data"]["page5"]["Doctor Check Up"]["Childhood Diseases"]) && is_array($student_data["doc_data"]["widget_data"]["page5"]["Doctor Check Up"]["Childhood Diseases"])){
        	foreach ($student_data["doc_data"]["widget_data"]["page5"]["Doctor Check Up"]["Childhood Diseases"] as $Childhood_Diseases) {
        		array_push($docCheckValues, $Childhood_Diseases);
        	}

        }

        if(!empty($docCheckValues) && is_array($docCheckValues)){
        	$objWorkSheet->setCellValue('T'.$cell_index, implode(",", $docCheckValues));
        	$objWorkSheet->setCellValue('U'.$cell_index, $student_data["history"]["last_stage"]["time"]);
       ...
       if(isset($student_data["doc_data"]["widget_data"]["page1"]["Personal Information"]['Photo']['file_path']) && !empty($student_data["doc_data"]["widget_data"]["page1"]["Personal Information"]['Photo']['file_path'])){
			$objWorkSheet->setCellValue('M'.$cell_index, $student_data["doc_data"]["widget_data"]["page1"]["Personal Information"]['Photo']['file_path']);
		}else{
			$objWorkSheet->setCellValue('M'.$cell_index, 'No Photo');
		}*/
	...
	public function get_excel_for_registered_parents($district, $scl, $start, $end)
{
	$this->ci->load->library('excel');
	...
	$objPHPExcel->getProperties()->setCreator("Havik Healthcare Technologies Pvt. Ltd");
	$objPHPExcel->getProperties()->setLastModifiedBy("Harish Seelam");
	$objPHPExcel->getProperties()->setTitle($title."Parents Registration");
	$objPHPExcel->getProperties()->setSubject($title."Parents Registration");
	$objPHPExcel->getProperties()->setDescription("Parents Registration Report");

	//Add new sheet
	$objWorkSheet = $objPHPExcel->createSheet(0); //select sheet index you want to print
	$objWorkSheet->setTitle("Parents Registration Report");
	$objWorkSheet->getRowDimension(1)->setRowHeight(44);
	...
	$objWorkSheet->setCellValue('A1', 'Student Health ID')->getStyle('A1')->applyFromArray($styleArray);
	$objWorkSheet->setCellValue('B1', 'Student Name')->getStyle('B1')->applyFromArray($styleArray);
	$objWorkSheet->setCellValue('C1', 'Mobile No')->getStyle('C1')->applyFromArray($styleArray);
	$objWorkSheet->setCellValue('D1', 'District')->getStyle('D1')->applyFromArray($styleArray);
	$objWorkSheet->setCellValue('E1', 'School Name')->getStyle('E1')->applyFromArray($styleArray);
	$objWorkSheet->setCellValue('F1', 'Date')->getStyle('F1')->applyFromArray($styleArray);
	$objWorkSheet->setCellValue('G1', 'Otp')->getStyle('G1')->applyFromArray($styleArray);
	//$objWorkSheet->setCellValue('H1', 'Phone No')->getStyle('H1')->applyFromArray($styleArray);
	...
	$screening_data = $this->ci->panacea_common_model->get_excel_for_registered_parents_data($district, $scl, $start, $end);

	$cell_index = 2;

	foreach ($screening_data as $screening) {
		
		$objWorkSheet->setCellValue('A'.$cell_index, $screening ["doc_data"]['Hospital Unique ID']);
		$objWorkSheet->setCellValue('B'.$cell_index, $screening ["doc_data"]['Student Name']);
		$objWorkSheet->setCellValue('C'.$cell_index, $screening ["doc_data"] ['Mobile No']);
		$objWorkSheet->setCellValue('D'.$cell_index, $screening ["doc_data"] ['District']);
		$objWorkSheet->setCellValue('E'.$cell_index, $screening ["doc_data"] ['School Name']);
		$objWorkSheet->setCellValue('F'.$cell_index, $screening ["doc_data"] ['Date']);
		$objWorkSheet->setCellValue('G'.$cell_index, $screening ["doc_data"] ['Otp']);
	...
	$file_save = BASEDIR.TENANT.'/'.$district."-Parent-Registration-Report.xlsx";
	$file_name = URLCustomer.$district."-Parent-Registration-Report.xlsx";
	$objWriter->save($file_save);
	//$this->secure_file_download($file_name);
	//unlink($file_name);
	return $file_name;
	...
	public function get_excel_for_dr_responded_reports_lib($dr_login, $start, $end)
{
	$this->ci->load->library('excel');
	...
	$objPHPExcel->getProperties()->setCreator("Havik Healthcare Technologies Pvt. Ltd");
	$objPHPExcel->getProperties()->setLastModifiedBy("Yoga");
	$objPHPExcel->getProperties()->setTitle("Doctor Responded Reports");
	$objPHPExcel->getProperties()->setSubject("Doctor Responded Reports");
	$objPHPExcel->getProperties()->setDescription("Doctor Responded Reports");

	//Add new sheet
	$objWorkSheet = $objPHPExcel->createSheet(0); //select sheet index you want to print
	$objWorkSheet->setTitle("Doctor Responded Reports Report");
	$objWorkSheet->getRowDimension(1)->setRowHeight(44);
	...
	$objWorkSheet->setCellValue('F1', 'Dr.Response Time')->getStyle('F1')->applyFromArray($styleArray);
	...
	$screening_data = $this->ci->panacea_common_model->get_excel_for_dr_responded_reports_model($dr_login, $start, $end);
	...
	foreach ($screening_data as $screening) {
		...
		$objWorkSheet->setCellValue('D'.$cell_index, $screening ["doc_data"]['widget_data']['page1']['Student Info']['School Name']['field_ref']);
		$objWorkSheet->setCellValue('E'.$cell_index, $screening ["history"]['0']['time']);
		
		$last_doc = end($screening['history']); 
		if(preg_match("/panacea.dr/i",$last_doc['submitted_by']));
		  $objWorkSheet->setCellValue('F'.$cell_index, $last_doc["time"]);	
		$cell_index ++;
	}
	...
	$file_save = BASEDIR.TENANT.'/'."Doctor-Responded-Reports.xlsx";
	$file_name = URLCustomer."Doctor-Responded-Reports.xlsx";
	$objWriter->save($file_save);
	//$this->secure_file_download($file_name);
	//unlink($file_name);
	return $file_name;
...
public function get_excel_for_request_notes($start, $end)
{
	...
	$screening_data = $this->ci->panacea_common_model->get_excel_for_request_notes_data($start, $end);
	...
	foreach ($screening_data as $screening) {
		...
		if(isset($screening["Name"]) && !empty($screening["Name"])){
			$objWorkSheet->setCellValue('B'.$cell_index, $screening["Name"]);
		}else{
			$objWorkSheet->setCellValue('B'.$cell_index, 'Not Available');
		}

		if(isset($screening["Class"]) && !empty($screening["Class"])){
			$objWorkSheet->setCellValue('C'.$cell_index, $screening["Class"]);
		}else{
			$objWorkSheet->setCellValue('C'.$cell_index, 'Not Available');
		}

		if(isset($screening["School_Name"]) && !empty($screening["School_Name"])){
			$objWorkSheet->setCellValue('D'.$cell_index, $screening["School_Name"]);
		}else{
			$objWorkSheet->setCellValue('D'.$cell_index, 'Not Available');
		}

		$objWorkSheet->setCellValue('E'.$cell_index, $screening ["datetime"]);
		$objWorkSheet->setCellValue('F'.$cell_index, $screening ["note"]);

		if(isset($screening["username"]) && !empty($screening["username"])){
			$objWorkSheet->setCellValue('G'.$cell_index, $screening["username"]);
		}else{
			$objWorkSheet->setCellValue('G'.$cell_index, 'Not Available');
		}
...
// Quotation notes for excel download

public function get_excel_for_quotation_notes_lib($start, $end)
{
	$this->ci->load->library('excel');
	...
	$objPHPExcel->getProperties()->setCreator("Havik Healthcare Technologies Pvt. Ltd");
	$objPHPExcel->getProperties()->setLastModifiedBy("Yoga");
	$objPHPExcel->getProperties()->setTitle("Quotation Notes");
	$objPHPExcel->getProperties()->setSubject("Quotation Notes");
	$objPHPExcel->getProperties()->setDescription("Quotation Notes Report");
	...
	$objWorkSheet->setCellValue('A1', 'Created Date')->getStyle('A1')->applyFromArray($styleArray);
	$objWorkSheet->setCellValue('B1', 'Health Quotations')->getStyle('B1')->applyFromArray($styleArray);
	...
	$health_quotation = $this->ci->panacea_common_model->get_excel_for_quotation_notes_model($start, $end);

	/*echo print_r($health_quotation,TRUE);
	exit();*/

	$cell_index = 2;

	foreach ($health_quotation as $quotation) {
		
		$objWorkSheet->setCellValue('A'.$cell_index, $quotation ["Created_on"]);
		$objWorkSheet->setCellValue('B'.$cell_index, $quotation ["News_Feed"]);
	...
	$file_save = BASEDIR.TENANT.'/'."Health-Quotation-Notes-Report.xlsx";
	$file_name = URLCustomer."Health-Quotation-Notes-Report.xlsx";
	$objWriter->save($file_save);
...
//Anemia Requests excel Download
public function get_excel_for_anemia_request_lib($start, $end)
{
    $this->ci->load->library('excel');
    ...
    $objPHPExcel->getProperties()->setCreator("Havik Healthcare Technologies Pvt. Ltd");
    $objPHPExcel->getProperties()->setLastModifiedBy("Yoga");
    $objPHPExcel->getProperties()->setTitle("Anemia Request Notes");
    $objPHPExcel->getProperties()->setSubject("Anemia Request Notes");
    $objPHPExcel->getProperties()->setDescription("Anemia Request Notes Report");

    //Add new sheet
    $objWorkSheet = $objPHPExcel->createSheet(0); //select sheet index you want to print
    $objWorkSheet->setTitle("Anemia Request Notes Report");
    $objWorkSheet->getRowDimension(1)->setRowHeight(44);
    ...
    $objWorkSheet->setCellValue('E1', 'Request Raised Time')->getStyle('E1')->applyFromArray($styleArray);
    $objWorkSheet->setCellValue('F1', 'Dr.Response Time')->getStyle('F1')->applyFromArray($styleArray);
    $objWorkSheet->setCellValue('G1', 'Doctor Name')->getStyle('G1')->applyFromArray($styleArray);
    ...
    $screening_data = $this->ci->panacea_common_model->get_excel_for_anemia_request_model($start, $end);
    ...
    foreach ($screening_data as $screening) {
		...
		if(preg_match("/panacea.dr/i",$last_doc['submitted_by']));
		  ...
		  if(preg_match("/panacea.dr/i",$last_doc['submitted_by'])){
		  $objWorkSheet->setCellValue('G'.$cell_index, $last_doc["submitted_by_name"]);
		}else{
            $objWorkSheet->setCellValue('G'.$cell_index, 'Doctor not to yet responded');
        }
    ...
    $file_save = BASEDIR.TENANT.'/'."Anemia-Request-Report.xlsx";
    $file_name = URLCustomer."Anemia-Request-Report.xlsx";
    $objWriter->save($file_save);
	...
	public function get_excel_for_requests_span($district, $scl, $start, $end)
{
	...
	$screening_data = $this->ci->panacea_common_model->get_excel_for_requests_span($district, $scl, $start, $end);
	...
	public function get_excel_for_hb_span($district, $scl, $start, $end)
{
	...
	$screening_data = $this->ci->panacea_common_model->get_excel_for_hb_span($district, $scl, $start, $end);
...
public function get_excel_for_attendance_span($dist, $scl, $start, $end)
{
	...
	$screening_data = $this->ci->panacea_common_model->get_excel_for_attendance_span($dist, $scl, $start, $end);
...
public function get_excel_for_sanitation_span($dist, $scl, $start, $end)
{
	...
	$screening_data = $this->ci->panacea_common_model->get_excel_for_sanitation_span($dist, $scl, $start, $end);
...
public function get_excel_for_screening_overall($dist, $scl, $academic)
{
	...
	$screening_data = $this->ci->panacea_common_model->get_excel_for_screening_overall($dist, $scl, $academic);
...
public function get_excel_for_doctor_not_responded_reports_lib($start_date, $end_date)
{
    ...
    $reports_data = $this->ci->panacea_common_model->get_excel_doctor_not_responded_req_notes_model($start_date, $end_date);
...
public function get_excel_for_covidcases_lib($start_date, $end_date)
    ...
    $reports_data = $this->ci->panacea_common_model->get_excel_covidcases_model($start_date, $end_date);
...
public function get_excel_for_covid_cured_cases_lib($start_date, $end_date)
    ...
    $reports_data = $this->ci->panacea_common_model->get_excel_covid_cured_cases_model($start_date, $end_date);
...
public function get_excel_for_deathcases_lib($start_date, $end_date)
{
    ...
    $reports_data = $this->ci->panacea_common_model->get_excel_deathcases_model($start_date, $end_date);
...
function import_aarogyasri_hospitals($post)
	{
		$uploaddir = EXCEL;
		$config['upload_path'] = $uploaddir;
		$config['allowed_types'] = 'xlsx|xls';
		...
		if($this->ci->upload->do_upload("file"))
		{
			...
			$header_array = array("hospital id","hospital name","specialities","md name","hospital contact number","arrogya mitra","district","district co-ordinator","district manager","team leader","hospital type");
			...
			if(count($check) == 0)
			{
				...
				for($j=2; $j<=count($arr_data)+2; $j++){				
					...
					if(!empty($arr_data[$j]['hospital id']) && $arr_data[$j]['hospital id'] != null && $arr_data[$j]['hospital id'] != ""){
						$doc_data['hospital id'] = $arr_data[$j]['hospital id'];						
						$doc_data['hospital name'] = $arr_data[$j]['hospital name'];
						$doc_data['specialities'] = $arr_data[$j]['specialities'];
						$doc_data['md name'] = $arr_data[$j]['md name'];
						$doc_data['contact number'] = $arr_data[$j]['hospital contact number'];
						$doc_data['district'] = $arr_data[$j]['district'];						
						$doc_data['arrogya mitra'] = $arr_data[$j]['arrogya mitra'];
						$doc_data['district co-ordinator'] = $arr_data[$j]['district co-ordinator'];
						$doc_data['district manager'] = $arr_data[$j]['district manager'];
						$doc_data['team leader'] = $arr_data[$j]['team leader'];
						$doc_data['hospital type'] = $arr_data[$j]['hospital type'];																	
						
						$history['last_stage']['doc_id'] = get_unique_id();
						$history['last_stage']['submitted_by'] = "panacea.user#gmail.com";
						$history['last_stage']['time'] = date("Y-m-d H:i:s");

						$this->ci->panacea_common_model->import_aarogyasri_hospitals_counts($doc_data, $history);
...
function import_organ_transplant_hospitals($post)
	{
		$uploaddir = EXCEL;
		$config['upload_path'] = $uploaddir;
		$config['allowed_types'] = 'xlsx|xls';
		...
		if($this->ci->upload->do_upload("file"))
		{
			...
			$header_array = array("hospital id","hospital name","hospital contact number","district","organ name","address");
			...
			if(count($check) == 0)
			{
				...
				for($j=2; $j<=count($arr_data)+2; $j++){
					...
					if(!empty($arr_data[$j]['hospital id']) && $arr_data[$j]['hospital id'] != null && $arr_data[$j]['hospital id'] != ""){
						...
						$doc_data['hospital name'] = $arr_data[$j]['hospital name'];
						$doc_data['contact number'] = $arr_data[$j]['hospital contact number'];
						$doc_data['organ name'] = $arr_data[$j]['organ name'];
						$doc_data['address'] = $arr_data[$j]['address'];
						$doc_data['district'] = $arr_data[$j]['district'];						
						...
						$this->ci->panacea_common_model->import_organ_transplant_hospitals_counts($doc_data, $history);
...
public function import_napkin_distribution_schools($post)

{
		...
		if($this->ci->upload->do_upload("file"))
		{
			...
			$header_array = array("school name","contact details","strength","packs distributed count","date of distribution");
			...
			if(count($check) == 0)
			{
				...
				for($j=2; $j<=count($arr_data)+2; $j++){					
					...
					if(!empty($arr_data[$j]['school name']) && $arr_data[$j]['school name'] != null && $arr_data[$j]['school name'] != ""){
						$doc_data['school name'] = $arr_data[$j]['school name'];						
						$doc_data['contact details'] = $arr_data[$j]['contact details'];
						$doc_data['strength'] = $arr_data[$j]['strength'];
						$doc_data['packs distributed count'] = $arr_data[$j]['packs distributed count'];
						$doc_data['date of distribution'] = $arr_data[$j]['date of distribution'];
						
						$history['last_stage']['doc_id'] = get_unique_id();
						$history['last_stage']['submitted_by'] = "panacea.user#gmail.com";
						$history['last_stage']['time'] = date("Y-m-d H:i:s");

						$this->ci->panacea_common_model->import_napkin_distribution_schools_model($doc_data, $history);
...

{
		...
		$header_array = array("school name","region","rco details","rhso details","district","prinicipal details","hs details","job type","qualification","care taker details","deo details","strength","ro plants availability","ro active","ro inactive","vendor details","incinerators availability","active","inactive");
			...
			if(count($check) == 0)
			{
				...
				for($j=2; $j<=count($arr_data)+2; $j++){					
					...
					if(!empty($arr_data[$j]['school name']) && $arr_data[$j]['school name'] != null && $arr_data[$j]['school name'] != ""){
						...
						$doc_data['rhso_details'] = $arr_data[$j]['rhso details'];
						$doc_data['district'] = $arr_data[$j]['district'];
						...
						$doc_data['care_taker_details'] = $arr_data[$j]['care taker details'];
						...
						$this->ci->panacea_common_model->import_detailed_school_information_model($doc_data, $history);
	...
	public function import_l3_services_data($post)

{
		...
		$header_array = array("date","time","district","place","patient name","age","contact number","patient illeness","doctor advice","call receiver");
			...
			if(count($check) == 0)
			{
				...
				for($j=2; $j<=count($arr_data)+2; $j++){	
					...
					if(!empty($arr_data[$j]['district']) && $arr_data[$j]['district'] != null && $arr_data[$j]['district'] != ""){
						$doc_data['district'] = $arr_data[$j]['district'];						
						$doc_data['date'] = $arr_data[$j]['date'];
						$doc_data['time'] = $arr_data[$j]['time'];
						$doc_data['place'] = $arr_data[$j]['place'];
						$doc_data['patient_name'] = $arr_data[$j]['patient name'];
						$doc_data['age'] = $arr_data[$j]['age'];
						$doc_data['contact_number'] = $arr_data[$j]['contact number'];
						$doc_data['patient_illeness'] = $arr_data[$j]['patient illeness'];						
						$doc_data['doctor_advice'] = $arr_data[$j]['doctor advice'];
						$doc_data['call_receiver'] = $arr_data[$j]['call receiver'];											
						
						$history['last_stage']['doc_id'] = get_unique_id();
						$history['last_stage']['submitted_by'] = "panacea.user#gmail.com";
						$history['last_stage']['time'] = date("Y-m-d H:i:s");

						$this->ci->panacea_common_model->import_l3_services_data_model($doc_data, $history);
			...
			if(count($check) == 0)
			{
				...
				for($j=2; $j<=count($arr_data)+2; $j++){	
					...
					if(!empty($arr_data[$j]['district']) && $arr_data[$j]['district'] != null && $arr_data[$j]['district'] != ""){
						...
						$this->ci->panacea_common_model->import_staff_covid_cases_model($doc_data, $history);
...
public function import_vaccination_details($post)

{
	...
	$header_array = array("region","employee name","designation","school name","district","job type","vaccine type","1st dose","2nd dose","reason for exemption","remarks");
			...
			if(count($check) == 0)
			{
				...
				for($j=2; $j<=count($arr_data)+2; $j++){	
					...
					if(!empty($arr_data[$j]['district']) && $arr_data[$j]['district'] != null && $arr_data[$j]['district'] != ""){
						$doc_data['district'] = $arr_data[$j]['district'];						
						$doc_data['region'] = $arr_data[$j]['region'];						
						$doc_data['employee_name'] = $arr_data[$j]['employee name'];
						$doc_data['designation'] = $arr_data[$j]['designation'];
						$doc_data['school_name'] = $arr_data[$j]['school name'];
						$doc_data['job_type'] = $arr_data[$j]['job type'];
						$doc_data['vaccine_type'] = $arr_data[$j]['vaccine type'];						
						$doc_data['first_dose'] = $arr_data[$j]['1st dose'];
						$doc_data['second_dose'] = $arr_data[$j]['2nd dose'];
						$doc_data['remarks'] = $arr_data[$j]['remarks'];											
						$doc_data['reason_for_exemption'] = $arr_data[$j]['reason for exemption'];						
						
						$history['last_stage']['doc_id'] = get_unique_id();
						$history['last_stage']['submitted_by'] = "panacea.user#gmail.com";
						$history['last_stage']['time'] = date("Y-m-d H:i:s");

						$this->ci->panacea_common_model->import_vaccination_details_model($doc_data, $history);

...
class Ttwreis_common_lib 
{
	...
	*/
	...
	public function __construct()
	{
		...
		$this->ci->load->model('ttwreis_common_model');
	...
	{
		$total_rows = $this->ci->ttwreis_common_model->statescount();
		...
		$this->data['states'] = $this->ci->ttwreis_common_model->get_states($config['per_page'], $page);
	...
	{
		$total_rows = $this->ci->ttwreis_common_model->distcount();
		...
		//find all the categories with paginate and save it in array to past to the view
		$this->data['dists'] = $this->ci->ttwreis_common_model->get_district($config['per_page'], $page);
		//create paginate´s links
		$this->data['links'] = $this->ci->pagination->create_links();
		...
		$this->data['statelist'] = $this->ci->ttwreis_common_model->get_all_states();
	...
	{
		$total_rows = $this->ci->ttwreis_common_model->health_supervisorscount();
		...
		//find all the categories with paginate and save it in array to past to the view
		$this->data['health_supervisors'] = $this->ci->ttwreis_common_model->get_health_supervisors($config['per_page'], $page);
		//create paginate´s links
		$this->data['links'] = $this->ci->pagination->create_links();
	...
	public function ttwreis_mgmt_doctors()
	{
		...
		$total_rows = $this->ci->ttwreis_common_model->doctorscount();
		...
		//find all the categories with paginate and save it in array to past to the view
		$this->data['doctors'] = $this->ci->ttwreis_common_model->get_doctors($config['per_page'], $page);
		//create paginate´s links
		$this->data['links'] = $this->ci->pagination->create_links();
	...
	{
		$total_rows = $this->ci->ttwreis_common_model->schoolscount();
		...
		//find all the categories with paginate and save it in array to past to the view
		$this->data['schools'] = $this->ci->ttwreis_common_model->get_schools($config['per_page'], $page);
		//create paginate´s links
		$this->data['links'] = $this->ci->pagination->create_links();
		...
		$this->data['distslist'] = $this->ci->ttwreis_common_model->get_all_district();
	...
	public function ttwreis_mgmt_classes()
	{
		...
		$total_rows = $this->ci->ttwreis_common_model->classescount();
		...
		//find all the categories with paginate and save it in array to past to the view
		$this->data['classes'] = $this->ci->ttwreis_common_model->get_classes($config['per_page'], $page);
		//create paginate´s links
		$this->data['links'] = $this->ci->pagination->create_links();
	...
	public function ttwreis_mgmt_sections()
	{
		...
		$total_rows = $this->ci->ttwreis_common_model->sectionscount();
		...
		//find all the categories with paginate and save it in array to past to the view
		$this->data['sections'] = $this->ci->ttwreis_common_model->get_sections($config['per_page'], $page);
		//create paginate´s links
		$this->data['links'] = $this->ci->pagination->create_links();
	...
	public function ttwreis_mgmt_symptoms()
	{
		...
		$total_rows = $this->ci->ttwreis_common_model->symptomscount();
		...
		//find all the categories with paginate and save it in array to past to the view
		$this->data['symptoms'] = $this->ci->ttwreis_common_model->get_symptoms($config['per_page'], $page);
		//create paginate´s links
		$this->data['links'] = $this->ci->pagination->create_links();
	...
	public function ttwreis_mgmt_diagnostic()
	{
		...
		$total_rows = $this->ci->ttwreis_common_model->diagnosticscount();
		...
		//find all the categories with paginate and save it in array to past to the view
		$this->data['diagnostics'] = $this->ci->ttwreis_common_model->get_diagnostics($config['per_page'], $page);
		//create paginate´s links
		$this->data['links'] = $this->ci->pagination->create_links();
	...
	{
		$total_rows = $this->ci->ttwreis_common_model->hospitalscount();
		...
		//find all the categories with paginate and save it in array to past to the view
		$this->data['hospitals'] = $this->ci->ttwreis_common_model->get_hospitals($config['per_page'], $page);
		//create paginate´s links
		$this->data['links'] = $this->ci->pagination->create_links();
	...
	public function ttwreis_mgmt_emp()
	{
		...
		$total_rows = $this->ci->ttwreis_common_model->empcount();
		...
		//find all the categories with paginate and save it in array to past to the view
		$this->data['emps'] = $this->ci->ttwreis_common_model->get_emp($config['per_page'], $page);
		//create paginate´s links
		$this->data['links'] = $this->ci->pagination->create_links();
	...
	{
		$docs = $this->ci->ttwreis_common_model->get_reports_ehr($post['ad_no']);
	...
	{
		$docs = $this->ci->ttwreis_common_model->get_reports_ehr_uid($post['uid']);
	...
	{
		$token = $this->ci->ttwreis_common_model->insert_request_note($post);
	...
	{	
		$total_rows = $this->ci->ttwreis_common_model->studentscount();
		$this->data['students'] = $this->ci->ttwreis_common_model->get_all_students();
	...
	{	
		...
		$this->data['schools'] = $this->ci->ttwreis_common_model->get_all_schools();
	...
	function to_dashboard($date = FALSE, $request_duration = "Daily", $screening_duration = "Yearly")
	{
		...
		$count = 0;
		$absent_report = $this->ci->ttwreis_common_model->get_all_absent_data($date);
		...
		$this->data['absent_report_schools_list'] = $this->ci->ttwreis_common_model->get_absent_pie_schools_data($date);
		
		$this->data['sanitation_report_schools_list'] = $this->ci->ttwreis_common_model->get_sanitation_report_pie_schools_data($date);
		
		$count = 0;
		$symptoms_report = $this->ci->ttwreis_common_model->get_all_symptoms($date,$request_duration);
		...
		$count = 0;
		$request_report = $this->ci->ttwreis_common_model->get_all_requests($date,$request_duration);
		...
		$count = 0;
		$screening_report = $this->ci->ttwreis_common_model->get_all_screenings($date,$screening_duration);
		...
		$app_template = $this->ci->ttwreis_common_model->get_sanitation_report_app();
		...
		$chronic_ids = $this->ci->ttwreis_common_model->get_all_chronic_unique_ids_model();
		
		$this->data['chronic_ids'] = json_encode($chronic_ids);
		
		$this->data['last_screening_update'] = $this->ci->ttwreis_common_model->get_last_screening_update();
		...
		$this->data['total_active_req'] = $this->ci->ttwreis_common_model->get_all_active_request();
		$this->data['total_raised_req'] = $this->ci->ttwreis_common_model->get_all_raised_request();
		
		$this->data ['news_feeds'] = $this->ci->ttwreis_common_model->get_today_news_feeds($date);
		log_message('error','asdadadadsaddddddddddd'.print_r($this->data, true));
		return $this->data;
	
	}
	
	function to_dashboard_with_date($date = FALSE, $request_duration = "Monthly", $screening_duration = "Yearly", $dt_name = "All", $school_name = "All")
	{
	    $pagenumber       = array();
	    $page_data        = array();
		$sanitation_report_app = array();
		$count = 0;
		$absent_report = $this->ci->ttwreis_common_model->get_all_absent_data($date, $dt_name, $school_name);
		foreach ($absent_report as $value){
			$count = $count + intval($value['value']);
		}
		if($count > 0){
		...
		$count = 0;
		$symptoms_report = $this->ci->ttwreis_common_model->get_all_symptoms($date,$request_duration, $dt_name, $school_name);
		foreach ($symptoms_report as $value){
			$count = $count + intval($value['value']);
		}
		if($count > 0){
		...
		$count = 0;
		$request_report = $this->ci->ttwreis_common_model->get_all_requests($date,$request_duration, $dt_name, $school_name);
		foreach ($request_report as $value){
			$count = $count + intval($value['value']);
		}
		if($count > 0){
	...
	function update_request_pie($date = FALSE,$request_pie_span  = "Daily",$request_pie_status = "All", $student_type = false,$student_age = false)
	{
		...
		$count = 0;
		$symptoms_report = $this->ci->ttwreis_common_model->get_all_symptoms($date,$request_pie_span,"All","All",$request_pie_status, $student_type,$student_age);
		foreach ($symptoms_report as $value){
			$count = $count + intval($value['value']);
		}
		if($count > 0){
		...
		$count = 0;
		$request_report = $this->ci->ttwreis_common_model->get_all_requests($date,$request_pie_span,"All","All",$request_pie_status, $student_type,$student_age);
		foreach ($request_report as $value){
			$count = $count + intval($value['value']);
		}
		if($count > 0){
	...
	function update_request_pie_old_dash($date = FALSE,$request_pie_span  = "Monthly")
	{
		...
		$count = 0;
		$symptoms_report = $this->ci->ttwreis_common_model->get_all_symptoms_old_dash($date,$request_pie_span);
		foreach ($symptoms_report as $value){
			$count = $count + intval($value['value']);
		}
		if($count > 0){
		...
		$count = 0;
		$request_report = $this->ci->ttwreis_common_model->get_all_requests_old_dash($date,$request_pie_span);
		foreach ($request_report as $value){
			$count = $count + intval($value['value']);
		}
		if($count > 0){
	...
	$count = 0;
		$screening_report = $this->ci->ttwreis_common_model->get_all_screenings($date,$screening_pie_span);
		foreach ($screening_report as $value){
			$count = $count + intval($value['value']);
		}
		if($count > 0){
		...
		if ($this->ci->upload->do_upload("file"))
		{
			...
			if (count($check)==0) {
				...
				for($j=2;$j<count($arr_data)+2;$j++){
					...
					$this->ci->ttwreis_common_model->create_diagnostic($data);
		...
		if ($this->ci->upload->do_upload("file"))
		{
			...
			if (count($check)==0) {
				...
				for($j=2;$j<count($arr_data)+2;$j++){
					...
					$this->ci->ttwreis_common_model->create_hospital($data);
		...
		if ($this->ci->upload->do_upload("file"))
		{
			...
			if (count($check)==0) {
				...
				for($j=2;$j<count($arr_data)+2;$j++){
					...
					$insert_success = $this->ci->ttwreis_common_model->create_school($data);
		...
		if ($this->ci->upload->do_upload("file"))
		{
			...
			if (count($check)==0) {
				...
				for($j=2;$j<count($arr_data)+2;$j++){
					...
					$insert_success = $this->ci->ttwreis_common_model->create_health_supervisors($data);
		...
		if ($this->ci->upload->do_upload("file"))
		{
			...
			if($import_type == "personal_info")
			{
				...
				if (count($check)==0) {
						...
						foreach ($cellIterator as $cell) {
							...
							if($check_col_array[$header_row] == "class")
							{
								...
								if($age == "")
								{
									...
									session_start();
									$_SESSION['request_message'] = "Update failed due to following error in excel sheet: Class value it Maybe Empty or Wrong Class for this Child ".$student_name;
							...
							if($check_col_array[$header_row] == "date of birth")
							{
								try {
									...
									session_start();
									$_SESSION['request_message'] = "Update failed due to following error in excel sheet: value in ".$colCoordinate." seems to of different type that of <strong>text</strong>.";
					...
					//$last_unique_id = $this->ci->ttwreis_common_model->check_unique_id_is_exists($school_name);
					$last_unique_id = $this->ci->ttwreis_common_model->check_last_unique_id_exists_in_school($school_name);
					...
					for($j=2;$j<count($arr_data)+2;$j++)
					{
						...
						$history['last_stage']['submitted_by'] = "ttwreis#gmail.com";
						...
						$this->ci->ttwreis_common_model->insert_student_data($doc_data,$history,$doc_properties);
		...
		if ($this->ci->upload->do_upload("file"))
		{
			...
			if ($check) {
				...
				for($j=2;$j<count($arr_data)+2;$j++){
					...
					$doc = $this->ci->ttwreis_common_model->get_students_uid($unique_id);
					...
					if($doc){
						...
						$this->ci->ttwreis_common_model->update_student_data($doc,$doc_id);
						$update_count++;
					}
	...
	public function get_excel_for_students_reports_lib($district, $scl ,$collection_year)
{
    ...
    $objWorkSheet->setCellValue('J1', 'Aadhar Card')->getStyle('J1')->applyFromArray($styleArray); 
    $objWorkSheet->setCellValue('K1', 'Ration Card')->getStyle('K1')->applyFromArray($styleArray); 
    $objWorkSheet->setCellValue('L1', 'Aarogyasri Card')->getStyle('L1')->applyFromArray($styleArray); 
    ...
    $reports_data = $this->ci->ttwreis_common_model->get_excel_for_students_reports_model($district, $scl,$collection_year);
    ...
    foreach ($reports_data as $student_data) {
        ...
        $objWorkSheet->setCellValue('H'.$cell_index, $student_data ["doc_data"]["widget_data"]["page1"]["Personal Information"]["Mobile"]["mob_num"]);
         ...
         /*$objWorkSheet->setCellValue('J'.$cell_index, $student_data ["doc_data"]["widget_data"]["page2"]["Personal Information"]["Aadhar card"]);
         $objWorkSheet->setCellValue('K'.$cell_index, $student_data ["doc_data"]["widget_data"]["page2"]["Personal Information"]["Ration card"]);
         $objWorkSheet->setCellValue('L'.$cell_index, $student_data ["doc_data"]["widget_data"]["page2"]["Personal Information"]["Aarogya sri"]);*/

         if(isset($student_data["doc_data"]["widget_data"]["page2"]["Personal Information"]["Aadhar card"]) && !empty($student_data["doc_data"]["widget_data"]["page2"]["Personal Information"]["Aadhar card"])){
			$objWorkSheet->setCellValue('J'.$cell_index, $student_data["doc_data"]["widget_data"]["page2"]["Personal Information"]["Aadhar card"]);
		}else{
			$objWorkSheet->setCellValue('J'.$cell_index, 'Not Available');
		}
        

        if(isset($student_data["doc_data"]["widget_data"]["page2"]["Personal Information"]["Ration card"]) && !empty($student_data["doc_data"]["widget_data"]["page2"]["Personal Information"]["Ration card"])){
			$objWorkSheet->setCellValue('K'.$cell_index, $student_data["doc_data"]["widget_data"]["page2"]["Personal Information"]["Ration card"]);
		}else{
			$objWorkSheet->setCellValue('K'.$cell_index, 'Not Available');
		}

		if(isset($student_data["doc_data"]["widget_data"]["page2"]["Personal Information"]["Aarogya sri"]) && !empty($student_data["doc_data"]["widget_data"]["page2"]["Personal Information"]["Aarogya sri"])){
			$objWorkSheet->setCellValue('L'.$cell_index, $student_data["doc_data"]["widget_data"]["page2"]["Personal Information"]["Aarogya sri"]);
		}else{
			$objWorkSheet->setCellValue('L'.$cell_index, 'Not Available');
		}
	...
	public function generate_excel_for_absent_pie($date, $dt_name = "All", $school_name = "All")
	{
		...
		$dist_list = $this->ci->ttwreis_common_model->get_all_district($dt_name);
		...
		foreach ($dist_list as $dist){
			...
			if($school_name == "All"){
				$schools_list = $this->ci->ttwreis_common_model->get_schools_by_dist_id($dist['_id']->{'$id'});
			}else{
				$schools_list = $this->ci->ttwreis_common_model->get_school_data_school_name($school_name);
			}
			...
			$reported_schools_data = $this->ci->ttwreis_common_model->get_reported_schools_count_by_dist_name($dist['dt_name'],$date);
		...
		foreach ($dist_list as $dist){
			...
			foreach ($schhols_in_dist as $school){
				...
				$hs_details = $this->ci->ttwreis_common_model->get_health_supervisors_school_id(strval($school['code']));
				...
				$school_data = $this->ci->ttwreis_common_model->get_absent_school_details($school['name']);
				...
				if($school_data){
					...
					$request = $this->ci->ttwreis_common_model->get_request_by_school_name($school['name'],$date);
					...
					$students_count = $this->ci->ttwreis_common_model->get_student_count_school_name($school['name']);
	...
	*/
	public function generate_excel_for_bmi_pie($date,$dt_name = "All", $school_name = "All")
	{
		...
		$data = $this->ci->ttwreis_common_model->get_reported_schools_bmi_count_by_dist_name($date,$dt_name,$school_name);
		...
		$file_save = BASEDIR.TENANT.'/'.$date."-TTWREIS-BMI_Report.xlsx";
		$file_name = URLCustomer.$date."-TTWREIS-BMI_Report.xlsx";
		$objWriter->save($file_save);
		//$this->secure_file_download($file_name);
		//unlink($file_name);
		return $file_name;
	...
	public function generate_excel_for_request_pie($date,$request_pie_span, $dt_name = "All", $school_name = "All")
	{
		...
		//$pie_stage1_data =  $this->ci->ttwreis_common_model->get_all_requests($date,$request_pie_span);
		
	
		
		$label = array('label' => 'Device Initiated');
		$data = json_encode($label);
		$pie_stage2_data =  $this->ci->ttwreis_common_model->drilldown_request_to_districts($data,$date,$request_pie_span,$dt_name,$school_name);
		...
		$dates = $this->ci->ttwreis_common_model->get_start_end_date($date,$request_pie_span);
		$student_data = $this->ci->ttwreis_common_model->get_all_requests_docs( $dates ['today_date'], $dates ['end_date'], "Normal", $dt_name, $school_name );
		
		$cell_index = 2;
		foreach ($student_data as $student){
			
		//	$doc = $this->ci->ttwreis_common_model->get_students_uid($student['doc_data']['widget_data']['page1']['Student Info']['Unique ID']);
			...
			//if($doc){
				...
				$objWorkSheet->setCellValue('F'.$cell_index, $student['doc_data']['widget_data']['page2']['Problem Info']['Description']);
				$objWorkSheet->setCellValue('G'.$cell_index, $student['doc_data']['widget_data']['page2']['Diagnosis Info']['Doctor Summary']);
				$objWorkSheet->setCellValue('H'.$cell_index, $student['doc_data']['widget_data']['page2']['Diagnosis Info']['Doctor Advice']);
				$objWorkSheet->setCellValue('I'.$cell_index, $student['doc_data']['widget_data']['page2']['Diagnosis Info']['Prescription']);
				$objWorkSheet->setCellValue('J'.$cell_index, $student['doc_data']['widget_data']['page2']['Review Info']['Request Type']);
				$objWorkSheet->setCellValue('K'.$cell_index, $student['doc_data']['widget_data']['page2']['Review Info']['Status']);
				$objWorkSheet->setCellValue('L'.$cell_index, $student ["doc_data"] ['widget_data'] ['page1'] ['Student Info'] ['Unique ID']);
				$cell_index ++;
			//}
		}
		...
		$dates = $this->ci->ttwreis_common_model->get_start_end_date($date,$request_pie_span);
		$student_data = $this->ci->ttwreis_common_model->get_all_requests_docs( $dates ['today_date'], $dates ['end_date'], "Emergency", $dt_name, $school_name );
		...
		$dates = $this->ci->ttwreis_common_model->get_start_end_date($date,$request_pie_span);
		$student_data = $this->ci->ttwreis_common_model->get_all_requests_docs( $dates ['today_date'], $dates ['end_date'], "Chronic", $dt_name, $school_name );
		...
		$objPHPExcel->setActiveSheetIndex(0);
			...
			$file_save = BASEDIR.TENANT.'/'.$date."-TTWREIS-Request_Report.xlsx";
			$file_name = URLCustomer.$date."-TTWREIS-Request_Report.xlsx";
			$objWriter->save($file_save);
			//$this->secure_file_download($file_name);
			//unlink($file_name);
			return $file_name;
	...
	public function generate_excel_for_screening_pie($date,$screening_pie_span, $dt_name = "All", $school_name = "All")
	{
		...
		$dates = $this->ci->ttwreis_common_model->get_start_end_date($date, $screening_pie_span);
		...
		if($school_name == "All"){
			if($dt_name == "All"){
				$pie_stage1_data =  $this->ci->ttwreis_common_model->get_all_screenings($date,$screening_pie_span);
				$cell_count = 0;
				...
				foreach ($pie_stage1_data as $pie_sector){
					...
					$pie_stage2_data =  $this->ci->ttwreis_common_model->get_drilling_screenings_abnormalities($data, $date, $screening_pie_span);
					$stage2_cell = 5;
					$stage1_value = 0;
				...
				$pie_data = $this->ci->ttwreis_common_model->get_screening_pie_stage4($dates);
				$requests1 = 0;
				$requests2 = 0;
				$requests3 = 0;
				$requests4 = 0;
			...
			$pie_data = $this->ci->ttwreis_common_model->get_screening_pie_stage5($dates);
			$requests1 = [ ];
			$requests2 = [ ];
			$requests3 = [ ];
			$requests4 = [ ];
			...
			foreach ($sheets_array as $sheet_key => $each_sheet){
				...
				if(count($each_sheet["unique_id"] ) >0){
					...
					$student_details = $this->ci->ttwreis_common_model->get_drilling_screenings_students_docs($each_sheet["unique_id"]);
					$cell_ind = 3;
					foreach ($student_details as $student){
						...
						foreach($each_sheet["ehr_value"] as $value){
							$val_arr = explode("^^",$value);
							//log_message("debug","val_arrrrrrrrrrr==========3710".print_r($val_arr,true));
	...
	function submit_request_to_doctor($form_data){
		...
		foreach ($id_array as $id){
			...
			$doc_data = '{"doc_data" : { "widget_data" : { "page1" : { "Student Info" : { "Unique ID" : "'.$id.'", "Name" : { "field_ref" : "page1_Personal Information_Name" }, "District" : { "field_ref" : "page2_Personal Information_District" }, "School Name" : { "field_ref" : "page2_Personal Information_School Name" }, "Class" : { "field_ref" : "page2_Personal Information_Class" }, "Section" : { "field_ref" : "page2_Personal Information_Section" } }, "Problem Info" : { "Identifier" : null } }, "page2" : { "Problem Info" : { "Description" : "'.$request_desc.'" }, "Diagnosis Info" : { "Doctor Summary" : null, "Doctor Advice" : "", "Prescription" : null }, "Review Info" : { "Request Type" : "Normal", "Status" : "Initiated" } } }, "stage_name" : "healthcare2016108181933756", "user_name" : "ttwreis.user#gmail.com", "chart_data" : "" }, "history" : [ { "current_stage" : "HS 1", "approval" : "true", "submitted_by" : "ttwreis.user#gmail.com", "submitted_user_type" : "TADMIN", "time" : "'.$time.'" } ], "app_properties" : { "app_name" : "Health Supervisor Request App", "app_id" : "healthcare2016108181933756" }, "doc_properties" : { "doc_id" : "'.$document_id.'", "status" : 0, "_version" : 1 }}';
				
			$user_data = '{"app_name" : "Health Supervisor Request App", "app_id" : "healthcare2016108181933756", "doc_id" : "'.$document_id.'", "stage" : "Doctor", "stg_name" : "Doctor", "status" : "new", "from_stage" : "HS 1", "from_user" : "ttwreis.user#gmail.com", "notification_param" : { "Name" : { "field_ref" : "page1_Personal Information_Name" } }, "doc_received_time" : "'.$time.'", "approval" : "true"}';
				
			$user_data_array = json_decode($user_data,true);
			$doc_data_array =json_decode($doc_data,true);
			$this->ci->ttwreis_common_model->initiate_request($doc_id,$user_data_array,$doc_data_array);
		}
	...
	{
		$total_rows = $this->ci->ttwreis_common_model->groupscount();
		...
		//find all the categories with paginate and save it in array to past to the view
		$this->data['groups'] = $this->ci->ttwreis_common_model->get_groups($config['per_page'], $page);
		//create paginate´s links
		$this->data['links'] = $this->ci->pagination->create_links();
		...
		//all users in ttwreis
		$this->data['users']["admin"] = $this->ci->ttwreis_common_model->get_all_admin_users();
		$this->data['users']["doctors"] = $this->ci->ttwreis_common_model->get_all_doctors();
		$this->data['users']["hs"] = $this->ci->ttwreis_common_model->get_all_health_supervisors();
		$this->data['users']["ha"] = $this->ci->ttwreis_common_model->get_all_cc_users();
		$this->data['users']["superior"] = $this->ci->ttwreis_common_model->get_all_superiors();
	...
	public function group_msg()
	{
		...
		$this->data['groups'] = $this->ci->ttwreis_common_model->get_all_groups();
	...
	public function insert_ehr_note($post,$username)
	{
		...
		$token = $this->ci->ttwreis_common_model->insert_ehr_note($post);
	...
	{
		$token = $this->ci->ttwreis_common_model->delete_ehr_note($doc_id);
	...
	function build_sanitation_report($sanitation_report_data)
	{
	   // Variables
		...
		if ((file_exists ( $_FILES ['file'] ['tmp_name'] [0] ) || is_uploaded_file ( $_FILES ['file'] ['tmp_name'] [0] ))) {
			...
			$token = $this->ci->ttwreis_common_model->add_news_feed ( $news_data );
			if ($token) {
	...
	public function update_news_feed() {
		...
		$news_data = $this->ci->ttwreis_common_model->get_news_feed($_POST['news_id']);
		...
		if ((file_exists ( $_FILES ['file'] ['tmp_name'] [0] ) || is_uploaded_file ( $_FILES ['file'] ['tmp_name'] [0] ))) {
			...
			$token = $this->ci->ttwreis_common_model->update_news_feed ( $news_data, $news_id );
			if ($token) {
				$data['return_error'] = false;
				return $data;
			} else {
	...
	public function app_access($doc_id)
	{
		...
		if(array_key_exists("user_type",$session_data))
		{
			...
			{
				$user = $this->ci->ion_auth->ttwreis_health_supervisor()->row();
			...
			else if($session_data['user_type'] == "TADMIN")
			{
			    $user = $this->ci->ion_auth->ttwreis_admin()->row();
		        $username = str_replace("@","#",$user->email);  
				$submitted_user_type = "TADMIN";				
			}
			else if($session_data['user_type'] == "CCUSER")
			{
			   $user = $this->ci->ion_auth->ttwreis_cc_user()->row();
		       $username = str_replace("@","#",$user->email); 
		...
		$this->ci->load->model('healthcare/healthcare2016108181933756_mod','healthcare2016108181933756_mod_calls');
		
		//search the item to show in edit form
			$healthcare2016108181933756_edit = $this->ci->healthcare2016108181933756_mod_calls->find('doc_id',$doc_id);
            $data['healthcare2016108181933756_mod'] = json_decode(json_encode($healthcare2016108181933756_edit),TRUE);
			$data['stagename'] = "HS 2";
			$data['template'] = $this->ci->healthcare2016108181933756_mod_calls->get_template();
			$app_template = $this->ci->healthcare2016108181933756_mod_calls->get_template_for_create();			
	         	 ...
	         	 foreach($page_data[$ii] as $section => $index_array)
	         	 {
	         	 	...
	         	 	foreach($index_array as $index => $value)
	         	 	{
	         	 		switch ($value['type'])
						{
						  ...
						  $query_value = $healthcare2016108181933756_edit['doc_data']['widget_data'][$page][$section][$index];
						  ...
						  //fetch document 
						  $mapper_document = $this->ci->healthcare2016108181933756_mod_calls->fetch_retriever_data_model($query_param,$query_value,$retrieval_list,$collection_name);
						  break;
						}
			...
			$this->data["hs_page"] = $this->ci->load->view('healthcare/healthcare2016108181933756_con/HS 2', $data, true);
			$this->data['message'] = false;
	...
	function hs_req_extend($form_data){
		...
		$healthcare2016108181933756_edit = $this->ci->healthcare2016108181933756_mod_calls->find('doc_id',$form_data['docid']);
		
		
		$reason_array = [];
		$student_name = "";
		//======================== student name======================================
		$student_name = $this->ci->ttwreis_common_model->get_students_uid($healthcare2016108181933756_edit['doc_data']['widget_data']["page1"]["Student Info"]["Unique ID"]);
		if($student_name){
		...
		if((file_exists ( $_FILES ['attachments'] ['tmp_name'] [0] ) || is_uploaded_file ( $_FILES ['attachments'] ['tmp_name'] [0] ))){
			...
			$config['upload_path'] = UPLOADFOLDERDIR.'public/uploads/healthcare2016108181933756_con/files/external_files/';
			$config['allowed_types'] = '*';
			...
			if(isset($healthcare2016108181933756_edit['doc_data']['external_attachments']))
			{
					   
				$external_merged_data = array_merge($healthcare2016108181933756_edit['doc_data']['external_attachments'],$external_final);
				$healthcare2016108181933756_edit['doc_data']['external_attachments'] = array_replace_recursive($healthcare2016108181933756_edit['doc_data']['external_attachments'],$external_merged_data);
			}
			else
			{
				$healthcare2016108181933756_edit['doc_data']['external_attachments'] = $external_final;
			}
		...
		$doc_symptoms = $healthcare2016108181933756_edit['doc_data']['widget_data']["page1"]["Problem Info"]["Identifier"];
		...
		if (!($symptoms == $doc_symptoms)) {
			$healthcare2016108181933756_edit['doc_data']['widget_data']["page1"]["Problem Info"]["Identifier"] = $symptoms;
			array_push($reason_array,"Symptoms changed");
		}
		
		//=====================Description check=====================================
		$description = trim($form_data['page2_ProblemInfo_Description']);
		$doc_description = trim($healthcare2016108181933756_edit['doc_data']['widget_data']["page2"]["Problem Info"]["Description"]);
		
		if ($description != $doc_description) {
			$healthcare2016108181933756_edit['doc_data']['widget_data']["page2"]["Problem Info"]["Description"] = $description;
			array_push($reason_array,"Description changed");
		}
		
		//=====================Requesttype check=====================================
		$req_type = $form_data['page2_ReviewInfo_RequestType'];
		$doc_req_type = $healthcare2016108181933756_edit['doc_data']['widget_data']["page2"]["Review Info"]["Request Type"];
		
		if ($req_type != $doc_req_type) {
			$healthcare2016108181933756_edit['doc_data']['widget_data']["page2"]["Review Info"]["Request Type"] = $req_type;
			array_push($reason_array,"Request type changed");
		}
		
		//=====================status check=====================================
		$status_type = $form_data['page2_ReviewInfo_Status'];
		$doc_status_type = $healthcare2016108181933756_edit['doc_data']['widget_data']["page2"]["Review Info"]["Status"];
		
		if ($status_type != $doc_status_type) {
			$healthcare2016108181933756_edit['doc_data']['widget_data']["page2"]["Review Info"]["Status"] = $status_type;
		...
		foreach($healthcare2016108181933756_edit['history'] as $stg_history){
			array_push($history_array, $stg_history);
		}
		$healthcare2016108181933756_edit['history'] = $history_array;
		
		$doc_update = $this->ci->ttwreis_common_model->update_doc_for_disapprove($healthcare2016108181933756_edit);
		$approval_data = array(
		...
		$approval_history = $this->ci->healthcare2016108181933756_mod_calls->get_approval_history($doc_id);
		array_push($approval_history,$approval_data);
		
		$select = array("workflow.Doctor");
		$stage_details = $this->ci->ttwreis_common_model->get_workflow_stage_details('healthcare2016108181933756','applications',$select);
		
		foreach($stage_details["workflow"]['Doctor']['UsersList'] as $user){
			$stage_type = $stage_details["workflow"]['Doctor']["Stage_Type"];
			unset($healthcare2016108181933756_edit['_id']);

			$uid_arr = explode("_",$healthcare2016108181933756_edit['doc_data']['widget_data']["page1"]["Student Info"]["Unique ID"]);
			$hs_user_col = strtolower($uid_arr[0]).".".$uid_arr[1].".hs#gmail.com";
			$this->ci->ttwreis_common_model->delete_doc_from_user_col($doc_id,$hs_user_col);
			
			$this->healthcare2016108181933756_mod = $this->ci->healthcare2016108181933756_mod_calls->web_disapprove($doc_id,$approval_history,$user,$redirected_stage,$disapproving_user,$stage_type,$current_stage,$notification_param,$reason,$healthcare2016108181933756_edit);
		}
		
		if ( $this->healthcare2016108181933756_mod ) // the information has therefore been successfully saved in the db
		{
	...
	public function ttwreis_update_personal_ehr_uid($post)
	{
		$docs = $this->ci->ttwreis_common_model->get_update_personal_ehr_uid($post['uid']);
		$this->data['docs'] = $docs['screening'];
		...
		//$this->data['docs_requests'] = $docs['request'];
		/* 
		$docs = $this->ci->ttwreis_common_model->get_student_hospital_report($post['uid']);
    ...
    {
		$uploaddir = ZIP_TTWREIS;
		log_message("debug","uploaddirrrrrrrrrr====3998".print_r($uploaddir,true));
		$config['upload_path'] 		= $uploaddir;
		$config['upload_mednote'] = './uploaddir/public/uploads/healthcare201671115519757_con/MedNote';
		...
		//$_FILES['userfile'] = $this->reArrayFiles($_FILES['userfile']);
			 ...
			 if ( ! $this->ci->upload->do_upload())
			{
				...
				log_message("debug","errorsssssss===4016".print_r($error,true));
				 $this->ci->_render_page('ttwreis_admins/ttwreis_zipfile_import_view',$error);
			}
		   else
			{
				...
				if ($zip->open($file) === TRUE) {
						 $zip->extractTo('./uploaddir/public/uploads/healthcare201671115519757_con/');
			  ...
			  log_message("debug","is_deleeeeeeee=====186".print_r($is_delete,true));
			   ...
			   {
				 //redirect('ttwreis_cc/file_upload');
				  return false;
			   } 
				return TRUE;
			   $this->data['message'] = "";
			    $this->ci->_render_page('ttwreis_admins/ttwreis_zipfile_import_view',$this->data);
			}
	...
	{  
			...
			$photo_status = "";
				   ...
				   $this->ci->load->library('excel');
					 $configUpload['upload_path'] = './uploaddir/public/uploads/healthcare201671115519757_con/MedNote/Screening.xls';
					...
					//Load excel file
					  $objPHPExcel = PHPExcel_IOFactory::load("./uploaddir/public/uploads/healthcare201671115519757_con/MedNote/Screening.xls");
		...
		if (count($check)==0)
		{
					...
					for($each_row=2 ; $each_row<=$total_rows ; $each_row++)
					{
						...
						foreach ($cellIterator as $cell)
						{
							...
							else if($check_col_array[$header_row] == "imagepath")
							{
								if(!empty($data_value))
								{
								$photo_folder_path = "./uploaddir/public/uploads/healthcare201671115519757_con/MedNote";
								$photo_folder_upload_test = "./uploaddir/public/uploads/healthcare201671115519757_con/photo";
							...
							else if($check_col_array[$header_row] == "general physian sign")
							{
								if(!empty($data_value))
								{
								$sign_folder_path = SIGNATURES_TTWREIS;
								$sign_folder_upload_test = DOCTORS_SIGN_TTWREIS;
							...
							else if($check_col_array[$header_row] == "dentist sign")
							{
								if(!empty($data_value))
								{
								$dentist_sign_folder_path = SIGNATURES_TTWREIS;
								$dentist_sign_folder_upload_test = DOCTORS_SIGN_TTWREIS;
							...
							else if($check_col_array[$header_row] == "opthomologist sign")
							{
								if(!empty($data_value))
								{
								$opthomologist_sign_folder_path = SIGNATURES_TTWREIS;
								$opthomologist_sign_folder_upload_test = DOCTORS_SIGN_TTWREIS;
							...
							else if($check_col_array[$header_row] == "audiologist sign")
							{
								if(!empty($data_value))
								{
								$audiologist_sign_folder_path = SIGNATURES_TTWREIS;
								$audiologist_sign_folder_upload_test = DOCTORS_SIGN_TTWREIS;
							...
							}else if($check_col_array[$header_row] == "attachments")
							{
								...
								$attachments_sign_folder_path = ATTACHMENTS_TTWREIS;
								$attachments_sign_folder_upload_test = "./uploaddir/public/uploads/healthcare201671115519757_con/files/external_files";
								...
								if (is_dir($attachments_sign_folder_path))
								{
								  if ($dh = opendir($attachments_sign_folder_path))
								  {
									...
									/* ob_start();
									  ...
									  log_message("debug","ttwreis===================5522");
					...
					for($i=2;$i<count($arr_data)+2;$i++)
					{
					 ...
					 $LastName = ($arr_data[$i]['lastname'] )? $arr_data[$i]['lastname'] : "" ; //Excel Column 1
					  ...
					  $Address= ($arr_data[$i]['address']) ? $arr_data[$i]['address'] : "" ;  //Excel Column 4 */ 
					    $result = $this->ci->ttwreis_common_model->unique_id_check($arr_data[$i]['hospital unique id']);
						...
						$db_doc = $this->ci->ttwreis_common_model->get_screening_reports_ehr_uid($arr_data[$i]['hospital unique id']);
						...
						$doc_data['widget_data']['page9']['Dental Check-up'] = array('Oral Hygiene' => ($arr_data[$i]['oral hygiene']) ? $arr_data[$i]['oral hygiene'] : "",
							...
							if(isset($arr_data[$i]['attachments']) && !empty($arr_data[$i]['attachments'])){
								...
								log_message('debug','doccccccccccccccccc.'.print_r($doc_data,true));
								log_message('debug','arrrrrrdtatattatattatatattatattatatat.'.print_r($arr_data[$i],true));
								//exit();
						}
						else{ 
						 ...
						 if($result == "Only personal info document")
						  {
							  //echo print_r("only personal information",true);
								//exit();
							  $this->ci->ttwreis_common_model->update_screening_details($db_doc['screening'][0]["_id"],$data_user);
							  ...
							  if(!$student_exists)
							  {
								...
								$this->ci->ttwreis_common_model->insert_screening_details($data_user);
						...
						//$this->ci->_render_page('ttwreis_cc/ttwreis_cc_dash',$data_user);
			...
			/* $this->data['message'] = "";
				$this->ci->_render_page("ttwreis_cc/to_dashboard",$this->data); */
	...
	public function get_initaite_requests_count($today_date)
	{
		...
		$initiate_count = $this->ci->ttwreis_common_model->get_initaite_requests_count($today_date);
		...
		/* if(count($initiate_count)>0)
		{
			...
			$doctors_response_count = $this->ci->ttwreis_common_model->get_doctors_response_count($today_date);
		...
		if(count($doctors_response_count)>0)
		{
			foreach($doctors_response_count as $index => $history)
			{
				...
				if($doctor_submitted_list == "ttwreis.dr1#gmail.com")
				{
				...
				else if($doctor_submitted_list == "ttwreis.dr2#gmail.com")
				{
				...
				else if($doctor_submitted_list == "ttwreis.dr3#gmail.com")
				{
				...
				else if($doctor_submitted_list == "ttwreis.dr4#gmail.com")
				{
				...
				else if($doctor_submitted_list == "ttwreis.dr6#gmail.com")
				{
				...
				else if($doctor_submitted_list == "ttwreis.dr11#gmail.com")
				{
					$this->data['doctor_name_dr11'] = $history['history'][1]['submitted_by_name'];
					array_push($doctors_email_list_dr11,$doctor_submitted_list);
				    $this->data['doctors_count_list_dr11'] = count($doctors_email_list_dr11);
				}
				else if($doctor_submitted_list == "ttwreis.dr12#gmail.com")
				{
					$this->data['doctor_name_dr12'] = $history['history'][1]['submitted_by_name'];
					array_push($doctors_email_list_dr12,$doctor_submitted_list);
				    $this->data['doctors_count_list_dr12'] = count($doctors_email_list_dr12);
				}
				else if($doctor_submitted_list == "ttwreis.dr13#gmail.com")
				{
					$this->data['doctor_name_dr13'] = $history['history'][1]['submitted_by_name'];
	...
	public function bmi_pie_view_lib_month_wise($current_month,$district_name, $school_name,$student_type = false,$student_age = false){
		...
		$count = 0;
		$bmi_report = $this->ci->ttwreis_common_model->get_bmi_report_model($current_month, $district_name, $school_name,$student_type,$student_age);
		...
		{
			$dist_list = $this->ci->ttwreis_common_model->get_all_district($district_name);
			//log_message('debug','panacea_get_all_district=============================6397=='.print_r($dist_list, true));

			$dist_id = $dist_list[0]['_id'];
			
			$bmi_reported_schools_list = $this->ci->ttwreis_common_model->get_bmi_submitted_schools_list($current_month,$district_name,$dist_id);
	...
	public function generate_bmi_report_to_excel_lib($date,$district_name, $school_name)
	{
		...
		$objPHPExcel->getProperties()->setTitle($date."-TTWREIS BMI Report.xlsx");
		$objPHPExcel->getProperties()->setSubject($date."-TTWREIS BMI Report.xlsx");
		$objPHPExcel->getProperties()->setDescription("BMI report of TTWREIS");
		...
		$data = $this->ci->ttwreis_common_model->export_bmi_reports_monthly_to_excel( $date,$district_name,$school_name);
		...
		$file_save = BASEDIR.TENANT.'/'.$date."-TTWREIS BMI Report.xlsx";
		
		$file_name = URLCustomer.$date."-TTWREIS BMI Report.xlsx";
		$objWriter->save($file_save);
		...
		public function ttwreis_chronic_pie_view(){
		
		$count = 0;
		$request_report = $this->ci->ttwreis_common_model->get_chronic_request();
		foreach ($request_report as $value){
			$count = $count + intval($value['value']);
		}
		if($count > 0){
	...
	public function update_chronic_request_pie($status_type){
		...
		$count = 0;
		$request_report = $this->ci->ttwreis_common_model->update_chronic_request_pie($status_type);
		foreach ($request_report as $value){
			$count = $count + intval($value['value']);
		}
		if($count > 0){
	...
	{
		$test = $this->ci->ttwreis_common_model->get_sanitation_report_fields_count($dt_name,$school_name);
	...
	public function get_initaite_requests_count_today_date($today_date)
		{
			...
			$initiate_count = $this->ci->ttwreis_common_model->get_initaite_requests_count_today_date($today_date);

			$requests_count = $this->ci->ttwreis_common_model->get_requests_count_today_date($today_date);
			...
			$doctors_response_count = $this->ci->ttwreis_common_model->get_doctors_response_count_today_date($today_date);
		...
		if ($this->ci->upload->do_upload("file"))
		{
			...
			if (count($check)==0) {
					...
					foreach ($cellIterator as $cell) {
						...
						$temp_data = trim(iconv("UTF-8","ISO-8859-1",$cell->getValue())," \t\n\r\0\x0B\xA0");
							...
							// Loggedin user
								...
								$this->data['distslist'] = $this->ci->ttwreis_schools_common_model->get_all_district();
				...
				for($j=2;$j<count($arr_data)+2;$j++)
				{
					...
					$is_exists = $this->ci->ttwreis_schools_common_model->check_if_doc_exists($unique_id);
					...
					if(!empty($is_exists)){
					    ...
					    $monthly_bmi = array(
						 ...
						 $existing_update = $this->ci->ttwreis_schools_common_model->ttwreis_update_bmi_values($ndate,$monthly_bmi,$unique_id);
			    	...
			    	array_push($bmi_final,$monthly_bmi);
						...
						$history['last_stage']['current_stage'] = "stage1";
						$history['last_stage']['approval'] = "true";
						$history['last_stage']['submitted_by'] = "medusertw1#gmail.com";
						$history['last_stage']['time'] = date("Y-m-d H:i:s");

						$this->ci->ttwreis_schools_common_model->create_bmi_values_monthly($doc_data,$doc_properties,$history);
	...
	{
		$docs = $this->ci->ttwreis_common_model->get_reports_ehr_uid_new_html_static_hs($unique_id,$school_name);
	...
	public function send_message_to_doctors($request_type,$unique_id,$student_name,$issueinfo_1)
		{
			...
			$doctors_1 = "8179075893";
	  	    ...
	  	    $doctors_5 = "7981982640";
	  	    $doctors_6 = "9700241243";
	...
	public function hb_pie_view_lib_month_wise($current_month,$district_name, $school_name,$student_type = false,$student_age = false)
	{
		$current_month = substr($current_month,0,-3);
		$count = 0;
		$hb_report = $this->ci->ttwreis_common_model->get_hb_report_model($current_month, $district_name, $school_name,$student_type,$student_age);
		...
		$hb_reported_schools_list = $this->ci->ttwreis_common_model->get_hb_submitted_schools_list($current_month,$district_name,$dist_id);
	...
	public function generate_health_summary_report($unique_id)
	{
	 ...
	 $summary_report.= "<html><body>";
	   ...
	   $student_docs = $this->ci->ttwreis_common_model->get_students_uid_for_print($unique_id);
	...
	// Request Information
	   ...
	   if($page2['Review Info']['Request Type'] == 'Normal')
	   {
	   	...
	   	{
	   		foreach($page1['Problem Info'] as $identifiers => $values){
	  			foreach($values as $value){
	  			 	if(isset($value)){
	  		   				$identifier_s = $value;
	  		   		}else{
	   	...
	   	{
	  		foreach($page1['Problem Info']['Emergency'] as $identifiers => $values){
	  			foreach($values as $value){
	  			 	if(isset($value)){
	  		   				$identifier_s = $value;
	  		   		}else{
	   	...
	   	{
	   		foreach($page1['Problem Info']['Chronic'] as $identifierss => $values){
	   			foreach($values as $value){
	   			 	if(isset($value)){
	   		   				$identifier_s = $value;
	   		   		}else{
	...
	function to_dashboard_old_dash($date = FALSE, $request_duration = "Yearly", $screening_duration = "Yearly")
	{
		...
		$count = 0;
		$symptoms_report = $this->ci->ttwreis_common_model->get_all_symptoms_old_dash($date,$request_duration);
		foreach ($symptoms_report as $value){
			$count = $count + intval($value['value']);
		}
		if($count > 0){
		...
		$count = 0;
		$request_report = $this->ci->ttwreis_common_model->get_all_requests_old_dash($date,$request_duration);
		foreach ($request_report as $value){
			$count = $count + intval($value['value']);
		}
		if($count > 0){
		...
		$this->data['last_screening_update'] = $this->ci->ttwreis_common_model->get_last_screening_update($date,$screening_duration);
		...
		$this->data ['news_feeds'] = $this->ci->ttwreis_common_model->get_all_news_feeds();
	...
	{
		$docs = $this->ci->ttwreis_common_model->get_study_circle_data_model($post);
		$this->data['docs'] = $docs['screening'];
		//$this->data['docs_requests'] = $docs['request'];
		/* 
	...
	public function get_excel_dr_not_responded_reports_lib($start_date, $end_date)
{
    ...
    $reports_data = $this->ci->ttwreis_common_model->get_excel_dr_not_responded_reports_model($start_date, $end_date);
...
public function get_excel_for_covidcases_lib($start_date, $end_date)
    ...
    $reports_data = $this->ci->ttwreis_common_model->ttwreis_get_excel_covidcases_model($start_date, $end_date);
...
public function get_excel_for_covid_cured_cases_lib($start_date, $end_date)
    ...
    $reports_data = $this->ci->ttwreis_common_model->get_excel_covid_cured_cases_model($start_date, $end_date);
...
public function get_excel_for_screening_overall($dist, $scl, $academic)
{
	...
	$objPHPExcel->getProperties()->setCreator("Havik Healthcare Technologies Pvt. Ltd");
	$objPHPExcel->getProperties()->setLastModifiedBy("Harish Seelam");
	$objPHPExcel->getProperties()->setTitle($today_date.$title."Screening Information");
	$objPHPExcel->getProperties()->setSubject($today_date.$title."Screening Information");
	$objPHPExcel->getProperties()->setDescription($academic."Screening Report");

	//Add new sheet
	$objWorkSheet = $objPHPExcel->createSheet(0); //select sheet index you want to print
	$objWorkSheet->setTitle("Overall Screening Report");
	$objWorkSheet->getRowDimension(1)->setRowHeight(44);
	...
	$objWorkSheet->setCellValue('A1', 'Hospital Unique ID')->getStyle('A1')->applyFromArray($styleArray);
	$objWorkSheet->setCellValue('B1', 'Name')->getStyle('B1')->applyFromArray($styleArray);
	$objWorkSheet->setCellValue('C1', 'Class')->getStyle('C1')->applyFromArray($styleArray);
	$objWorkSheet->setCellValue('D1', 'Section')->getStyle('D1')->applyFromArray($styleArray);
	$objWorkSheet->setCellValue('E1', 'Gender')->getStyle('E1')->applyFromArray($styleArray);
	$objWorkSheet->setCellValue('F1', 'Date of Birth')->getStyle('F1')->applyFromArray($styleArray);
	$objWorkSheet->setCellValue('G1', 'School Name')->getStyle('G1')->applyFromArray($styleArray);
	$objWorkSheet->setCellValue('H1', 'District')->getStyle('H1')->applyFromArray($styleArray);
	$objWorkSheet->setCellValue('I1', 'Father Name')->getStyle('I1')->applyFromArray($styleArray);
	$objWorkSheet->setCellValue('J1', 'AD No')->getStyle('J1')->applyFromArray($styleArray);
	$objWorkSheet->setCellValue('K1', 'Date Of Exam')->getStyle('K1')->applyFromArray($styleArray);
	$objWorkSheet->setCellValue('L1', 'Mobile')->getStyle('L1')->applyFromArray($styleArray);
	$objWorkSheet->setCellValue('M1', 'Height cms')->getStyle('M1')->applyFromArray($styleArray);
	$objWorkSheet->setCellValue('N1', 'Weight kgs')->getStyle('N1')->applyFromArray($styleArray);
	$objWorkSheet->setCellValue('O1', 'BMI%')->getStyle('O1')->applyFromArray($styleArray);
	$objWorkSheet->setCellValue('P1', 'Pulse')->getStyle('P1')->applyFromArray($styleArray);
	$objWorkSheet->setCellValue('Q1', 'B P')->getStyle('Q1')->applyFromArray($styleArray);
	$objWorkSheet->setCellValue('R1', 'H B')->getStyle('R1')->applyFromArray($styleArray);
	$objWorkSheet->setCellValue('S1', 'Blood Group')->getStyle('S1')->applyFromArray($styleArray);
	$objWorkSheet->setCellValue('T1', 'Check the box if normal else describe abnormalities')->getStyle('T1')->applyFromArray($styleArray);
	$objWorkSheet->setCellValue('U1', 'Ortho')->getStyle('U1')->applyFromArray($styleArray);
	$objWorkSheet->setCellValue('V1', 'Postural')->getStyle('V1')->applyFromArray($styleArray);
	$objWorkSheet->setCellValue('W1', 'Description')->getStyle('W1')->applyFromArray($styleArray);
	$objWorkSheet->setCellValue('X1', 'Treatment')->getStyle('X1')->applyFromArray($styleArray);
	$objWorkSheet->setCellValue('Y1', 'Skin conditions')->getStyle('Y1')->applyFromArray($styleArray);
	$objWorkSheet->setCellValue('Z1', 'Defects at Birth')->getStyle('Z1')->applyFromArray($styleArray);
	$objWorkSheet->setCellValue('AA1', 'Deficencies')->getStyle('AA1')->applyFromArray($styleArray);
	$objWorkSheet->setCellValue('AB1', 'Childhood Diseases')->getStyle('AB1')->applyFromArray($styleArray);
	$objWorkSheet->setCellValue('AC1', 'N A D')->getStyle('AC1')->applyFromArray($styleArray);
	$objWorkSheet->setCellValue('AD1', 'Screenings')->getStyle('AD1')->applyFromArray($styleArray);
	$objWorkSheet->setCellValue('AE1', 'Without Glasses Right')->getStyle('AE1')->applyFromArray($styleArray);
	$objWorkSheet->setCellValue('AF1', 'Without Glasses Left')->getStyle('AF1')->applyFromArray($styleArray);
	$objWorkSheet->setCellValue('AG1', 'With Glasses Right')->getStyle('AG1')->applyFromArray($styleArray);
	$objWorkSheet->setCellValue('AH1', 'With Glasses Left')->getStyle('AH1')->applyFromArray($styleArray);
	$objWorkSheet->setCellValue('AI1', 'Colour Blindness Right')->getStyle('AI1')->applyFromArray($styleArray);
	$objWorkSheet->setCellValue('AJ1', 'Colour Blindness Left')->getStyle('AJ1')->applyFromArray($styleArray);
	$objWorkSheet->setCellValue('AK1', 'Eye Lids')->getStyle('AK1')->applyFromArray($styleArray);
	$objWorkSheet->setCellValue('AL1', 'Conjunctiva')->getStyle('AL1')->applyFromArray($styleArray);
	$objWorkSheet->setCellValue('AM1', 'Cornea')->getStyle('AM1')->applyFromArray($styleArray);
	$objWorkSheet->setCellValue('AN1', 'Pupil')->getStyle('AN1')->applyFromArray($styleArray);
	$objWorkSheet->setCellValue('AO1', 'Complaints')->getStyle('AO1')->applyFromArray($styleArray);
	$objWorkSheet->setCellValue('AP1', 'Wearing Spectacles')->getStyle('AP1')->applyFromArray($styleArray);
	$objWorkSheet->setCellValue('AQ1', 'Subjective Refraction')->getStyle('AQ1')->applyFromArray($styleArray);
	$objWorkSheet->setCellValue('AR1', 'Ocular Diagnosis')->getStyle('AR1')->applyFromArray($styleArray);
	$objWorkSheet->setCellValue('AS1', 'Colour Blindness Referral Made')->getStyle('AS1')->applyFromArray($styleArray);
	$objWorkSheet->setCellValue('AT1', 'Colour Blindness Description')->getStyle('AT1')->applyFromArray($styleArray);
	$objWorkSheet->setCellValue('AU1', 'Auditory Screening Right')->getStyle('AU1')->applyFromArray($styleArray);
	$objWorkSheet->setCellValue('AV1', 'Auditory Screening Left')->getStyle('AV1')->applyFromArray($styleArray);
	$objWorkSheet->setCellValue('AW1', 'Speech Screening')->getStyle('AW1')->applyFromArray($styleArray);
	$objWorkSheet->setCellValue('AX1', 'D D and disability')->getStyle('AX1')->applyFromArray($styleArray);
	$objWorkSheet->setCellValue('AY1', 'Auditory Screening Description')->getStyle('AY1')->applyFromArray($styleArray);
	$objWorkSheet->setCellValue('AZ1', 'Auditory Screening Referral Made')->getStyle('AZ1')->applyFromArray($styleArray);
	$objWorkSheet->setCellValue('BA1', 'Oral Hygiene')->getStyle('BA1')->applyFromArray($styleArray);
	$objWorkSheet->setCellValue('BB1', 'Carious Teeth')->getStyle('BB1')->applyFromArray($styleArray);
	$objWorkSheet->setCellValue('BC1', 'Flourosis')->getStyle('BC1')->applyFromArray($styleArray);
	$objWorkSheet->setCellValue('BD1', 'Orthodontic Treatment')->getStyle('BD1')->applyFromArray($styleArray);
	$objWorkSheet->setCellValue('BE1', 'Indication for extraction')->getStyle('BE1')->applyFromArray($styleArray);
	$objWorkSheet->setCellValue('BF1', 'Root Canal Treatment')->getStyle('BF1')->applyFromArray($styleArray);
	$objWorkSheet->setCellValue('BG1', 'CROWNS')->getStyle('BG1')->applyFromArray($styleArray);
	$objWorkSheet->setCellValue('BH1', 'Fixed Partial Denture')->getStyle('BH1')->applyFromArray($styleArray);
	$objWorkSheet->setCellValue('BI1', 'Curettage')->getStyle('BI1')->applyFromArray($styleArray);
	$objWorkSheet->setCellValue('BJ1', 'Estimated Amount')->getStyle('BJ1')->applyFromArray($styleArray);
	$objWorkSheet->setCellValue('BK1', 'DC 11')->getStyle('BK1')->applyFromArray($styleArray);
	$objWorkSheet->setCellValue('BL1', 'DC 12')->getStyle('BL1')->applyFromArray($styleArray);
	$objWorkSheet->setCellValue('BM1', 'DC 13')->getStyle('BM1')->applyFromArray($styleArray);
	$objWorkSheet->setCellValue('BN1', 'DC 14')->getStyle('BN1')->applyFromArray($styleArray);
	$objWorkSheet->setCellValue('BO1', 'Dental Check-up Referral Made')->getStyle('BO1')->applyFromArray($styleArray);
	$objWorkSheet->setCellValue('BP1', 'Dental Check-up Result')->getStyle('BP1')->applyFromArray($styleArray);
	...
	$objWorkSheet->getColumnDimension("BF")->setAutoSize(true);
	$objWorkSheet->getColumnDimension("BG")->setAutoSize(true);
	$objWorkSheet->getColumnDimension("BH")->setAutoSize(true);
	$objWorkSheet->getColumnDimension("BI")->setAutoSize(true);
	$objWorkSheet->getColumnDimension("BJ")->setAutoSize(true);
	$objWorkSheet->getColumnDimension("BK")->setAutoSize(true);
	$objWorkSheet->getColumnDimension("BL")->setAutoSize(true);
	$objWorkSheet->getColumnDimension("BM")->setAutoSize(true);
	$objWorkSheet->getColumnDimension("BN")->setAutoSize(true);
	$objWorkSheet->getColumnDimension("B0")->setAutoSize(true);
	$objWorkSheet->getColumnDimension("BP")->setAutoSize(true);

	$screening_data = $this->ci->ttwreis_common_model->get_excel_for_screening_overall($dist, $scl, $academic);

	$cell_index = 2;

	foreach ($screening_data as $screening) {
		
		$objWorkSheet->setCellValue('A'.$cell_index, $screening ["doc_data"] ['widget_data'] ['page1'] ['Personal Information'] ['Hospital Unique ID']);
		$objWorkSheet->setCellValue('B'.$cell_index, $screening ["doc_data"] ['widget_data'] ['page1'] ['Personal Information'] ['Name']);
		$objWorkSheet->setCellValue('C'.$cell_index, $screening ["doc_data"] ['widget_data'] ['page2'] ['Personal Information'] ['Class']);
		$objWorkSheet->setCellValue('D'.$cell_index, $screening ["doc_data"] ['widget_data'] ['page2'] ['Personal Information'] ['Section']);
		$objWorkSheet->setCellValue('E'.$cell_index, (isset($screening ["doc_data"] ['widget_data'] ['page1'] ['Personal Information'] ['Gender'])) ? $screening ["doc_data"] ['widget_data'] ['page1'] ['Personal Information'] ['Gender'] : "Nil");
		$objWorkSheet->setCellValue('F'.$cell_index, $screening ["doc_data"] ['widget_data'] ['page1'] ['Personal Information'] ['Date of Birth']);
		$objWorkSheet->setCellValue('G'.$cell_index, $screening ["doc_data"] ['widget_data'] ['page2'] ['Personal Information'] ['School Name']);
		$objWorkSheet->setCellValue('H'.$cell_index, $screening ["doc_data"] ['widget_data'] ['page2'] ['Personal Information'] ['District']);
		$objWorkSheet->setCellValue('I'.$cell_index, $screening ["doc_data"] ['widget_data'] ['page2'] ['Personal Information'] ['Father Name']);
		

		$objWorkSheet->setCellValue('J'.$cell_index, $screening ["doc_data"] ['widget_data'] ['page2'] ['Personal Information'] ['AD No']);
		

		if(!empty($screening ["doc_data"] ['widget_data'] ['page2'] ['Personal Information'] ['Date of Exam'])){
			$objWorkSheet->setCellValue('K'.$cell_index, $screening ["doc_data"] ['widget_data'] ['page2'] ['Personal Information'] ['Date of Exam']);
		}else{
			$objWorkSheet->setCellValue('K'.$cell_index, 'Nil');
		}

		if((!empty($screening ["doc_data"] ['widget_data'] ['page1'] ['Personal Information'] ['Mobile']['mob_num'])) || ($screening ["doc_data"] ['widget_data'] ['page1'] ['Personal Information'] ['Mobile']['mob_num'] == 'Nil'))
		{

		$objWorkSheet->setCellValue('L'.$cell_index, $screening ["doc_data"] ['widget_data'] ['page1'] ['Personal Information'] ['Mobile']['mob_num']);
		}else
		{
			$objWorkSheet->setCellValue('L'.$cell_index, 'Nil');
		}

		$objWorkSheet->setCellValue('M'.$cell_index, (isset($screening ["doc_data"] ['widget_data'] ['page3'] ['Physical Exam'] ['Height cms'])) ? $screening ["doc_data"] ['widget_data'] ['page3'] ['Physical Exam'] ['Height cms'] : 'Nil');

		$objWorkSheet->setCellValue('N'.$cell_index, (isset($screening ["doc_data"] ['widget_data'] ['page3'] ['Physical Exam'] ['Weight kgs'])) ? $screening ["doc_data"] ['widget_data'] ['page3'] ['Physical Exam'] ['Weight kgs'] : 'Nil');

		$objWorkSheet->setCellValue('O'.$cell_index, (isset($screening ["doc_data"] ['widget_data'] ['page3'] ['Physical Exam'] ['BMI%'])) ? $screening ["doc_data"] ['widget_data'] ['page3'] ['Physical Exam'] ['BMI%'] : 'Nil');

		$objWorkSheet->setCellValue('P'.$cell_index, (isset($screening ["doc_data"] ['widget_data'] ['page3'] ['Physical Exam'] ['Pulse'])) ? $screening ["doc_data"] ['widget_data'] ['page3'] ['Physical Exam'] ['Pulse'] : 'Nil');

		$objWorkSheet->setCellValue('Q'.$cell_index, (isset($screening ["doc_data"] ['widget_data'] ['page3'] ['Physical Exam'] ['B P'])) ? $screening ["doc_data"] ['widget_data'] ['page3'] ['Physical Exam'] ['B P'] : 'Nil');

		$objWorkSheet->setCellValue('R'.$cell_index, (isset($screening ["doc_data"] ['widget_data'] ['page3'] ['Physical Exam'] ['H B'])) ? $screening ["doc_data"] ['widget_data'] ['page3'] ['Physical Exam'] ['H B'] : 'Nil');

		$objWorkSheet->setCellValue('S'.$cell_index, (isset($screening ["doc_data"] ['widget_data'] ['page3'] ['Physical Exam'] ['Blood Group'])) ? $screening ["doc_data"] ['widget_data'] ['page3'] ['Physical Exam'] ['Blood Group'] : 'Nil');

		/*$objWorkSheet->setCellValue('T'.$cell_index, (isset($screening ["doc_data"] ['widget_data'] ['page3'] ['Physical Exam'] ['Ni Gluc'])) ? $screening ["doc_data"] ['widget_data'] ['page3'] ['Physical Exam'] ['Ni Gluc'] : 'Nil');

		$objWorkSheet->setCellValue('U'.$cell_index, (isset($screening ["doc_data"] ['widget_data'] ['page3'] ['Physical Exam'] ['Temperature'])) ? $screening ["doc_data"] ['widget_data'] ['page3'] ['Physical Exam'] ['Temperature'] : 'Nil');

		$objWorkSheet->setCellValue('V'.$cell_index, (isset($screening ["doc_data"] ['widget_data'] ['page3'] ['Physical Exam'] ['B P'])) ? $screening ["doc_data"] ['widget_data'] ['page3'] ['Physical Exam'] ['B P'] : 'Nil');*/


		if(is_array($screening['doc_data']['widget_data']['page4']['Doctor Check Up']['Check the box if normal else describe abnormalities']) && !empty($screening['doc_data']['widget_data']['page4']['Doctor Check Up']['Check the box if normal else describe abnormalities'])){
			$objWorkSheet->setCellValue('T'.$cell_index, (isset($screening['doc_data']['widget_data']['page4']['Doctor Check Up']['Check the box if normal else describe abnormalities']) && ($screening['doc_data']['widget_data']['page4']['Doctor Check Up']['Check the box if normal else describe abnormalities'] == !array()) ) ? (implode(', ',  $screening['doc_data']['widget_data']['page4']['Doctor Check Up']['Check the box if normal else describe abnormalities'])) : 'Nil');
		}else
		{
			$objWorkSheet->setCellValue('T'.$cell_index, 'Nil');
		}
		

		if(is_array($screening['doc_data']['widget_data']['page4']['Doctor Check Up']['Ortho'])){

		$objWorkSheet->setCellValue('U'.$cell_index, (isset($screening['doc_data']['widget_data']['page4']['Doctor Check Up']['Ortho']) && ($screening['doc_data']['widget_data']['page4']['Doctor Check Up']['Ortho'] == !array() )) ? (implode(', ',  $screening['doc_data']['widget_data']['page4']['Doctor Check Up']['Ortho'])) : 'Nil');
		}else
		{
			$objWorkSheet->setCellValue('U'.$cell_index, 'Nil');
		}

		if(is_array($screening['doc_data']['widget_data']['page4']['Doctor Check Up']['Postural'])){

		$objWorkSheet->setCellValue('V'.$cell_index, (isset($screening['doc_data']['widget_data']['page4']['Doctor Check Up']['Postural']) && ($screening['doc_data']['widget_data']['page4']['Doctor Check Up']['Postural'] == !array())) ? (implode(', ',  $screening['doc_data']['widget_data']['page4']['Doctor Check Up']['Postural'])) : 'Nil');
		}else
		{
			$objWorkSheet->setCellValue('V'.$cell_index, 'Nil');
		}

		$objWorkSheet->setCellValue('W'.$cell_index, (isset($screening['doc_data']['widget_data']['page4']['Doctor Check Up']['Description'])) ? $screening['doc_data']['widget_data']['page4']['Doctor Check Up']['Description'] : 'Nil');

		$objWorkSheet->setCellValue('X'.$cell_index, (isset($screening['doc_data']['widget_data']['page4']['Doctor Check Up']['Treatment'])) ? $screening['doc_data']['widget_data']['page4']['Doctor Check Up']['Treatment'] : 'Nil');

		if(is_array($screening['doc_data']['widget_data']['page4']['Doctor Check Up']['Skin conditions'])){

		$objWorkSheet->setCellValue('Y'.$cell_index, (isset($screening['doc_data']['widget_data']['page4']['Doctor Check Up']['Skin conditions']) && ($screening['doc_data']['widget_data']['page4']['Doctor Check Up']['Skin conditions'] == !array())) ? (implode(', ',  $screening['doc_data']['widget_data']['page4']['Doctor Check Up']['Skin conditions'])) : 'Nil');
		}else
		{
			$objWorkSheet->setCellValue('Y'.$cell_index, 'Nil');
		}

		if(is_array($screening['doc_data']['widget_data']['page5']['Doctor Check Up']['Defects at Birth'])){

		$objWorkSheet->setCellValue('Z'.$cell_index, (isset($screening['doc_data']['widget_data']['page5']['Doctor Check Up']['Defects at Birth']) && ($screening['doc_data']['widget_data']['page5']['Doctor Check Up']['Defects at Birth'] == !array())) ? (implode(', ',  $screening['doc_data']['widget_data']['page5']['Doctor Check Up']['Defects at Birth'])) : 'Nil'); 
		}else
		{
			$objWorkSheet->setCellValue('Z'.$cell_index, 'Nil');
		}

		if (is_array($screening ["doc_data"] ['widget_data'] ['page5'] ['Doctor Check Up'] ['Deficencies'])) {
			
		$objWorkSheet->setCellValue('AA'.$cell_index, (isset($screening ["doc_data"] ['widget_data'] ['page5'] ['Doctor Check Up'] ['Deficencies']) && ($screening ["doc_data"] ['widget_data'] ['page5'] ['Doctor Check Up'] ['Deficencies'] == !array())) ? implode(',', $screening ["doc_data"] ['widget_data'] ['page5'] ['Doctor Check Up'] ['Deficencies']) : 'Nil');
		}else
		{
			$objWorkSheet->setCellValue('AA'.$cell_index, 'Nil');
		}

		if(is_array($screening ["doc_data"] ['widget_data'] ['page5'] ['Doctor Check Up'] ['Childhood Diseases'])){

		$objWorkSheet->setCellValue('AB'.$cell_index, (isset($screening ["doc_data"] ['widget_data'] ['page5'] ['Doctor Check Up'] ['Childhood Diseases']) && ($screening ["doc_data"] ['widget_data'] ['page5'] ['Doctor Check Up'] ['Childhood Diseases'] == !array())) ? implode(',', $screening ["doc_data"] ['widget_data'] ['page5'] ['Doctor Check Up'] ['Childhood Diseases']) : 'Nil');
		}else
		{
			$objWorkSheet->setCellValue('AB'.$cell_index, 'Nil');
		}
		
		$objWorkSheet->setCellValue('AC'.$cell_index, (isset($screening['doc_data']['widget_data']['page5']['Doctor Check Up']['N A D'])) ? $screening['doc_data']['widget_data']['page5']['Doctor Check Up']['N A D'] : 'Nil');

		//$objWorkSheet->setCellValue('AD'.$cell_index, (isset($screening['doc_data']['widget_data']['page5']['Doctor Check Up']['Defects at Birth'])) ? (implode(', ',  $screening['doc_data']['widget_data']['page5']['Doctor Check Up']['Defects at Birth'])) : 'Nil');

		//$objWorkSheet->setCellValue('AE'.$cell_index, (isset($screening['doc_data']['widget_data']['page5']['Doctor Check Up']['Deficencies'])) ? (implode(', ',  $screening['doc_data']['widget_data']['page5']['Doctor Check Up']['Deficencies'])) : 'Nil');

		//$objWorkSheet->setCellValue('AF'.$cell_index, (isset($screening['doc_data']['widget_data']['page5']['Doctor Check Up']['Childhood Diseases'])) ? (implode(', ',  $screening['doc_data']['widget_data']['page5']['Doctor Check Up']['Childhood Diseases'])) : 'Nil');

		//$objWorkSheet->setCellValue('AG'.$cell_index, isset($screening ['doc_data']['widget_data']['page5']['Doctor Check Up']['Referral Made']) ? $screening ['doc_data']['widget_data']['page5']['Doctor Check Up']['Referral Made']);

		//$objWorkSheet->setCellValue('AH'.$cell_index, (isset($screening ["doc_data"] ['widget_data'] ['page5'] ['Doctor Check Up'] ['N A D'])) ? ($screening ["doc_data"] ['widget_data'] ['page5'] ['Doctor Check Up'] ['N A D']) : 'Nil');

		if(is_array($screening ["doc_data"] ['widget_data'] ['page6'] ['Screenings'])){

		$objWorkSheet->setCellValue('AD'.$cell_index, (isset($screening ["doc_data"] ['widget_data'] ['page6'] ['Screenings']) && ($screening ["doc_data"] ['widget_data'] ['page6'] ['Screenings'] == !array())) ? implode(',', $screening ["doc_data"] ['widget_data'] ['page6'] ['Screenings']) : 'Nil');
		}else
		{
			$objWorkSheet->setCellValue('AD'.$cell_index, 'Nil');
		}

		$objWorkSheet->setCellValue('AE'.$cell_index, (isset($screening ["doc_data"] ['widget_data'] ['page6'] ['Without Glasses'] ['Right'])) ? $screening ["doc_data"] ['widget_data'] ['page6'] ['Without Glasses'] ['Right'] : 'Nil');

		$objWorkSheet->setCellValue('AF'.$cell_index, (isset($screening ["doc_data"] ['widget_data'] ['page6'] ['Without Glasses'] ['Left'])) ? $screening ["doc_data"] ['widget_data'] ['page6'] ['Without Glasses'] ['Left'] : 'Nil');

		$objWorkSheet->setCellValue('AG'.$cell_index, (isset($screening ["doc_data"] ['widget_data'] ['page6'] ['With Glasses'] ['Right'])) ? $screening ["doc_data"] ['widget_data'] ['page6'] ['With Glasses'] ['Right'] : 'Nil');

		$objWorkSheet->setCellValue('AH'.$cell_index, (isset($screening ["doc_data"] ['widget_data'] ['page6'] ['With Glasses'] ['Left'])) ? $screening ["doc_data"] ['widget_data'] ['page6'] ['With Glasses'] ['Left'] : 'Nil');

		//$objWorkSheet->setCellValue('AM'.$cell_index, (isset($screening['doc_data']['widget_data']['page6']['Vision Screening'])) ? implode(', ',  $screening['doc_data']['widget_data']['page6']['Vision Screening']) : 'Nil');

		//$objWorkSheet->setCellValue('AN'.$cell_index, (isset($screening['doc_data']['widget_data']['page6']['Bitot Night Blindness'])) ? implode(', ',  $screening['doc_data']['widget_data']['page6']['Bitot Night Blindness']) : 'Nil');

		$objWorkSheet->setCellValue('AI'.$cell_index, (isset($screening ["doc_data"] ['widget_data'] ['page7'] ['Colour Blindness'] ['Right'])) ? $screening ["doc_data"] ['widget_data'] ['page7'] ['Colour Blindness'] ['Right'] : 'Nil');

		$objWorkSheet->setCellValue('AJ'.$cell_index, (isset($screening ["doc_data"] ['widget_data'] ['page7'] ['Colour Blindness'] ['Left'])) ? $screening ["doc_data"] ['widget_data'] ['page7'] ['Colour Blindness'] ['Left'] : 'Nil');

		$objWorkSheet->setCellValue('AK'.$cell_index, (isset($screening ["doc_data"] ['widget_data'] ['page7'] ['Colour Blindness'] ['Eye Lids'])) ? $screening ["doc_data"] ['widget_data'] ['page7'] ['Colour Blindness'] ['Eye Lids'] : 'Nil');

		
		$objWorkSheet->setCellValue('AL'.$cell_index, (isset($screening ["doc_data"] ['widget_data'] ['page7'] ['Colour Blindness'] ['Conjunctiva'])) ? $screening ["doc_data"] ['widget_data'] ['page7'] ['Colour Blindness'] ['Conjunctiva'] : 'Nil');
		$objWorkSheet->setCellValue('AM'.$cell_index, (isset($screening ["doc_data"] ['widget_data'] ['page7'] ['Colour Blindness'] ['Cornea'])) ? $screening ["doc_data"] ['widget_data'] ['page7'] ['Colour Blindness'] ['Cornea'] : 'Nil');

		$objWorkSheet->setCellValue('AN'.$cell_index, (isset($screening ["doc_data"] ['widget_data'] ['page7'] ['Colour Blindness'] ['Pupil'])) ? $screening ["doc_data"] ['widget_data'] ['page7'] ['Colour Blindness'] ['Pupil'] : 'Nil');

		$objWorkSheet->setCellValue('AO'.$cell_index, (isset($screening ["doc_data"] ['widget_data'] ['page7'] ['Colour Blindness'] ['Complaints'])) ? $screening ["doc_data"] ['widget_data'] ['page7'] ['Colour Blindness'] ['Complaints'] : 'Nil');
		$objWorkSheet->setCellValue('AP'.$cell_index, (isset($screening ["doc_data"] ['widget_data'] ['page7'] ['Colour Blindness'] ['Wearing Spectacles'])) ? $screening ["doc_data"] ['widget_data'] ['page7'] ['Colour Blindness'] ['Wearing Spectacles'] : 'Nil');
		$objWorkSheet->setCellValue('AQ'.$cell_index, (isset($screening ["doc_data"] ['widget_data'] ['page7'] ['Colour Blindness'] ['Subjective Refraction'])) ? $screening ["doc_data"] ['widget_data'] ['page7'] ['Colour Blindness'] ['Subjective Refraction'] : 'Nil');

		$objWorkSheet->setCellValue('AR'.$cell_index, (isset($screening ["doc_data"] ['widget_data'] ['page7'] ['Colour Blindness'] ['Ocular Diagnosis'])) ? $screening ["doc_data"] ['widget_data'] ['page7'] ['Colour Blindness'] ['Ocular Diagnosis'] : 'Nil');

		$objWorkSheet->setCellValue('AS'.$cell_index, (isset($screening ["doc_data"] ['widget_data'] ['page7'] ['Colour Blindness'] ['Referral Made'])) ? $screening ["doc_data"] ['widget_data'] ['page7'] ['Colour Blindness'] ['Referral Made'] : 'Nil');

		$objWorkSheet->setCellValue('AT'.$cell_index, (isset($screening ["doc_data"] ['widget_data'] ['page7'] ['Colour Blindness'] ['Description'])) ? $screening ["doc_data"] ['widget_data'] ['page7'] ['Colour Blindness'] ['Description'] : 'Nil');


		$objWorkSheet->setCellValue('AU'.$cell_index, (isset($screening ["doc_data"] ['widget_data'] ['page8'] [' Auditory Screening'] ['Right'])) ? $screening ["doc_data"] ['widget_data'] ['page8'] [' Auditory Screening'] ['Right'] : 'Nil');

		$objWorkSheet->setCellValue('AV'.$cell_index, (isset($screening ["doc_data"] ['widget_data'] ['page8'] [' Auditory Screening'] ['Left'])) ? $screening ["doc_data"] ['widget_data'] ['page8'] [' Auditory Screening'] ['Left'] : 'Nil');

		if(is_array($screening['doc_data']['widget_data']['page8'][' Auditory Screening']['Speech Screening'])){

		$objWorkSheet->setCellValue('AW'.$cell_index, (isset($screening['doc_data']['widget_data']['page8'][' Auditory Screening']['Speech Screening']) && ($screening['doc_data']['widget_data']['page8'][' Auditory Screening']['Speech Screening'] == !array())) ? implode(', ',  $screening['doc_data']['widget_data']['page8'][' Auditory Screening']['Speech Screening']) : 'Nil');
		}else
		{
			$objWorkSheet->setCellValue('AW'.$cell_index, 'Nil');
		}

		$objWorkSheet->setCellValue('AX'.$cell_index, (isset($screening['doc_data']['widget_data']['page8'][' Auditory Screening']['D D and disability'])) ? $screening['doc_data']['widget_data']['page8'][' Auditory Screening']['D D and disability'] : 'Nil');

		$objWorkSheet->setCellValue('AY'.$cell_index, (isset($screening ["doc_data"] ['widget_data'] ['page8'] [' Auditory Screening'] ['Description'])) ? $screening ["doc_data"] ['widget_data'] ['page8'] [' Auditory Screening'] ['Description'] : 'Nil');

		$objWorkSheet->setCellValue('AZ'.$cell_index, (isset($screening ["doc_data"] ['widget_data'] ['page8'] [' Auditory Screening'] ['Referral Made'])) ? $screening ["doc_data"] ['widget_data'] ['page8'] [' Auditory Screening'] ['Referral Made'] : 'Nil');
	
		$objWorkSheet->setCellValue('BA'.$cell_index, (isset($screening ["doc_data"] ['widget_data'] ['page9'] ['Dental Check-up'] ['Oral Hygiene'])) ? $screening ["doc_data"] ['widget_data'] ['page9'] ['Dental Check-up'] ['Oral Hygiene'] : 'Nil');

		$objWorkSheet->setCellValue('BB'.$cell_index, (isset($screening ["doc_data"] ['widget_data'] ['page9'] ['Dental Check-up'] ['Carious Teeth'])) ? $screening ["doc_data"] ['widget_data'] ['page9'] ['Dental Check-up'] ['Carious Teeth'] : 'Nil');

		$objWorkSheet->setCellValue('BC'.$cell_index, (isset($screening ["doc_data"] ['widget_data'] ['page9'] ['Dental Check-up'] ['Flourosis'])) ? $screening ["doc_data"] ['widget_data'] ['page9'] ['Dental Check-up'] ['Flourosis'] : 'Nil');

		$objWorkSheet->setCellValue('BD'.$cell_index, (isset($screening ["doc_data"] ['widget_data'] ['page9'] ['Dental Check-up'] ['Orthodontic Treatment'])) ? $screening ["doc_data"] ['widget_data'] ['page9'] ['Dental Check-up'] ['Orthodontic Treatment'] : 'Nil');

		$objWorkSheet->setCellValue('BE'.$cell_index, (isset($screening ["doc_data"] ['widget_data'] ['page9'] ['Dental Check-up'] ['Indication for extraction'])) ? $screening ["doc_data"] ['widget_data'] ['page9'] ['Dental Check-up'] ['Indication for extraction'] : 'Nil');

		$objWorkSheet->setCellValue('BF'.$cell_index, (isset($screening ["doc_data"] ['widget_data'] ['page9'] ['Dental Check-up'] ['Root Canal Treatment'])) ? $screening ["doc_data"] ['widget_data'] ['page9'] ['Dental Check-up'] ['Root Canal Treatment'] : 'Nil');

		$objWorkSheet->setCellValue('BG'.$cell_index, (isset($screening ["doc_data"] ['widget_data'] ['page9'] ['Dental Check-up'] ['CROWNS'])) ? $screening ["doc_data"] ['widget_data'] ['page9'] ['Dental Check-up'] ['CROWNS'] : 'Nil');

		$objWorkSheet->setCellValue('BH'.$cell_index, (isset($screening ["doc_data"] ['widget_data'] ['page9'] ['Dental Check-up'] ['Fixed Partial Denture'])) ? $screening ["doc_data"] ['widget_data'] ['page9'] ['Dental Check-up'] ['Fixed Partial Denture'] : 'Nil');

		$objWorkSheet->setCellValue('BI'.$cell_index, (isset($screening ["doc_data"] ['widget_data'] ['page9'] ['Dental Check-up'] ['Curettage'])) ? $screening ["doc_data"] ['widget_data'] ['page9'] ['Dental Check-up'] ['Curettage'] : 'Nil');

		$objWorkSheet->setCellValue('BJ'.$cell_index, (isset($screening ["doc_data"] ['widget_data'] ['page9'] ['Dental Check-up'] ['Estimated Amount'])) ? $screening ["doc_data"] ['widget_data'] ['page9'] ['Dental Check-up'] ['Estimated Amount'] : 'Nil');
		$objWorkSheet->setCellValue('BK'.$cell_index, (isset($screening ["doc_data"] ['widget_data'] ['page9'] ['Dental Check-up'] ['DC 11'])) ? $screening ["doc_data"] ['widget_data'] ['page9'] ['Dental Check-up'] ['DC 11'] : 'Nil');

		$objWorkSheet->setCellValue('BL'.$cell_index, (isset($screening ["doc_data"] ['widget_data'] ['page9'] ['Dental Check-up'] ['DC 11'])) ? $screening ["doc_data"] ['widget_data'] ['page9'] ['Dental Check-up'] ['DC 12'] : 'Nil');

		$objWorkSheet->setCellValue('BM'.$cell_index, (isset($screening ["doc_data"] ['widget_data'] ['page9'] ['Dental Check-up'] ['DC 11'])) ? $screening ["doc_data"] ['widget_data'] ['page9'] ['Dental Check-up'] ['DC 13'] : 'Nil');

		$objWorkSheet->setCellValue('BN'.$cell_index, (isset($screening ["doc_data"] ['widget_data'] ['page9'] ['Dental Check-up'] ['DC 11'])) ? $screening ["doc_data"] ['widget_data'] ['page9'] ['Dental Check-up'] ['DC 14'] : 'Nil');

		$objWorkSheet->setCellValue('BO'.$cell_index, (isset($screening ["doc_data"] ['widget_data'] ['page9'] ['Dental Check-up'] ['Referral Made'])) ? $screening ["doc_data"] ['widget_data'] ['page9'] ['Dental Check-up'] ['Referral Made'] : 'Nil');

		$objWorkSheet->setCellValue('BP'.$cell_index, (isset($screening ["doc_data"] ['widget_data'] ['page9'] ['Dental Check-up'] ['Result'])) ? $screening ["doc_data"] ['widget_data'] ['page9'] ['Dental Check-up'] ['Result'] : 'Nil');
	...
	$file_save = BASEDIR.TENANT.'/'.$today_date."-Screening-Report.xlsx";
	$file_name = URLCustomer.$today_date."-Screening-Report.xlsx";
	$objWriter->save($file_save);
	//$this->secure_file_download($file_name);
	//unlink($file_name);
	return $file_name;
...
public function get_excel_for_requests_span($district, $scl, $start, $end)
{
	...
	$screening_data = $this->ci->ttwreis_common_model->get_excel_for_requests_span($district, $scl, $start, $end);
...
public function get_excel_for_attendance_span($dist, $scl, $start, $end)
{
	...
	$screening_data = $this->ci->ttwreis_common_model->get_excel_for_attendance_span($dist, $scl, $start, $end);
...
public function get_excel_for_sanitation_span($dist, $scl, $start, $end)
{
	...
	$screening_data = $this->ci->ttwreis_common_model->get_excel_for_sanitation_span($dist, $scl, $start, $end);
...
public function get_excel_for_hb_span($district, $scl, $start, $end)
{
	...
	$objWorkSheet->setCellValue('G1', 'Blood Group')->getStyle('G1')->applyFromArray($styleArray);
	$objWorkSheet->setCellValue('H1', 'Latest Month')->getStyle('H1')->applyFromArray($styleArray);
	...
	$screening_data = $this->ci->ttwreis_common_model->get_excel_for_hb_span($district, $scl, $start, $end);
	...
	foreach ($screening_data as $screening) {
		...
		//$objWorkSheet->setCellValue('G'.$cell_index, $screening ["doc_data"]['widget_data']['page1']['Student Details'] ['Age']);

		$objWorkSheet->setCellValue('G'.$cell_index, isset($screening ["doc_data"]['widget_data']['page1']['Student Details']['bloodgroup']['field_ref']) ? $screening ["doc_data"]['widget_data']['page1']['Student Details']['bloodgroup']['field_ref'] : 'Nil');

		$objWorkSheet->setCellValue('H'.$cell_index, $screening ["doc_data"]['widget_data']['page1']['Student Details'] ['HB_latest']['hb']);

		$objWorkSheet->setCellValue('I'.$cell_index, $screening ["doc_data"]['widget_data']['page1']['Student Details'] ['HB_latest']['month']);		
...
public function get_excel_for_bmi_span($district, $scl, $start, $end)
{
	...
	$objPHPExcel->getProperties()->setCreator("Havik Healthcare Technologies Pvt. Ltd");
	$objPHPExcel->getProperties()->setLastModifiedBy("Yoga Narasimha Reddy");
	...
	$objWorkSheet->setCellValue('F1', 'Gender')->getStyle('F1')->applyFromArray($styleArray);
	//$objWorkSheet->setCellValue('G1', 'Age')->getStyle('G1')->applyFromArray($styleArray);
	$objWorkSheet->setCellValue('G1', 'Month')->getStyle('G1')->applyFromArray($styleArray);
	$objWorkSheet->setCellValue('H1', 'Height')->getStyle('H1')->applyFromArray($styleArray);
	$objWorkSheet->setCellValue('I1', 'Weight')->getStyle('I1')->applyFromArray($styleArray);
	$objWorkSheet->setCellValue('J1', 'BMI')->getStyle('J1')->applyFromArray($styleArray);
	...
	$screening_data = $this->ci->ttwreis_common_model->get_excel_for_bmi_span($district, $scl, $start, $end);
	...
	foreach ($screening_data as $screening) {
		...
		$objWorkSheet->setCellValue('G'.$cell_index, $screening ["doc_data"]['widget_data']['page1']['Student Details']['BMI_latest']['month']);

		$objWorkSheet->setCellValue('H'.$cell_index, $screening ["doc_data"]['widget_data']['page1']['Student Details']['BMI_latest']['height']);

		$objWorkSheet->setCellValue('I'.$cell_index, $screening ["doc_data"]['widget_data']['page1']['Student Details']['BMI_latest']['weight']);

		$objWorkSheet->setCellValue('J'.$cell_index, $screening ["doc_data"]['widget_data']['page1']['Student Details']['BMI_latest']['bmi']);
...
public function get_symptoms_monthly_tracking_excel($selectedSchool, $selectedDistrict, $selectedMonth)
			{
				$this->ci->load->library('excel');
				...
				//set Properties
				$objPHPExcel->getProperties()->setCreator("Havik Healthcare Technologies Pvt. Ltd");
				$objPHPExcel->getProperties()->setLastModifiedBy("Yoga Narasimha");
				$objPHPExcel->getProperties()->setTitle($selectedMonth.$title."Monthly Symptoms List");
				$objPHPExcel->getProperties()->setSubject($selectedMonth.$title."Monthly Symptoms List");
				
				//Add new sheet
				$objWorkSheet = $objPHPExcel->createSheet(0); 
				$objWorkSheet->setTitle("Monthly Health Track Symptoms");
				$objWorkSheet->getRowDimension(1)->setRowHeight(44);
				...
				$styleArray = array(
						...
						'fill' => array('type' => PHPExcel_Style_Fill::FILL_SOLID,
								'color' => array('rgb' => 'DCE6F1') ));
		//General
	    $objWorkSheet->setCellValue('A1', 'Problem Info')->getStyle('A1')->applyFromArray($styleArray);
	    $objWorkSheet->setCellValue('B1', 'Count')->getStyle('B1')->applyFromArray($styleArray);

	    $objWorkSheet->getColumnDimension("A")->setAutoSize(true);
	    $objWorkSheet->getColumnDimension("B")->setAutoSize(true);

	  
			$data = $this->ci->ttwreis_common_model->get_data($selectedMonth, $selectedDistrict, $selectedSchool);
			
			$cell_index = 2;

			foreach ($data as $screening) {
				
				$objWorkSheet->setCellValue('A'.$cell_index, $screening ['label']);
				$objWorkSheet->setCellValue('B'.$cell_index, $screening ['y']);
				$cell_index ++;
			}
			...
			$file_save = BASEDIR.TENANT.'/'.$selectedMonth."-Mothly-Symptoms-List.xlsx";
			$file_name = URLCustomer.$selectedMonth."-Mothly-Symptoms-List.xlsx";
			$objWriter->save($file_save);
			//$this->secure_file_download($file_name);
			//unlink($file_name);
			return $file_name;
	...
	public function get_excel_for_request_notes($start, $end)
{
	...
	$screening_data = $this->ci->ttwreis_common_model->get_excel_for_request_notes_data($start, $end);
			...
			if(count($check) == 0)
			{
				...
				for($j=2; $j<=count($arr_data)+2; $j++){					
					...
					if(!empty($arr_data[$j]['school name']) && $arr_data[$j]['school name'] != null && $arr_data[$j]['school name'] != ""){
						...
						$history['last_stage']['submitted_by'] = "ttwreis.user#gmail.com";
						$history['last_stage']['time'] = date("Y-m-d H:i:s");

						$this->ci->ttwreis_common_model->import_detailed_school_information_model($doc_data, $history);
			...
			if(count($check) == 0)
			{
				...
				for($j=2; $j<=count($arr_data)+2; $j++){	
					...
					if(!empty($arr_data[$j]['district']) && $arr_data[$j]['district'] != null && $arr_data[$j]['district'] != ""){
						...
						$this->ci->ttwreis_common_model->import_staff_covid_cases_model($doc_data, $history);

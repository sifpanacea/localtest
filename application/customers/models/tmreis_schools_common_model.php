...
class Tmreis_schools_common_model extends CI_Model {
	...
	function __construct() {
		...
		$this->sanitation_report_app_col_version_2 = "healthcare2017121175645993_version_2";
		//$this->bmi_app_col         = "healthcare2018213172422286";
		$this->bmi_app_col         = "healthcare201761916814158";
		//$this->hb_app_col = "tmreis_himglobin_report_col_XL_Import";
	...
	public function update_screening_collection($date, $screening_duration, $school_name) 
	{
		...
		$query = $this->mongo_db->orderBy(array('history.last_stage.time' => 1))->
					select(array('history.last_stage.time'))->where(array('doc_data.widget_data.page2.Personal Information.School Name'=> $school_name))->limit(1)->get($this->screening_app_col);
					
					if($query){
						
					foreach($query as $history)
					{
						$end_date = $history['history']['last_stage']['time'];
		...
		{ */
		for($init_date = $dates['today_date']; $init_date >= $end_date;) 
		{ 
		...
		if ($query) {
			...
			if(count($query_request) > 0){
				foreach($query_request as $req_ind => $req){
					...
					$notes_data = $this->mongo_db->where ("req_doc_id", $req['doc_properties']['doc_id'])->get ( 'tmreis_req_notes' );
		...
		if ($date) 
{
	...
	{
	  $query = $this->mongo_db->select(array('app_template'))->where('_id','healthcare2017121175645993')->get($this->collections['records']);
	...
	public function get_sanitation_report_pie_data($date, $search_criteria, $opt) {
	 ...
	 $query = $this->mongo_db->select(array('doc_data.widget_data'),array())->where(array('doc_data.widget_data.page4.Declaration Information.Date:'=>$date,$search_criteria=>$opt))->get('healthcare2017121175645993');
	...
	{
		$qry = $this->mongo_db->where('_id', new MongoId($district_id))->get('tmreis_district');
		if($qry)
		{
			return $qry;
		}
	...
	public function get_drill_down_to_bmi_report($type, $current_month,  $school_name) 
	{
		...
		switch ($type) {
			case "UNDER WEIGHT" :
				...
				$query = $this->mongo_db->select ( array (
						"doc_data" 
				) )->whereLt('doc_data.widget_data.page1.Student Details.BMI_values.bmi',"18.50")->whereLike ( 'doc_data.widget_data.page1.Student Details.BMI_values.month',$selected_month)->where("doc_data.school_details.School Name",$school_name)->get ( $this->bmi_app_col );
	...
	*/ 
	...
	public function fetch_student_info_model($school_name, $unique_id)
	{
		...
		$res = $this->mongo_db->select(array('doc_data.widget_data'))->where(array('doc_data.widget_data.page2.Personal Information.School Name' => $school_name, 'doc_data.widget_data.page1.Personal Information.Hospital Unique ID'=> $unique_id))->get('healthcare201672020159570');
	...
	function tmreis_update_hb_values($month,$monthly_hb,$unique_id)
	{
	 $check_query = array('doc_data.widget_data.page1.Student Details.Hospital Unique ID'=> $unique_id,"doc_data.widget_data.page1.Student Details.HB_values"=>array('$elemMatch'=>array("month"=>$month)));
	...
	*/
		...
		public function get_attendance_report_daily_count($school_name,$date)
		{
			...
			))->where(array('doc_data.widget_data.page1.Attendence Details.Select School' => $school_name))->WhereLike('history.last_stage.time', $today_date)->get('healthcare2017120192713965');
			if($query)
			{
...
/*  HS Daily Requests Count to show on table ================ By Harish ============   */
		...
		public function get_requests_daily_count($district_school_code,$date)
		{
			...
			))->count('healthcare201610114435690_static_html');
 ...
 /* HS Daily Requests information to show on modal */ 
	...
	$normal_requests_info = $this->mongo_db->select($selected_fields)->whereLike(
				'doc_data.widget_data.page1.Student Info.Unique ID',$district_school_code)
			   ->WhereLike('history.time', $today_date)
			  ->where(array('doc_data.widget_data.page2.Review Info.Request Type' => "Normal"))->get('healthcare201610114435690_static_html');
	...
	$emergency_requests_info = $this->mongo_db->select($selected_fields)->whereLike('doc_data.widget_data.page1.Student Info.Unique ID',$district_school_code)
			   ->WhereLike('history.time', $today_date)
			   ->where(array('doc_data.widget_data.page2.Review Info.Request Type' => "Emergency"))->get('healthcare201610114435690_static_html');
	...
	$chronic_requests_info = $this->mongo_db->select($selected_fields)->whereLike('doc_data.widget_data.page1.Student Info.Unique ID',$district_school_code)
			   ->WhereLike('history.time', $today_date)
			   ->where(array('doc_data.widget_data.page2.Review Info.Request Type' => "Chronic"))->get('healthcare201610114435690_static_html');
	...
	public function get_docs_daily_count($district_school_code, $date)
		{
			...
			$doc_normal_requests = $this->mongo_db->whereLike(
              ...
              ->where(array('doc_data.widget_data.page2.Review Info.Request Type' => "Normal"))->count('healthcare201610114435690_static_html');
...
/*  HS Daily Sanitation count to show on Table */
		...
		public function get_sanitation_report_daily_count($school_name,$date)
		{
				...
				))->where(array('doc_data.widget_data.page4.School Information.School Name' => $school_name))->WhereLike('history.last_stage.time', $today_date)->get('healthcare2017121175645993');
	...
	public function get_bmi_report_model_count($current_month, $school_name) {
		...
		if ($current_month) {
			...
			$selected_month = $this->selected_month;
				...
				$under_weight = $this->mongo_db->where('doc_data.school_details.School Name',$school_name)->whereLt('doc_data.widget_data.page1.Student Details.BMI_values.bmi',18.50) ->whereLike('doc_data.widget_data.page1.Student Details.BMI_values.month',$selected_month)->count('healthcare201761916814158');
				
				$normal_weight = $this->mongo_db->where('doc_data.school_details.School Name',$school_name)->whereBetween('doc_data.widget_data.page1.Student Details.BMI_values.bmi',18.50,24.99)->whereLike('doc_data.widget_data.page1.Student Details.BMI_values.month',$selected_month)->count('healthcare201761916814158');
				
				$over_weight = $this->mongo_db->where('doc_data.school_details.School Name',$school_name)->whereBetween('doc_data.widget_data.page1.Student Details.BMI_values.bmi',25.00,29.99)->whereLike('doc_data.widget_data.page1.Student Details.BMI_values.month',$selected_month)->count('healthcare201761916814158');

				$obese = $this->mongo_db->where('doc_data.school_details.School Name',$school_name)->wheregte('doc_data.widget_data.page1.Student Details.BMI_values.bmi',30.0)->whereLike('doc_data.widget_data.page1.Student Details.BMI_values.month',$selected_month)->count('healthcare201761916814158');
	...
	public function get_bmi_report_model_info($current_month, $school_name) {
		...
		if ($current_month) {
			...
			$selected_month = $this->selected_month;
				...
				$under_weight_info = $this->mongo_db->select(array('doc_data.widget_data.page1.Student Details'))->where($under_weight_array)->whereLt('doc_data.widget_data.page1.Student Details.BMI_values.bmi',18.49)->get('healthcare201761916814158');
			    ...
			    $normal_weight_info = $this->mongo_db->select(array('doc_data.widget_data.page1.Student Details'))->where($normal_weight_array)->whereBetween('doc_data.widget_data.page1.Student Details.BMI_values.bmi',18.50,24.99)->get('healthcare201761916814158');
				...
				$over_weight_info = $this->mongo_db->select(array('doc_data.widget_data.page1.Student Details'))->where($normal_weight_array)->whereBetween('doc_data.widget_data.page1.Student Details.BMI_values.bmi',25.00,29.99)->get('healthcare201761916814158');
				...
				$obese_info = $this->mongo_db->select(array('doc_data.widget_data.page1.Student Details'))->where($obese_weight_array)->wheregte('doc_data.widget_data.page1.Student Details.BMI_values.bmi',30.0)->get('healthcare201761916814158');
...
/* Severe Type Anamea */			
			...
			$severe_anamia = $this->mongo_db->where(array('doc_data.school_details.School Name' => $school_name,'doc_data.widget_data.page1.Student Details.HB_values.month' => $selected_month))->whereLt('doc_data.widget_data.page1.Student Details.HB_values.hb',8)->count($this->hb_app_col);
...
/* Moderate Type Anamea */	
				...
				$moderate_anamia = $this->mongo_db->where(array('doc_data.school_details.School Name' => $school_name,'doc_data.widget_data.page1.Student Details.HB_values.month' => $selected_month))->whereBetween('doc_data.widget_data.page1.Student Details.HB_values.hb',8,10)->count($this->hb_app_col);
...
/* Mild Type Anamea */	
				...
				$mild_anamia = $this->mongo_db->where(array('doc_data.school_details.School Name' => $school_name,'doc_data.widget_data.page1.Student Details.HB_values.month' => $selected_month))->whereBetween('doc_data.widget_data.page1.Student Details.HB_values.hb',10,12)->count($this->hb_app_col);
...
/* Normal Type Anamea */	
			...
			/* 	$normal_scl = array('doc_data.widget_data.school_details.School Name' => $school_name);
				...
				$normal_anamia = $this->mongo_db->where(array('doc_data.school_details.School Name' => $school_name,'doc_data.widget_data.page1.Student Details.HB_values.month' => $selected_month))->whereGt('doc_data.widget_data.page1.Student Details.HB_values.hb',12.1)->count($this->hb_app_col);
...
/* Severe Type Anamea */			
				...
				$severe_anamia_info = $this->mongo_db->where(array('doc_data.school_details.School Name' => $school_name,'doc_data.widget_data.page1.Student Details.HB_values.month' => $selected_month))->whereLt('doc_data.widget_data.page1.Student Details.HB_values.hb',8)->get($this->hb_app_col);
...
/* Moderate Type Anamea */	
				/* $moderate_gt = array('doc_data.widget_data.page1.Student Details.HB_values.hb' => array('$gt' => 9));
					$moderate_lt = array('doc_data.widget_data.page1.Student Details.HB_values.hb' => array('$lt' => 10));
					$sclname = array('doc_data.school_details.School Name' => $school_name);
					$slct_mnth = array('doc_data.widget_data.page1.Student Details.HB_values.month' => $selected_month);		
					$moderate_anamia_info = $this->mongo_db->where($moderate_gt,$moderate_lt,$sclname,$slct_mnth)->get($this->hb_app_col); */
					
				$moderate_anamia_info = $this->mongo_db->where(array('doc_data.school_details.School Name' => $school_name,'doc_data.widget_data.page1.Student Details.HB_values.month' => $selected_month))->whereBetween('doc_data.widget_data.page1.Student Details.HB_values.hb',8,10)->get($this->hb_app_col);
...
/* Mild Type Anamea */	
				...
				$school_name = array(''doc_data.school_details.School Name' => $school_name);
	  			$selected_month = array('doc_data.widget_data.page1.Student Details.HB_values.month' => $selected_month);
				
				$mild_anamia_info = $this->mongo_db->where($mild_gt,$mild_lt,$school_name,$selected_month)->get($this->hb_app_col); */
				
				$mild_anamia_info = $this->mongo_db->where(array('doc_data.school_details.School Name' => $school_name,'doc_data.widget_data.page1.Student Details.HB_values.month' => $selected_month))->whereBetween('doc_data.widget_data.page1.Student Details.HB_values.hb',10,12)->get($this->hb_app_col);


				
/* Normal Type Anamea */	

				/* $normal_scl = array(''doc_data.school_details.School Name' => $school_name);
				...
				$normal_anamia_info = $this->mongo_db->where(array('doc_data.school_details.School Name' => $school_name,'doc_data.widget_data.page1.Student Details.HB_values.month' => $selected_month))->whereGt('doc_data.widget_data.page1.Student Details.HB_values.hb',12.1)->get($this->hb_app_col);
	...
	public function get_sanitation_report_data_with_date_version_2($date,$school_name)
	{
		...
		$this->mongo_db->whereLike('doc_data.widget_data.page4.Declaration Information.Date:',$selected_date)->where(array('doc_data.widget_data.page4.School Information.School Name'=>$school_name));
		$query = $this->mongo_db->get('healthcare2017121175645993_version_2');
		if($query)
			return $query;
		else
			return FALSE;
	...
	public function update_principal_hs_profile($data,$schoolCode)
	{
		//echo print_r($data,TRUE);exit();
			...
			$this->mongo_db->switchDatabase($this->common_db['common_db']);
			$result = $this->mongo_db->where('school_code',$schoolCode)->set($update_profile_hs)->update('tmreis_health_supervisors');
			$this->mongo_db->switchDatabase($this->common_db['dsn']);
			...
			$this->mongo_db->switchDatabase($this->common_db['common_db']);
			$result = $this->mongo_db->where('school_code',$schoolCode)->set($update_profile_principal)->update('tmreis_schools');
			$this->mongo_db->switchDatabase($this->common_db['dsn']);
	...
	public function fetch_student_information_model($school_name,$unique_id)
	{
		...
		$res = $this->mongo_db->select(array('doc_data.widget_data.page1','doc_data.widget_data.page2'))->where(array('doc_data.widget_data.page2.Personal Information.School Name' => $school_name, 'doc_data.widget_data.page1.Personal Information.Hospital Unique ID'=> $unique_id))->get('healthcare201672020159570');
	...
	public function initiate_request_model($doc_data,$doc_properties,$app_properties,$array_history)
	{
		...
		$res = $this->mongo_db->insert('healthcare201610114435690_static_html',$doc_data);
	...
	public function get_hs_req_normal($usercollection,$unique_id)
    {
        ...
        $pipeline = [ 
           ...
           exit();
            return $result;*/
          $query_normal = array('app_properties.status'=>'new','app_properties.app_id'=>'healthcare201610114435690','doc_data.widget_data.page2.Review Info.Request Type' => 'Normal','doc_data.widget_data.page1.Student Info.Unique ID' => array(
          				'$regex' => $unique_id));
     ...
     public function get_hs_req_emergency($usercollection,$unique_id)
    {
    	...
    	$query_emergency = array('app_properties.status'=>'new','app_properties.app_id'=>'healthcare201610114435690','doc_data.widget_data.page2.Review Info.Request Type' => 'Emergency','doc_data.widget_data.page1.Student Info.Unique ID' => array(
          				'$regex' => $unique_id));
        $this->mongo_db->orderBy(array('history.0.time' => -1));
        $query=$this->mongo_db->select(array(),array('_id'))->limit(200)->
    ...
    {
    	$query_chronic = array('app_properties.status'=>'new','app_properties.app_id'=>'healthcare201610114435690','doc_data.widget_data.page2.Review Info.Request Type' => 'Chronic','doc_data.widget_data.page1.Student Info.Unique ID' => array(
          				'$regex' => $unique_id));
        $this->mongo_db->orderBy(array('history.0.time' => -1));
        $query=$this->mongo_db->select(array(),array('_id'))->limit(200)->
	...
	{
		$query = $this->mongo_db->select(array(),array('_id'))->where(array('doc_properties.doc_id' => $doc_id))->get('healthcare201610114435690_static_html');
	...
	public function get_students_load_ehr_doc_model($_id) {
		...
		if ($query) {
			$query_request = $this->mongo_db->where ( "doc_data.widget_data.page1.Student Info.Unique ID", $query [0] ["doc_data"] ['widget_data'] ['page1'] ['Personal Information'] ['Hospital Unique ID'] )->get ( 'healthcare201610114435690_static_html' );
			
			$bmi_value_table_addto_ehr = $this->mongo_db->where("doc_data.widget_data.page1.Student Details.Hospital Unique ID", $query [0] ["doc_data"] ['widget_data'] ['page1'] ['Personal Information'] ['Hospital Unique ID'])->get('healthcare201761916814158');
	...
	public function add_student_BMI_model($doc_data, $school_details, $doc_properties, $app_properties, $history)
	{
		  $doc_data = array("doc_data"=>array("widget_data"=>$doc_data, "school_details"=>$school_details), "doc_properties" => $doc_properties, "app_properties" => $app_properties, "history" => $history);

		  $query = $this->mongo_db->insert($this->bmi_app_col,$doc_data);
		  if($query)
			  return TRUE;
		  else
			  return FALSE;
	...
	function tmreis_update_bmi_values($month,$monthly_bmi,$unique_id)
	{
	...
	{
		$is_exists = $this->mongo_db->where(array('doc_data.widget_data.page1.Student Details.Hospital Unique ID'=> $unique_id))->get('healthcare201761916814158');
	...
	function update_bmi_values($month,$monthly_bmi,$unique_id)
	{
		  ...
		  $is_already_updated = $this->mongo_db->where($check_query)->get('healthcare201761916814158');
		  ...
		  if($is_already_updated)
		  {
			...
			$response = $this->mongo_db->command(array( 
			'findAndModify' => 'healthcare201761916814158',
			'query'         => $query,
			'update'        => $update
			 ));
			...
			'update'        => $update_main
			 ...
			 $after_update = $this->mongo_db->where(array('doc_data.widget_data.page1.Student Details.Hospital Unique ID'=> $unique_id))->set(array('doc_data.widget_data.page1.Student Details.Height cms'=>$monthly_bmi['height'],
										...
										'doc_data.widget_data.page1.Student Details.BMI'=>$monthly_bmi['bmi'],
										'doc_data.widget_data.page1.Student Details.Date'=>$ndate))
										->update('healthcare201761916814158');
				
		  $after_update = $this->mongo_db->where(array('doc_data.widget_data.page1.Student Details.Hospital Unique ID'=> $unique_id))->push('doc_data.widget_data.page1.Student Details.BMI_values',$monthly_bmi)->update('healthcare201761916814158');
    ...
    {
        $query = $this->mongo_db->select(array(),array('_id'))->where(array('doc_data.widget_data.page1.Student Info.Unique ID' => $unique_id,'doc_properties.doc_id' => $doc_id))->get('healthcare201610114435690_static_html');
       /* echo "<pre>";
    ...
    function get_approval_history($doc_id)
    {
    	...
    	$query = $this->mongo_db->select(array('history'))->where(array('doc_properties.doc_id'=> $doc_id))->get('healthcare201610114435690_static_html');
	...
	{	
		...
		$update_query = $this->mongo_db->where(array('doc_data.widget_data.page1.Student Info.Unique ID' => $unique_id, 'doc_properties.doc_id' => $doc_id))->set(array('doc_data' =>$doc_data,'history'=>$history_array))->update('healthcare201610114435690_static_html');
		...
		$this->mongo_db->insert('healthcare201610114435690_static_html_shadow',$doc_data);
	...
	function create_chronic_case_new($student_unique_id,$request_type,$chronic_disease,$disease_desc,$schoolName)
	{
		...
		$query = $this->mongo_db->insert('tmreis_chronic_cases', $data);
	...
	public function get_hb_report_model($current_month, $school_name) {
		...
		$requests = [ ];
				...
				$sevier = $this->mongo_db->where('doc_data.widget_data.school_details.School Name',$school_name)->whereLt('doc_data.widget_data.page1.Student Details.HB_values.hb',8) ->whereLike('doc_data.widget_data.page1.Student Details.HB_values.month',$selected_month)->get('tmreis_himglobin_report_col');
				$request ['label'] = 'Sevier';
				$request ['value'] = count($sevier);
				array_push ( $requests, $request );
				
				$normal = $this->mongo_db->where('doc_data.widget_data.school_details.School Name',$school_name)->whereBetween('doc_data.widget_data.page1.Student Details.HB_values.hb',12,18)->whereLike('doc_data.widget_data.page1.Student Details.HB_values.month',$selected_month)->get('tmreis_himglobin_report_col');
				$request ['label'] = 'NORMAL';
				$request ['value'] = count($normal);
				array_push ( $requests, $request );
				
				$moderate = $this->mongo_db->where('doc_data.widget_data.school_details.School Name',$school_name)->whereBetween('doc_data.widget_data.page1.Student Details.HB_values.hb',8.1,10)->whereLike('doc_data.widget_data.page1.Student Details.HB_values.month',$selected_month)->get('tmreis_himglobin_report_col');
				$request ['label'] = 'Moderate';
				$request ['value'] = count($moderate);
				array_push ( $requests, $request );
				
				$mild = $this->mongo_db->where('doc_data.widget_data.school_details.School Name',$school_name)->whereBetween('doc_data.widget_data.page1.Student Details.HB_values.hb',10.1,12)->whereLike('doc_data.widget_data.page1.Student Details.HB_values.month',$selected_month)->get('tmreis_himglobin_report_col');
				$request ['label'] = 'Mild';
		...
		switch ($type) {
			...
			ini_set ( 'memory_limit', '10G' );
				...
				if ($school_name == "All") {
					...
					foreach ( $query as $doc ) {
						//doc_data.widget_data.school_details
						if (strtolower ( $doc ['doc_data']['widget_data']['school_details']['School Name'] ) == strtolower ( $school_name )) {
							array_push ( $doc_query, $doc );
						}
					}
		...
		public function get_doctor_visiting_report_date_wise($school_name)
    {
          ...
          $response = $this->mongo_db->command(array('distinct' => "tmreis_doctor_visiting_reports" ,'key' => "doc_data.widget_data.Student Details.doctor_visiting_date",'query'=>$query));


          foreach ($response['values'] as $dates) {
            
             $getSubmittedDocs = $this->mongo_db->where(array('doc_data.widget_data.Student Details.doctor_visiting_date' =>$dates,"doc_data.widget_data.school_details.School Name"=>$school_name))->count('tmreis_doctor_visiting_reports');
    ...
    {
        $is_exists = $this->mongo_db->where(array('doc_data.widget_data.Student Details.Hospital Unique ID'=> $unique_id))->get('tmreis_doctor_visiting_reports');
        if($is_exists)
        {
            return TRUE;
        }
    ...
    public function fetch_student_attachments($school_name, $unique_id)
    {
        ...
        $res  = $this->mongo_db->select(array("doc_data.widget_data.Student Details.Hospital Unique ID","doc_properties.doc_id","doc_attachments"))->where(array('doc_data.widget_data.Student Details.Hospital Unique ID' =>$unique_id,"doc_data.widget_data.school_details.School Name"=>$school_name))->get('tmreis_doctor_visiting_reports');
    ...
    public function submit_doctor_visiting_report($doc_data, $doc_attachments,  $doc_properties, $app_properties, $history)
    {
          ...
          $query = $this->mongo_db->insert('tmreis_doctor_visiting_reports',$doc_data);
          if($query)
              return TRUE;
          else
              return FALSE;
    ...
    {
        $query = $this->mongo_db->select(array(),array('_id'))->where(array('doc_data.widget_data.Student Details.Hospital Unique ID' => $unique_id,'doc_properties.doc_id' => $doc_id))->get('tmreis_doctor_visiting_reports');
    ...
    {   
        ...
        $update_query = $this->mongo_db->where(array('doc_data.widget_data.Student Details.Hospital Unique ID' => $uniqueId, 'doc_properties.doc_id' => $doc_id))->set(array('doc_attachments' =>$doc_attachments))->update('tmreis_doctor_visiting_reports');
    ...
    public function drill_down_to_doctor_treated_list($school_name,$selectedDate)
    {
             ...
             $getSubmittedDocs = $this->mongo_db->where(array('doc_data.widget_data.Student Details.doctor_visiting_date' =>$selectedDate,"doc_data.widget_data.school_details.School Name"=>$school_name))->get('tmreis_doctor_visiting_reports');
    ...
    public function show_doctor_treated_student(/*$student_id,$doctor_visit_date*/ $doc_id)
    {
             ...
             $getSubmittedDocs = $this->mongo_db->select(array(),array('_id'))->where(array('doc_properties.doc_id'=>$doc_id))->get('tmreis_doctor_visiting_reports');

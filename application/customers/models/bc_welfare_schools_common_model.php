...
class Bc_welfare_schools_common_model extends CI_Model {
	/**
	...
	function __construct() {
		...
		$this->screening_app_col = 'healthcare201812217594045';
		$this->screening_app_col_2020_21 = 'bcwelfare_screening_report_col_2020-2021';
		$this->screening_staff_app_col = 'healthcare201812217594045_staff_bc_welfare_emp';
		$this->screening_app_col_screening = $email."_pie_analytics";
		
		$this->absent_app_col = "healthcare2018130121531948";
		$this->request_app_col = "healthcare2018122191146894";
		$this->request_app_col_static_html = "healthcare2018122191146894_static_html"; /* Current using collection for Requests */
		$this->sanitation_infra_app_col  = "healthcare2018213174639148";
		$this->sanitation_report_app_col = "healthcare201822113134483";
		$this->bmi_app_col = "healthcare2018213172422286";
		/*$this->bmi_app_col_xl_import = "healthcare2018213172422286_XL_Import";*/
		$this->hb_app_col = "bc_welfare_himglobin_report_col";
		/*$this->hb_app_col_xl_import = "bc_welfare_himglobin_report_col_XL_Import";*/
		$this->notes_col = "bc_welfare_ehr_notes";
	...
	private function screening_pie_data_for_stage3($dates,$school_name) 
	{
		...
		$overweight_array = array (
				array("doc_data.widget_data.page5.Doctor Check Up.Deficencies" => array (
						'$in' => array (
								"Over Weight" 
						) 
				    )
	...
	public function statescount() {
		$count = $this->mongo_db->count ( 'bc_welfare_states' );
		return $count;
	}
	public function get_states($per_page, $page) {
		$offset = $per_page * ($page - 1);
		$query = $this->mongo_db->limit ( $per_page )->offset ( $offset )->get ( 'bc_welfare_states' );
		return $query;
	}
	public function get_all_states() {
		$query = $this->mongo_db->get ( 'bc_welfare_states' );
		return $query;
	}
	
	// =================================================
	public function distcount() {
		$count = $this->mongo_db->count ( 'bc_welfare_district' );
		return $count;
	}
	...
	public function health_supervisorscount() {
		$this->mongo_db->switchDatabase ( $this->common_db ['common_db'] );
		$count = $this->mongo_db->count ( $this->collections ['bc_welfare_health_supervisors'] );
		$this->mongo_db->switchDatabase ( $this->common_db ['dsn'] );
		return $count;
	}
	public function get_health_supervisors($per_page, $page) {
		$offset = $per_page * ($page - 1);
		$this->mongo_db->switchDatabase ( $this->common_db ['common_db'] );
		$query = $this->mongo_db->limit ( $per_page )->offset ( $offset )->get ( $this->collections ['bc_welfare_health_supervisors'] );
		$this->mongo_db->switchDatabase ( $this->common_db ['dsn'] );
		return $query;
	}
	public function create_health_supervisors($post) {
		...
		$this->mongo_db->switchDatabase ( $this->common_db ['common_db'] );
		$query = $this->mongo_db->insert ( $this->collections ['bc_welfare_health_supervisors'], $data );
		$this->mongo_db->switchDatabase ( $this->common_db ['dsn'] );
	...
	public function cc_users_count() {
		$this->mongo_db->switchDatabase ( $this->common_db ['common_db'] );
		$count = $this->mongo_db->count ( $this->collections ['bc_welfare_cc'] );
		$this->mongo_db->switchDatabase ( $this->common_db ['dsn'] );
		return $count;
	}
	public function get_cc_users($per_page, $page) {
		$offset = $per_page * ($page - 1);
		$this->mongo_db->switchDatabase ( $this->common_db ['common_db'] );
		$query = $this->mongo_db->limit ( $per_page )->offset ( $offset )->get ( $this->collections ['bc_welfare_cc'] );
		$this->mongo_db->switchDatabase ( $this->common_db ['dsn'] );
		return $query;
	}
	public function create_cc_user($post) {
		...
		$this->mongo_db->switchDatabase ( $this->common_db ['common_db'] );
		$query = $this->mongo_db->insert ( $this->collections ['bc_welfare_cc'], $data );
		$this->mongo_db->switchDatabase ( $this->common_db ['dsn'] );
		// Return new document _id or FALSE on failure
		return isset ( $query ) ? $query : FALSE;
	}
	public function delete_cc_user($cc_id) {
		...
		) )->delete ( $this->collections ['bc_welfare_cc'] );
		$this->mongo_db->switchDatabase ( $this->common_db ['dsn'] );
		return $query;
	}
	...
	public function schoolscount() {
		$this->mongo_db->switchDatabase ( $this->common_db ['common_db'] );
		$count = $this->mongo_db->count ( $this->collections ['bc_welfare_schools'] );
		$this->mongo_db->switchDatabase ( $this->common_db ['dsn'] );
		return $count;
	}
	public function get_schools($per_page, $page) {
		$offset = $per_page * ($page - 1);
		$this->mongo_db->switchDatabase ( $this->common_db ['common_db'] );
		$query = $this->mongo_db->limit ( $per_page )->offset ( $offset )->get ( $this->collections ['bc_welfare_schools'] );
		$this->mongo_db->switchDatabase ( $this->common_db ['dsn'] );
	...
	public function create_school($post) {
		...
		$this->mongo_db->switchDatabase ( $this->common_db ['common_db'] );
		$query = $this->mongo_db->insert ( $this->collections ['bc_welfare_schools'], $data );
		$this->mongo_db->switchDatabase ( $this->common_db ['dsn'] );
		// Return new document _id or FALSE on failure
		return isset ( $query ) ? $query : FALSE;
	}
	...
	public function classescount($school_code) {
		$count = $this->mongo_db->where('school_code',$school_code)->count ($this->collections['bc_welfare_classes']);
		return $count;
	}
	...
	{
		$offset = $per_page * ($page - 1);
		$query = $this->mongo_db->limit ( $per_page )->offset ( $offset )->where('school_code',$school_code)->get ( $this->collections['bc_welfare_classes']);
		return $query;
	}
	
	public function sectionscount($school_code) {
		$count = $this->mongo_db->where('school_code',$school_code)->count ($this->collections['bc_welfare_sections']);
		return $count;
	}
	public function get_sections($per_page, $page, $school_code) {
		$offset = $per_page * ($page - 1);
		$query = $this->mongo_db->limit ( $per_page )->offset ( $offset )->where('school_code',$school_code)->get ( $this->collections['bc_welfare_sections']);
		return $query;
	}
		...
		if ($query) {
			...
			if(count($query_request) > 0){
				foreach($query_request as $req_ind => $req){
					...
					$notes_data = $this->mongo_db->where ("req_doc_id", $req['doc_properties']['doc_id'])->get ( 'bc_welfare_req_notes' );
			...
			if(isset($query[0]['doc_data']['widget_data']['page2']['Personal Information']['School Name'])){
				$this->mongo_db->switchDatabase ( $this->common_db ['common_db'] );
				$school_details = $this->mongo_db->where ( "school_name", $query[0]['doc_data']['widget_data']['page2']['Personal Information']['School Name'] )->get ( $this->collections ['bc_welfare_schools'] );
				
				if(count($school_details) > 0){
					$query_hs = $this->mongo_db->where ( "school_code", $school_details[0]['school_code'] )->get ( $this->collections ['bc_welfare_health_supervisors'] );
				}else{
					$query_hs[0] = false;
				}
				$this->mongo_db->switchDatabase ( $this->common_db ['dsn'] );
	...
	public function create_diagnostic($post) {
		...
		$query = $this->mongo_db->insert ( 'bc_welfare_diagnostics', $data );
		return $query;
	}
	public function create_hospital($post) {
		...
		$query = $this->mongo_db->insert ( 'bc_welfare_hospitals', $data );
		return $query;
	}
	...
	public function hospitalscount() {
		$count = $this->mongo_db->count ( 'bc_welfare_hospitals' );
		return $count;
	}
	public function get_hospitals($per_page, $page) {
		$offset = $per_page * ($page - 1);
		$query = $this->mongo_db->limit ( $per_page )->offset ( $offset )->get ( 'bc_welfare_hospitals' );
		foreach ( $query as $hospitals => $hospital ) {
			$dt_name = $this->mongo_db->where ( '_id', new MongoId ( $hospital ['dt_name'] ) )->get ( 'bc_welfare_district' );
	...
	public function diagnosticscount() {
		$count = $this->mongo_db->count ( 'bc_welfare_diagnostics' );
		return $count;
	}
	public function get_diagnostics($per_page, $page) {
		$offset = $per_page * ($page - 1);
		$query = $this->mongo_db->limit ( $per_page )->offset ( $offset )->get ( 'bc_welfare_diagnostics' );
		foreach ( $query as $diagnostics => $dia ) {
			$dt_name = $this->mongo_db->where ( '_id', new MongoId ( $dia ['dt_name'] ) )->get ( 'bc_welfare_district' );
	...
	public function empcount() {
		$count = $this->mongo_db->count ( 'bc_welfare_emp' );
		return $count;
	}
	public function get_emp($per_page, $page) {
		$offset = $per_page * ($page - 1);
		$query = $this->mongo_db->limit ( $per_page )->offset ( $offset )->get ( 'bc_welfare_emp' );
		return $query;
	}
	...
	public function get_attendance_submitted_school_name($date = FALSE,$school_name)
	{
		...
		log_message("debug","query====================3190".print_r($query,true));
		return $query;
	...
	public function get_all_absent_data($date = FALSE,$school_name) {
		...
		$request ['value'] = $r2h;
		array_push ( $requests, $request );
		log_message("debug","requests====================3235".print_r($requests,true));
		return $requests;
	}
	...
	public function get_all_symptoms_docs($start_date, $end_date, $unique_id_pattern,$id_for_school = false) {
		...
		foreach ( $query as $doc ) {
			...
			foreach ( $doc ['history'] as $date ) {
				$time = $date ['time'];
				log_message('debug','$timeeeeeeeeeeeee=====3890====='.print_r($time,true));
				log_message('debug','$start_date=====3891====='.print_r($start_date,true));
				log_message('debug','$end_date=====3892====='.print_r($end_date,true));
		...
		$query = $result;
		log_message('debug','get_all_symptoms_docs=============3901'.print_r(count($query),true));
		log_message('debug','get_all_symptoms_docs=============3901'.print_r($query,true));
		return $query;
	}
	...
	public function get_all_requests_docs($start_date, $end_date, $type = false, $school_name, $unique_id_pattern) 
	{
		...
		{
			//$type = "Cured";
			//$whereArray = array('doc_data.widget_data.page2.Review Info.Status' => $type);
			...
			log_message('debug','get_all_requests_docs====4058==>'.print_r($query,true));
		}
		...
		$query = $result;
		log_message('debug','query_requests_docs====4135==>'.print_r($query,true));
		return $query;
	}
	...
	public function get_drilling_request_students($type, $date = false, $request_duration = "Monthly",$school_name,$id_pattern) 
	{
		...
		if ($type == "Device Initiated") 
		{
			...
			/* $query_temp = $this->get_all_requests_docs ( $dates ['today_date'], $dates ['end_date'], "Device Initiated", $school_name, $id_pattern);
			//echo print_r($query_temp,true);
			//exit();
			$query = [ ];
			foreach ( $query_temp as $report ) {
				if (isset ( $report ['history'] [0] ['submitted_user_type'] )) {
		...
		{
			/* $query_temp = $this->get_all_requests_docs ( $dates ['today_date'], $dates ['end_date'], "Web Initiated",$school_name, $id_pattern);
	...
	public function get_schools_by_dist_id($dist_id) {
		if ($dist_id == "All") {
			...
			$query = $this->mongo_db->select ( array (
					...
					'school_name' => 1 
			) )->where ( 'dt_name', $dist_id )->get ( $this->collections ['bc_welfare_schools'] );
			$this->mongo_db->switchDatabase ( $this->common_db ['dsn'] );
			return $query;
		}
	}
	...
	public function get_school_data_school_name($school_name) {
		...
		$query = $this->mongo_db->select ( array (
				...
				'school_name' => 1 
		) )->where ( 'school_name', $school_name )->get ( $this->collections ['bc_welfare_schools'] );
		$this->mongo_db->switchDatabase ( $this->common_db ['dsn'] );
		return $query;
	}
	...
	public function messaging($message)
	{
		...
		$response = $this->data = $this->bc_welfare_common_model->messaging($message);
		//$this->data = "";
	
		$this->output->set_output($response);
	}
	public function groupscount() {
		$count = $this->mongo_db->count ( 'bc_welfare_chat_groups' );
		return $count;
	}
	public function get_groups($per_page, $page) {
		$offset = $per_page * ($page - 1);
		$query = $this->mongo_db->limit ( $per_page )->offset ( $offset )->get ( 'bc_welfare_chat_groups' );
		return $query;
	}
	...
	public function get_all_admin_users() {
		$this->mongo_db->switchDatabase ( $this->common_db ['common_db'] );
		$query = $this->mongo_db->get ( $this->collections ['bc_welfare_admins'] );
		$this->mongo_db->switchDatabase ( $this->common_db ['dsn'] );
		return $query;
	}
	public function get_all_health_supervisors() {
		$this->mongo_db->switchDatabase ( $this->common_db ['common_db'] );
		$query = $this->mongo_db->get ( $this->collections ['bc_welfare_health_supervisors'] );
		$this->mongo_db->switchDatabase ( $this->common_db ['dsn'] );
		return $query;
	}
	public function get_all_cc_users() {
		$this->mongo_db->switchDatabase ( $this->common_db ['common_db'] );
		$query = $this->mongo_db->get ( $this->collections ['bc_welfare_cc'] );
		$this->mongo_db->switchDatabase ( $this->common_db ['dsn'] );
		return $query;
	}
	...
	{
		$query = $this->mongo_db->where("chat_room_id",$msg_id)->get($this->collections['bc_welfare_messages']);
		return $query;
	}
	public function get_user_by_email($name, $email){
		...
		$user = $this->mongo_db->where("email",$email)->get($this->collections['bc_welfare_admins']);
	...
	public function updateGcmID($login, $gcm_registration_id){
		$id_exists = $this->mongo_db->where("user_id",$login)->get($this->collections['bc_welfare_users_gcm']);
		$data = array(
			"user_id" => $login,
			"gcm_registration_id" => $gcm_registration_id
		);
		if($id_exists){
			$query = $this->mongo_db->where("user_id",$login)->set($data)->get($this->collections['bc_welfare_users_gcm']);
		}else{
			$query = $this->mongo_db->insert($this->collections['bc_welfare_users_gcm'],$data);
		}
		return $query;
	}
	...
	{
	  $query = $this->mongo_db->select(array('app_template'))->where('_id','healthcare201812315245587')->get($this->collections['records']);
	...
	public function get_sanitation_report_pie_data($date, $search_criteria, $opt) {
	 ...
	 $query = $this->mongo_db->select(array('doc_data.widget_data'),array())->whereLike('doc_data.widget_data.page4.Declaration Information.Date:',$date)->where(array('doc_data.widget_data.page4.Declaration Information.Date:'=>$date,$search_criteria=>$opt))->get('healthcare201812315245587');
	...
	public function get_hs_req_docs($usercollection)
	{
		$this->mongo_db->orderBy(array('doc_received_time' => -1));
	  	$query=$this->mongo_db->select(array(),array('_id'))->where(array('status'=>'new','app_id'=>'healthcare2018122191146894'))->get($usercollection);
		return $query; 
	}
	...
	public function get_school_details_for_school_code($school_code)
	{
		...
		$school_data = $this->mongo_db->where(array('school_code'=>$school_code))->select(array('school_name','dt_name'),array())->get('bc_welfare_schools');
	...
	public function get_students_by_class($class, $school_name)
	{
		 if($class == "All")
		{
			$query = $this->mongo_db->where(array("doc_data.widget_data.page2.Personal Information.School Name"=>$school_name))->get("bcwelfare_screening_report_col_2020-2021");
			return $query;
		}
		else
		{ 
		   $query = $this->mongo_db->where(array("doc_data.widget_data.page2.Personal Information.School Name"=>$school_name,"doc_data.widget_data.page2.Personal Information.Class"=>$class))->get("bcwelfare_screening_report_col_2020-2021"); 
		   return $query;
		 } 
	}
	...
	{
		$qry = $this->mongo_db->where('_id', new MongoId($district_id))->get('bc_welfare_districts');
		if($qry)
		{
			return $qry;
		}
	...
	public function get_all_classes($cls_name = "All",$school_code) 
	{
		if ($cls_name == "All") 
		{
			$query = $this->mongo_db->orderBy ( array (
					'class_name' => 1 
			) )->where(array('school_code'=>$school_code))->get ($this->collections['bc_welfare_classes']);
		} 
		else 
		{
	...
	public function get_all_sections($section_name = "All",$school_code) 
	{
		if ($section_name == "All") 
		{
			$query = $this->mongo_db->orderBy ( array (
					'section_name' => 1 
			) )->where(array('school_code'=>$school_code))->get ($this->collections['bc_welfare_sections']);
		} 
		else 
		{
	...
	public function generate_new_uniqueid_for_new_student_by_checking_all_collection($school_code,$district_code,$school_name)
	{
		...
		$all_uniqueID = $this->mongo_db->where(array('doc_data.widget_data.page2.Personal Information.School Name'=>$school_name))->select(array('doc_data.widget_data.page1.Personal Information.Hospital Unique ID'),array())->orderBy(array('doc_data.widget_data.page1.Personal Information.Hospital Unique ID' => 1))->get('bcwelfare_screening_report_col_2020-2021');
		if(!empty($all_uniqueID)){
		...
		$all_uniqueID_frm_pasout = $this->mongo_db->where(array('doc_data.widget_data.page2.Personal Information.School Name'=>$school_name))->select(array('doc_data.widget_data.page1.Personal Information.Hospital Unique ID'),array())->orderBy(array('doc_data.widget_data.page1.Personal Information.Hospital Unique ID' => 1))->get('bcwelfare_screening_report_col_2020-2021_passed_out');
		...
		$all_uniqueID_frm_otherscls = $this->mongo_db->where(array('doc_data.widget_data.page2.Personal Information.School Name'=>$school_name))->select(array('doc_data.widget_data.page1.Personal Information.Hospital Unique ID'),array())->orderBy(array('doc_data.widget_data.page1.Personal Information.Hospital Unique ID' => 1))->get('bcwelfare_other_classes_screening_data_2019-2020');
	...
	public function add_student_ehr_model($doc_data,$history,$doc_properties)
	{
	  ...
	  $query = $this->mongo_db->insert($this->screening_app_col_2020_21,$doc_data);
	  if($query)
		  return TRUE;
	  else
		  return FALSE;
	...
	public function create_class($post, $school_code)
    {
    	...
    	$query = $this->mongo_db->insert($this->collections['bc_welfare_classes'],$data);
    	return $query;
    }
	
	public function delete_class($class_id, $school_code)
    {
    	$query = $this->mongo_db->where(array("school_code" => $school_code, "_id"=>new MongoId($class_id)))->delete($this->collections['bc_welfare_classes']);
		return $query;
    }
    ...
    public function create_section($post, $school_code)
    {
    	...
    	$query = $this->mongo_db->insert($this->collections['bc_welfare_sections'],$data);
    	return $query;
    }
    
    public function delete_section($section_id, $school_code)
    {
    	$query = $this->mongo_db->where(array("school_code" => $school_code, "_id"=>new MongoId($section_id)))->delete($this->collections['bc_welfare_sections']);
    	return $query;
    }
	...
	public function bc_welfare_chronic_cases_count($school_name)
    {
     $query = $this->mongo_db->getWhere($this->collections['bc_welfare_chronic_cases'],array('school_name'=>$school_name));
	 return count($query);
    }
	...
	{
	    $this->mongo_db->orderBy(array('_id' => 1));
		$query = $this->mongo_db->select(array('student_unique_id','case_id','scheduled_months'),array())->getWhere($this->collections['bc_welfare_chronic_cases'],array('school_name'=>$school_name,'followup_scheduled'=>'true'));
		if ($query)
		{
			return $query;
		}
	...
	function create_schedule_followup_model($unique_id,$medication_schedule,$treatment_period,$start_date,$monthNames,$case_id)
	{
	   ...
	   $updated = $this->mongo_db->where(array('student_unique_id'=>$unique_id,'case_id'=>$case_id))->set($update_array)->update($this->collections['bc_welfare_chronic_cases']);
	...
	function calculate_chronic_graph_compliance_percentage($case_id,$unique_id,$medication_taken)
	{
	   ...
	   $query = $this->mongo_db->select(array(),array())->getWhere($this->collections['bc_welfare_chronic_cases'],array('student_unique_id'=>$unique_id,'case_id'=>$case_id));  
	...
	function update_schedule_followup_model($unique_id,$case_id,$compliance,$selected_date)
	{
	   $check_query = array("student_unique_id"=>$unique_id,"case_id"=>$case_id,"medication_taken"=>array('$elemMatch'=>array("date"=>$selected_date)));
		...
		$is_already_updated = $this->mongo_db->where($check_query)->get($this->collections['bc_welfare_chronic_cases']);
		...
		else
		{
		   ...
		   $response = $this->mongo_db->command(array( 
			'findAndModify' => $this->collections['bc_welfare_chronic_cases'],
			'query'         => $query,
			'update'        => $update,
			'upsert'        => true
			));
	...
	public function fetch_student_pill_compliance_data($case_id,$student_unique_id)
	{
		...
		$query = $this->mongo_db->where($where_clause)->get($this->collections['bc_welfare_chronic_cases']);
        return $query;
	}
	/*
	...
	*/ 
	...
	{
		$query = $this->mongo_db->select(array('doc_data.widget_data.page1.Student Details.BMI_values'))->where(array('doc_data.widget_data.page1.Student Details.Hospital Unique ID' => $unique_id))->get('healthcare2018213172422286');
   ...
   public function get_update_personal_ehr_uid($uid) {
	...
	public function get_today_news_feeds($date){
		...
		$result = [ ];
			...
			$response = $this->mongo_db->command ( array (
					'aggregate' => $this->collections ['bc_welfare_news_feed'],
					'pipeline' => $pipeline 
			) );
			$query = array();
			if($response['ok']){
	...
	public function get_all_news_feeds(){
		...
		$query = $this->mongo_db->get ( $this->collections ['bc_welfare_news_feed'] );
	...
	/*@Naresh
	get Bmi values for school wise color code
	*/
	public function get_school_color_code($school_code)
	{
		...
		$query_normal = $this->mongo_db->whereLike("doc_data.widget_data.page1.Student Details.Hospital Unique ID",$school_code,'i')->whereBetween('doc_data.widget_data.page1.Student Details.BMI_values.bmi',"18.5" , 'doc_data.widget_data.page1.Student Details.BMI_values.bmi',"24.9")->get($this->bmi_app_col);
		
		$count_normal = count($query_normal);
		$query_over = $this->mongo_db->whereLike("doc_data.widget_data.page1.Student Details.Hospital Unique ID",$school_code,'i')->whereBetween('doc_data.widget_data.page1.Student Details.BMI_values.bmi',"25.0" , 'doc_data.widget_data.page1.Student Details.BMI_values.bmi',"29.9")->get($this->bmi_app_col);
	...
	{
		$res = $this->mongo_db->select(array('doc_data.widget_data'))->where(array('doc_data.widget_data.page2.Personal Information.School Name' => $school_name, 'doc_data.widget_data.page1.Personal Information.Hospital Unique ID'=> trim($unique_id)))->get('bcwelfare_screening_report_col_2020-2021');
	...
	{
		  $query = $this->mongo_db->getWhere('healthcare2018213172422286', array('doc_properties.doc_id' => $doc_id));
		  return $query[0];
	}	
	function check_if_doc_exists_in_bmi($unique_id)
	{
		$is_exists = $this->mongo_db->where(array('doc_data.widget_data.page1.Student Details.Hospital Unique ID'=> $unique_id))->get('healthcare2018213172422286');
	...
	function update_bmi_values($month,$monthly_bmi,$unique_id,$bmi_values_data)
	{
		...
		$after_update = $this->mongo_db->where(array('doc_data.widget_data.page1.Student Details.Hospital Unique ID'=> $unique_id))->set(array('doc_data.widget_data.page1.Student Details.BMI_latest'=> array('height' => $monthly_bmi['height'],
		  ...
		  $after_update = $this->mongo_db->where(array('doc_data.widget_data.page1.Student Details.Hospital Unique ID'=> $unique_id))->push('doc_data.widget_data.page1.Student Details.BMI_values',$monthly_bmi)->update('healthcare2018213172422286');
	...
	public function add_student_BMI_model($doc_data,  $doc_properties, $app_properties, $history)
	{
		  ...
		  $query = $this->mongo_db->insert('healthcare2018213172422286',$doc_data);
		  if($query)
			  return TRUE;
		  else
			  return FALSE;
	...
	{
		$is_exists = $this->mongo_db->where(array('doc_data.widget_data.page1.Student Details.Hospital Unique ID'=> $unique_id))->get('bc_welfare_himglobin_report_col');
		if($is_exists)
		{
			return TRUE;
		}
	...
	public function add_student_HB_model($monthly_hb,$doc_properties, $app_properties, $history)
	{
		  ...
		  $query = $this->mongo_db->insert('bc_welfare_himglobin_report_col',$doc_data);
		  if($query)
			  return TRUE;
		  else
			  return FALSE;
	...
	{
		$query = $this->mongo_db->select(array('doc_data.widget_data.page1.Student Details'))->where(array('doc_data.widget_data.page1.Student Details.Hospital Unique ID' => $unique_id))->get('bc_welfare_himglobin_report_col');
	...
	public function fetch_student_information_model($school_name,$unique_id)
	{
		...
		$res = $this->mongo_db->select(array('doc_data.widget_data.page1','doc_data.widget_data.page2'))->where(array('doc_data.widget_data.page2.Personal Information.School Name' => $school_name, 'doc_data.widget_data.page1.Personal Information.Hospital Unique ID'=> $unique_id))->get('healthcare201812217594045');
		...
		function create_chronic_case_new($student_unique_id,$request_type,$chronic_disease,$disease_desc,$schoolName)
	{
		...
		$query = $this->mongo_db->insert('bc_welfare_chronic_cases', $data);
	...
	public function initiate_request_model($doc_data,$doc_properties,$app_properties,$array_history)
	{
		...
		$res = $this->mongo_db->insert('healthcare2018122191146894_static_html',$doc_data);
	...
	public function get_hs_req_normal($usercollection,$unique_id)
    {
        ...
        $pipeline = [ 
           ...
           exit();
            return $result;*/
          $query_normal = array('app_properties.status'=>'new','app_properties.app_id'=>'healthcare2018122191146894','doc_data.widget_data.page2.Review Info.Request Type' => 'Normal','doc_data.widget_data.page1.Student Info.Unique ID' => array('$regex' => $unique_id),'doc_data.widget_data.page2.Review Info.Status' => array('$ne' => "Cured"));
        $this->mongo_db->orderBy(array('history.0.time' => -1));
        $query=$this->mongo_db->select(array(),array('_id'))->limit(200)->
        where($query_normal)->get($usercollection);
    ...
    public function get_hs_req_emergency($usercollection,$unique_id)
    {
    	...
    	$query_emergency = array('app_properties.status'=>'new','app_properties.app_id'=>'healthcare2018122191146894','doc_data.widget_data.page2.Review Info.Request Type' => 'Emergency','doc_data.widget_data.page1.Student Info.Unique ID' => array('$regex' => $unique_id),'doc_data.widget_data.page2.Review Info.Status' => array('$ne' => "Cured"));
        $this->mongo_db->orderBy(array('history.0.time' => -1));
        $query=$this->mongo_db->select(array(),array('_id'))->limit(200)->
        where($query_emergency)->get($usercollection);
    ...
    {
    	$query_chronic = array('app_properties.status'=>'new','app_properties.app_id'=>'healthcare2018122191146894','doc_data.widget_data.page2.Review Info.Request Type' => 'Chronic','doc_data.widget_data.page1.Student Info.Unique ID' => array('$regex' => $unique_id),'doc_data.widget_data.page2.Review Info.Status' => array('$ne' => "Cured"));
        $this->mongo_db->orderBy(array('history.0.time' => -1));
        $query=$this->mongo_db->select(array(),array('_id'))->limit(200)->
    ...
    {
    	$query_chronic = array('app_properties.status'=>'new','app_properties.app_id'=>'healthcare2018122191146894','doc_data.widget_data.page2.Review Info.Status' => 'Cured','doc_data.widget_data.page1.Student Info.Unique ID' => array('$regex' => $unique_id));
        $this->mongo_db->orderBy(array('history.0.time' => -1));
        $query=$this->mongo_db->select(array(),array('_id'))->limit(2000)->
        where($query_chronic)->get($usercollection);
    ...
    {
    	$query_chronic = array('app_properties.status'=>'new','app_properties.app_id'=>'healthcare2018122191146894','doc_data.widget_data.page2.Review Info.Status' => 'Expired','doc_data.widget_data.page1.Student Info.Unique ID' => array('$regex' => $unique_id));
        $this->mongo_db->orderBy(array('history.0.time' => -1));
        $query=$this->mongo_db->select(array(),array('_id'))->limit(200)->
        where($query_chronic)->get($usercollection);
	...
	{
		$query = $this->mongo_db->select(array(),array('_id'))->where(array('doc_properties.doc_id' => $doc_id))->get('healthcare2018122191146894_static_html');
	...
	{
		$query = $this->mongo_db->select(array(),array('_id'))->where(array('doc_data.widget_data.page1.Student Info.Unique ID' => $unique_id,'doc_properties.doc_id' => $doc_id))->get('healthcare2018122191146894_static_html');
		return $query;
	}
	 /**
	...
	{	
		...
		$this->mongo_db->insert('healthcare2018122191146894_static_html_shadow',$doc_data);
	...
	{
		$this->mongo_db->switchDatabase($this->_configvalue['common_db']);
		$school_data = $this->mongo_db->where(array('school_code'=>$school_code))->select(array('school_name','dt_name','contact_person_name','school_mob'),array())->get('bc_welfare_schools');
		$this->mongo_db->switchDatabase($this->_configvalue['dsn']);
		foreach ( $school_data as $schools => $school ) 
		{
			$dt_name = $this->mongo_db->where ( '_id', new MongoId ( $school ['dt_name'] ) )->get ( 'bc_welfare_district' );
    ...
    {
    	$query = $this->mongo_db->select(array('history'))->where(array('doc_properties.doc_id'=> $doc_id))->get('healthcare2018122191146894_static_html');
	...
	{
	  $this->mongo_db->switchDatabase($this->_configvalue['common_db']);
	  $ha_email = $this->mongo_db->select(array('email'),array())->get($this->collections['bc_welfare_cc']);
	  $this->mongo_db->switchDatabase($this->_configvalue['dsn']);
	  $ha_emails = [];
	...
	{
	  $this->mongo_db->switchDatabase($this->_configvalue['common_db']);
	  $email = $this->mongo_db->select(array('email'),array())->where('school_code',$school_code)->get($this->collections['bc_welfare_health_supervisors']);
	  $this->mongo_db->switchDatabase($this->_configvalue['dsn']);
	...
	function bc_welfare_update_hb_values($month,$monthly_hb,$unique_id)
	{
	...
	function bc_welfare_update_bmi_values($month,$monthly_bmi,$unique_id)
	{
	...
	*/
		...
		public function get_attendance_report_daily_count($school_name,$date)
		{
			...
			))->where(array('doc_data.widget_data.page1.Attendence Details.Select School' => $school_name))->WhereLike('history.last_stage.time', $today_date)->get('healthcare2018130121531948');
			if($query)
			{
		...
		public function get_requests_daily_count($district_school_code,$date)
		{
			...
			))->count('healthcare2018122191146894_static_html');
 ...
 /* HS Daily Requests information to show on modal */ 
	...
	$normal_requests_info = $this->mongo_db->select($selected_fields)->whereLike(
				'doc_data.widget_data.page1.Student Info.Unique ID',$district_school_code)
			   ->WhereLike('history.time', $today_date)
			  ->where(array('doc_data.widget_data.page2.Review Info.Request Type' => "Normal"))->get('healthcare2018122191146894_static_html');
	...
	$emergency_requests_info = $this->mongo_db->select($selected_fields)->whereLike('doc_data.widget_data.page1.Student Info.Unique ID',$district_school_code)
			   ->WhereLike('history.time', $today_date)
			   ->where(array('doc_data.widget_data.page2.Review Info.Request Type' => "Emergency"))->get('healthcare2018122191146894_static_html');
	...
	$chronic_requests_info = $this->mongo_db->select($selected_fields)->whereLike('doc_data.widget_data.page1.Student Info.Unique ID',$district_school_code)
			   ->WhereLike('history.time', $today_date)
			   ->where(array('doc_data.widget_data.page2.Review Info.Request Type' => "Chronic"))->get('healthcare2018122191146894_static_html');
	...
	public function get_docs_daily_count($district_school_code, $date)
		{
			...
			$doc_normal_requests = $this->mongo_db->whereLike(
              ...
              ->where(array('doc_data.widget_data.page2.Review Info.Request Type' => "Normal"))->count('healthcare2018122191146894_static_html');
	...
	public function get_bmi_report_model_count($current_month, $school_name) {
		...
		if ($current_month) {
			...
			$selected_month = $this->selected_month;
				...
				$under_weight = $this->mongo_db->where('doc_data.widget_data.school_details.School Name',$school_name)->whereLt('doc_data.widget_data.page1.Student Details.BMI_values.bmi',18.50) ->whereLike('doc_data.widget_data.page1.Student Details.BMI_values.month',$selected_month)->count('healthcare2018213172422286');
				
				$normal_weight = $this->mongo_db->where('doc_data.widget_data.school_details.School Name',$school_name)->whereBetween('doc_data.widget_data.page1.Student Details.BMI_values.bmi',18.50,24.99)->whereLike('doc_data.widget_data.page1.Student Details.BMI_values.month',$selected_month)->count('healthcare2018213172422286');
				
				$over_weight = $this->mongo_db->where('doc_data.widget_data.school_details.School Name',$school_name)->whereBetween('doc_data.widget_data.page1.Student Details.BMI_values.bmi',25.00,29.99)->whereLike('doc_data.widget_data.page1.Student Details.BMI_values.month',$selected_month)->count('healthcare2018213172422286');

				$obese = $this->mongo_db->where('doc_data.widget_data.school_details.School Name',$school_name)->wheregte('doc_data.widget_data.page1.Student Details.BMI_values.bmi',30.0)->whereLike('doc_data.widget_data.page1.Student Details.BMI_values.month',$selected_month)->count('healthcare2018213172422286');
	...
	public function get_bmi_report_model_info($current_month, $school_name) {
		...
		if ($current_month) {
			...
			$selected_month = $this->selected_month;
				...
				$under_weight_info = $this->mongo_db->select(array('doc_data.widget_data.page1.Student Details'))->where('doc_data.widget_data.school_details.School Name',$school_name)->whereLt('doc_data.widget_data.page1.Student Details.BMI_values.bmi',18.50)->get('healthcare2018213172422286');
				
			    $normal_weight_info = $this->mongo_db->select(array('doc_data.widget_data.page1.Student Details'))->where('doc_data.widget_data.school_details.School Name',$school_name)->whereBetween('doc_data.widget_data.page1.Student Details.BMI_values.bmi',18.50,24.99)->get('healthcare2018213172422286');
			   
			 
				$over_weight_info = $this->mongo_db->select(array('doc_data.widget_data.page1.Student Details'))->where('doc_data.widget_data.school_details.School Name',$school_name)->whereBetween('doc_data.widget_data.page1.Student Details.BMI_values.bmi',25.00,29.99)->get('healthcare2018213172422286');
			
				$obese_info = $this->mongo_db->select(array('doc_data.widget_data.page1.Student Details'))->where('doc_data.widget_data.school_details.School Name',$school_name)->wheregte('doc_data.widget_data.page1.Student Details.BMI_values.bmi',30.0)->get('healthcare2018213172422286');
...
/* Severe Type Anamea */			
				...
				$severe_anamia = $this->mongo_db->where(array('doc_data.widget_data.school_details.School Name' => $school_name,'doc_data.widget_data.page1.Student Details.HB_values.month' => $selected_month))->whereLt('doc_data.widget_data.page1.Student Details.HB_values.hb',8)->count($this->hb_app_col);


/* Moderate Type Anamea */	
				$moderate_anamia = $this->mongo_db->where(array('doc_data.widget_data.school_details.School Name' => $school_name,'doc_data.widget_data.page1.Student Details.HB_values.month' => $selected_month))->whereBetween('doc_data.widget_data.page1.Student Details.HB_values.hb',8,10)->count($this->hb_app_col);

				
/* Mild Type Anamea */	
				$mild_anamia = $this->mongo_db->where(array('doc_data.widget_data.school_details.School Name' => $school_name,'doc_data.widget_data.page1.Student Details.HB_values.month' => $selected_month))->whereBetween('doc_data.widget_data.page1.Student Details.HB_values.hb',10,12)->count($this->hb_app_col);
				
/* Normal Type Anamea */	

					$normal_anamia = $this->mongo_db->where(array('doc_data.widget_data.school_details.School Name' => $school_name,'doc_data.widget_data.page1.Student Details.HB_values.month' => $selected_month))->whereGt('doc_data.widget_data.page1.Student Details.HB_values.hb',12.1)->count($this->hb_app_col);
...
public function get_hb_report_model_info($current_month, $school_name) {
		...
		if ($current_month) {
			...
			$selected_month = $this->selected_month;
				...
				$severe_anamia_info = $this->mongo_db->where(array('doc_data.widget_data.school_details.School Name' => $school_name,'doc_data.widget_data.page1.Student Details.HB_values.month' => $selected_month))->whereLt('doc_data.widget_data.page1.Student Details.HB_values.hb',8)->get($this->hb_app_col);

				$moderate_anamia_info = $this->mongo_db->where(array('doc_data.widget_data.school_details.School Name' => $school_name,'doc_data.widget_data.page1.Student Details.HB_values.month' => $selected_month))->whereBetween('doc_data.widget_data.page1.Student Details.HB_values.hb',8,10)->get($this->hb_app_col);

				$mild_anamia_info = $this->mongo_db->where(array('doc_data.widget_data.school_details.School Name' => $school_name,'doc_data.widget_data.page1.Student Details.HB_values.month' => $selected_month))->whereBetween('doc_data.widget_data.page1.Student Details.HB_values.hb',10,12)->get($this->hb_app_col);

				$normal_anamia_info = $this->mongo_db->where(array('doc_data.widget_data.school_details.School Name' => $school_name,'doc_data.widget_data.page1.Student Details.HB_values.month' => $selected_month))->whereGt('doc_data.widget_data.page1.Student Details.HB_values.hb',12.1)->get($this->hb_app_col);
	...
	public function get_sanitation_report_data_with_date_version_2($date,$school_name)
	{
		...
		$this->mongo_db->whereLike('doc_data.widget_data.page4.Declaration Information.Date:',$selected_date)->where(array('doc_data.widget_data.page4.School Information.School Name'=>$school_name));
		$query = $this->mongo_db->get('healthcare201822113134483_version_2');
		if($query)
			return $query;
		else
			return FALSE;
	...
	public function get_screened_students_count($unique_id_pattern)
	{
		...
		$screening_count = $this->mongo_db->whereLike('doc_data.widget_data.page1.Personal Information.Hospital Unique ID',$unique_id_pattern)->whereNe(array('doc_data.widget_data.page3.Physical Exam' => array()))->count($this->screening_app_col);
	...
	public function get_students_load_ehr_doc_model($_id) {
		...
		if ($query) {
			$query_request = $this->mongo_db->where ( "doc_data.widget_data.page1.Student Info.Unique ID", $query [0] ["doc_data"] ['widget_data'] ['page1'] ['Personal Information'] ['Hospital Unique ID'] )->get ( 'healthcare2018122191146894_static_html' );
	...
	public function update_principal_hs_profile($data,$schoolCode)
	{
		//echo print_r($data,TRUE);exit();
			...
			$this->mongo_db->switchDatabase($this->common_db['common_db']);
			$result = $this->mongo_db->where('school_code',$schoolCode)->set($update_profile_hs)->update('bc_welfare_health_supervisors');
			$this->mongo_db->switchDatabase($this->common_db['dsn']);
			...
			$this->mongo_db->switchDatabase($this->common_db['common_db']);
			$result = $this->mongo_db->where('school_code',$schoolCode)->set($update_profile_principal)->update('bc_welfare_schools');
			$this->mongo_db->switchDatabase($this->common_db['dsn']);
	...
	public function get_bmi_report_model($current_month, $school_name) {
		...
		$requests = [ ];
				...
				$under_weight = $this->mongo_db->where('doc_data.widget_data.school_details.School Name',$school_name)->whereLt('doc_data.widget_data.page1.Student Details.BMI_values.bmi',18.50) ->whereLike('doc_data.widget_data.page1.Student Details.BMI_values.month',$selected_month)->get('healthcare2018213172422286');
				$request ['label'] = 'UNDER WEIGHT';
				$request ['value'] = count($under_weight);
				array_push ( $requests, $request );
				
				$normal_weight = $this->mongo_db->where('doc_data.widget_data.school_details.School Name',$school_name)->whereBetween('doc_data.widget_data.page1.Student Details.BMI_values.bmi',18.50,24.99)->whereLike('doc_data.widget_data.page1.Student Details.BMI_values.month',$selected_month)->get('healthcare2018213172422286');
				$request ['label'] = 'NORMAL WEIGHT';
				$request ['value'] = count($normal_weight);
				array_push ( $requests, $request );
				
				$over_weight = $this->mongo_db->where('doc_data.widget_data.school_details.School Name',$school_name)->whereBetween('doc_data.widget_data.page1.Student Details.BMI_values.bmi',25.00,29.99)->whereLike('doc_data.widget_data.page1.Student Details.BMI_values.month',$selected_month)->get('healthcare2018213172422286');
				$request ['label'] = 'OVER WEIGHT';
				$request ['value'] = count($over_weight);
				array_push ( $requests, $request );
				
				$obese = $this->mongo_db->where('doc_data.widget_data.school_details.School Name',$school_name)->wheregte('doc_data.widget_data.page1.Student Details.BMI_values.bmi',30.0)->whereLike('doc_data.widget_data.page1.Student Details.BMI_values.month',$selected_month)->get('healthcare2018213172422286');
				$request ['label'] = 'OBESE';
				$request ['value'] = count($obese);
				array_push ( $requests, $request );
				//echo print_r($requests,true);
				//echo print_r($selected_month,true);exit;
	...
	public function get_drill_down_to_bmi_report($type, $current_month,  $school_name) 
	{
		...
		switch ($type) {
			...
			//ini_set ( 'memory_limit', '10G' );
				...
				//log_message('error','query================14095'.print_r($query,TRUE));
				$doc_query = array ();
				if ($school_name == "All") {
					if ($district_name != "All") {
						foreach ( $query as $doc ) {
	...
	public function get_hb_report_model($current_month, $school_name) {
		...
		$requests = [ ];
				...
				$sevier = $this->mongo_db->where('doc_data.widget_data.school_details.School Name',$school_name)->whereLt('doc_data.widget_data.page1.Student Details.HB_latest.hb',8) ->whereLike('doc_data.widget_data.page1.Student Details.HB_latest.month',$selected_month)->get('bc_welfare_himglobin_report_col');
				$request ['label'] = 'Sevier';
				$request ['value'] = count($sevier);
				array_push ( $requests, $request );
				
				$normal = $this->mongo_db->where('doc_data.widget_data.school_details.School Name',$school_name)->whereBetween('doc_data.widget_data.page1.Student Details.HB_latest.hb',12,18)->whereLike('doc_data.widget_data.page1.Student Details.HB_latest.month',$selected_month)->get('bc_welfare_himglobin_report_col');
				$request ['label'] = 'NORMAL';
				$request ['value'] = count($normal);
				array_push ( $requests, $request );
				
				$moderate = $this->mongo_db->where('doc_data.widget_data.school_details.School Name',$school_name)->whereBetween('doc_data.widget_data.page1.Student Details.HB_latest.hb',8.1,10)->whereLike('doc_data.widget_data.page1.Student Details.HB_latest.month',$selected_month)->get('bc_welfare_himglobin_report_col');
				$request ['label'] = 'Moderate';
				$request ['value'] = count($moderate);
				array_push ( $requests, $request );
				
				$mild = $this->mongo_db->where('doc_data.widget_data.school_details.School Name',$school_name)->whereBetween('doc_data.widget_data.page1.Student Details.HB_latest.hb',10.1,12)->whereLike('doc_data.widget_data.page1.Student Details.HB_latest.month',$selected_month)->get('bc_welfare_himglobin_report_col');
				$request ['label'] = 'Mild';
	...
	public function get_drill_down_to_hb_report($type, $current_month,  $school_name) 
	{
		...
		switch ($type) {
			case "Severe" :
				...
				) )->whereLt('doc_data.widget_data.page1.Student Details.HB_values.hb',8)->whereLike ( 'doc_data.widget_data.page1.Student Details.HB_values.month',$selected_month)->where("doc_data.widget_data.school_details.School Name",$school_name)->get ( $this->hb_app_col );
			...
			ini_set ( 'memory_limit', '10G' );
				$query = $this->mongo_db->select ( array (
						"doc_data" 
				) )->whereBetween('doc_data.widget_data.page1.Student Details.HB_values.hb',12,18) ->whereLike ( 'doc_data.widget_data.page1.Student Details.HB_values.month',$selected_month)->get ( $this->hb_app_col );
			...
			ini_set ( 'memory_limit', '10G' );
				...
				$query = $this->mongo_db->select ( array (
						"doc_data" 
				) )->whereBetween('doc_data.widget_data.page1.Student Details.HB_values.hb',8.1,10)->whereLike ( 'doc_data.widget_data.page1.Student Details.HB_values.month',$selected_month)-> get ( $this->hb_app_col );
				...
				$query = $this->mongo_db->select ( array (
						"doc_data" 
				) )->whereBetween('doc_data.widget_data.page1.Student Details.HB_values.hb',10.1,12)->whereLike ( 'doc_data.widget_data.page1.Student Details.HB_values.month',$selected_month)-> get ( $this->hb_app_col );
		...
		public function get_doctor_visiting_report_date_wise($school_name)
    {
          ...
          $response = $this->mongo_db->command(array('distinct' => "bc_welfare_doctor_visiting_reports" ,'key' => "doc_data.widget_data.Student Details.doctor_visiting_date",'query'=>$query));


          foreach ($response['values'] as $dates) {
            
             $getSubmittedDocs = $this->mongo_db->where(array('doc_data.widget_data.Student Details.doctor_visiting_date' =>$dates,"doc_data.widget_data.school_details.School Name"=>$school_name))->count('bc_welfare_doctor_visiting_reports');
     ...
     public function fetch_student_info_for_doctor_visit($school_name, $unique_id)
    {
        ...
        $res = $this->mongo_db->select(array('doc_data.widget_data'))->where(array('doc_data.widget_data.page2.Personal Information.School Name' => $school_name, 'doc_data.widget_data.page1.Personal Information.Hospital Unique ID'=> $unique_id))->get('healthcare201812217594045');
     ...
     public function fetch_student_attachments($school_name, $unique_id)
    {
        ...
        $res  = $this->mongo_db->select(array("doc_data.widget_data.Student Details.Hospital Unique ID","doc_properties.doc_id","doc_attachments"))->where(array('doc_data.widget_data.Student Details.Hospital Unique ID' =>$unique_id,"doc_data.widget_data.school_details.School Name"=>$school_name))->get('bc_welfare_doctor_visiting_reports');
    ...
    {
        $is_exists = $this->mongo_db->where(array('doc_data.widget_data.Student Details.Hospital Unique ID'=> $unique_id))->get('bc_welfare_doctor_visiting_reports');
        if($is_exists)
        {
            return TRUE;
        }
    ...
    public function drill_down_to_doctor_treated_list($school_name,$selectedDate)
    {
             ...
             $getSubmittedDocs = $this->mongo_db->where(array('doc_data.widget_data.Student Details.doctor_visiting_date' =>$selectedDate,"doc_data.widget_data.school_details.School Name"=>$school_name))->get('bc_welfare_doctor_visiting_reports');
    ...
    public function show_doctor_treated_student(/*$student_id,$doctor_visit_date*/ $doc_id)
    {
             ...
             $getSubmittedDocs = $this->mongo_db->select(array(),array('_id'))->where(array('doc_properties.doc_id'=>$doc_id))->get('bc_welfare_doctor_visiting_reports');
    ...
    {
        $query = $this->mongo_db->select(array(),array('_id'))->where(array('doc_data.widget_data.Student Details.Hospital Unique ID' => $unique_id,'doc_properties.doc_id' => $doc_id))->get('bc_welfare_doctor_visiting_reports');
    ...
    {   
        ...
        $update_query = $this->mongo_db->where(array('doc_data.widget_data.Student Details.Hospital Unique ID' => $uniqueId, 'doc_properties.doc_id' => $doc_id))->set(array('doc_attachments' =>$doc_attachments))->update('bc_welfare_doctor_visiting_reports');
...
//		foreach ($final_result as $school_name) 
		{
//			$this->mongo_db->switchDatabase ( $this->common_db ['common_db'] );
			$pc_result = $this->mongo_db->select(array('school_code','school_mob','contact_person_name'))->get($this->collections['bc_welfare_schools']);
			$this->mongo_db->switchDatabase ( $this->common_db ['dsn'] );
			...
			foreach ($pc_result as $school_code)
			{
				...
				where(array('school_code' => $school_code['school_code']))->get($this->collections['bc_welfare_health_supervisors']);
				$this->mongo_db->switchDatabase ( $this->common_db ['dsn'] );
	...
	public function send_sms_for_not_submited_attendance()
	{
		...
		$this->mongo_db->switchDatabase ( $this->common_db ['common_db'] );
		$res = $this->mongo_db->select(array('school_name'))->get($this->collections['bc_welfare_schools']);
		$this->mongo_db->switchDatabase ( $this->common_db ['dsn'] );
		foreach ($res as $school) {
			$school_name_col = $school['school_name'];
			array_push($schools_array_col, $school_name_col);
		...
		{
			$this->mongo_db->switchDatabase ( $this->common_db ['common_db'] );
			$pc_result = $this->mongo_db->select(array('school_code','school_mob','contact_person_name'))->where(array('school_name' => $school_name))->get($this->collections['bc_welfare_schools']);
			$this->mongo_db->switchDatabase ( $this->common_db ['dsn'] );
	...
	{
		$today_date = Date("Y-m-d");
		$get_count  = $this->mongo_db->where(array('Messages_info.school_name' => $data['school_name']))->count('BC_WELFARE_Attendance_msg_count');
		if($get_count != 0)
		{
			$query = $this->mongo_db->push('Sent_dates', $today_date)->inc(array('Sent_count' => 1))->update('BC_WELFARE_Attendance_msg_count');
		}else
		{
			$today_date  = Date("Y-m-d");
			$insert_data = array('Messages_info' => $data,'Sent_count' => 1,'Sent_dates' => [$today_date],'Type_of_Msg' => "Attendance Not Submitted");
			$query = $this->mongo_db->insert("BC_WELFARE_Attendance_msg_count", $insert_data);	
		}

		$over_all_count = $this->mongo_db->where(array("$today_date.Attendance_Messages_info.Sent_date" => $today_date))->count('BC_WELFARE_Over_all_Msg_status');

		if($over_all_count != 0)
		{
			$query = $this->mongo_db->where(array("$today_date.Attendance_Messages_info.Sent_date" => $today_date))->push("$today_date.Attendance_Messages_info.school_name", $data['school_name'])->inc(array("$today_date.Attendance_Messages_info.Sent_count" => 1))->update('BC_WELFARE_Over_all_Msg_status');
		}else
		{
			...
			$query = $this->mongo_db->insert('BC_WELFARE_Over_all_Msg_status', $insert_over_data);
		}
	...
	{
		$today_date = Date("Y-m-d");
		$get_count  = $this->mongo_db->where(array('Messages_info.school_name' => $data['school_name']))->count('BC_WELFARE_Sanitation_msg_count');
		if($get_count != 0)
		{
			$query = $this->mongo_db->push('Sent_dates', $today_date)->inc(array('Sent_count' => 1))->update('BC_WELFARE_Sanitation_msg_count');
		}else
		{
			$today_date  = Date("Y-m-d");
			$insert_data = array('Messages_info' => $data,'Sent_count' => 1,'Sent_dates' => [$today_date],'Type_of_Msg' => "Sanitation Not Submitted");
			$query = $this->mongo_db->insert("BC_WELFARE_Sanitation_msg_count", $insert_data);	
		}

		$over_all_count = $this->mongo_db->where(array("$today_date.Sanitation_Messages_info.Sent_date" => $today_date))->count('BC_WELFARE_Over_all_Msg_status');

		if($over_all_count != 0 )
		{
			$query = $this->mongo_db->where(array("$today_date.Sanitation_Messages_info.Sent_date" => $today_date))->push("$today_date.Sanitation_Messages_info.school_name", $data['school_name'])->inc(array("$today_date.Sanitation_Messages_info.Sent_count" => 1))->update('BC_WELFARE_Over_all_Msg_status');
		}else
		{
	 ...
	 public function insert_hospitalised_students_data($doc_data,$array_history,$unique_id,$doc_id,$doc_properties)
    {
        ...
        $this->mongo_db->insert('bc_hospitalised_students_col',$doc_data);

    }

     public function check_doc_id_of_request($doc_id)
    {
        $query = $this->mongo_db->where('doc_id', $doc_id)->get('bc_hospitalised_students_col');
	...
	{
		$query = $this->mongo_db->select(array('doc_data.widget_data.page1.Personal Information.Name'))->where(array("history.last_stage.submitted_by" => str_replace("@", "#", $email)))->get('bc_doctor_personal_profile_report');
		return $query;
	}
	...
	public function get_doc_id_for_check($initate_submit)
	{
		...
		$get_id = $this->mongo_db->select('doc_properties.doc_id')->where ( '_id', $initate_submit )->get('healthcare2018122191146894_static_html');
	...
	public function get_searched_student_sick_requests_model($search_data,$school_name)
	{
		...
		if(preg_match("/_/", $data)){
			...
			$query = $this->mongo_db->where('doc_data.widget_data.page1.Student Info.School Name.field_ref',$school_name)->whereLike('doc_data.widget_data.page1.Student Info.Unique ID', $data)->whereNe(array('doc_data.widget_data.page2.Review Info.Status' => "Cured"))->orderBy(array('history.0.time' => -1))->get($this->request_app_col_static_html);

		}else{

		$query = $this->mongo_db->where('doc_data.widget_data.page1.Student Info.School Name.field_ref',$school_name)->whereLike('doc_data.widget_data.page1.Student Info.Name.field_ref', $data)->whereNe(array('doc_data.widget_data.page2.Review Info.Status' => "Cured"))->orderBy(array('history.0.time' => -1))->get($this->request_app_col_static_html);

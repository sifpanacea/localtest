...
class Power_of_ten_mgmt_model extends CI_Model 
{
	/**
    ...
    public function swaero_create_state($post)
    {
    	
        $data = array(
                "state_name" => $post);
        $query = $this->mongo_db->insert('power_of_ten_states',$data);

        return $query;

    }

     public function swaero_create_zonal($post)
    {

        $data = array(
                "state_name" => $post['state_name'], 'zonal_name'=>$post['zonal_name']);
        $query = $this->mongo_db->insert('power_of_ten_zones',$data);

        return $query;

    }

    public function swaero_create_district($post)
    {

        $data = array(
                "state_name" => $post['state_name'], 'zonal_name'=>$post['zonal_name'], 'district_name'=>$post['district_name']);
        $query = $this->mongo_db->insert('power_of_ten_districts',$data);

        return $query;

    }

    public function swaero_create_mandal($post)
    {

        $data = array(
                "state_name" => $post['state_name'], 'zonal_name'=>$post['zonal_name'], 'district_name'=>$post['district_name'], 'mandal_name' => $post['mandal_name']);
        $query = $this->mongo_db->insert('power_of_ten_mandals',$data);

        return $query;

    }

    public function swaero_create_village($post)
    {

        $data = array(
                "state_name" => $post['state_name'], 'zonal_name'=>$post['zonal_name'], 'district_name'=>$post['district_name'], 'mandal_name' => $post['mandal_name'], 'village_name' => $post['village_name']);
        $query = $this->mongo_db->insert('power_of_ten_villages',$data);

        return $query;

    }

    public function swaero_create_network($post)
    {

        $data = array(
                "state_name" => $post['state_name'], 'zonal_name'=>$post['zonal_name'], 'district_name'=>$post['district_name'], 'mandal_name' => $post['mandal_name'], 'village_name' => $post['village_name'], 'swaero_type' => $post['swaero_type']);
        $query = $this->mongo_db->insert('power_of_ten_swaero_types',$data);

        return $query;

    }

    public function swaero_create_district_coordinator($post)
    {
        $password = $post['district_coordinator_password'];
        $salt = $this->store_salt ? $this->salt () : FALSE;

        $password = $this->hash_password ( $password, $salt );

        $data = array(
                "username" => $post['district_coordinator_name'], 'phone_no'=>$post['district_coordinator_mob'], 'district'=>$post['district_coordinator_dist'], 'password' => $password, 'email' => $post['district_coordinator_email'], 'address' => $post['district_coordinator_addr'], 'company'=>"healthcare", 'active'=>1);

        $this->mongo_db->switchDatabase($this->common_db['common_db']);
        $query = $this->mongo_db->insert('power_of_ten_district_coordinators',$data);
        $this->mongo_db->switchDatabase($this->common_db['dsn']);
    ...
    public function get_district_coordinators_data()
    {
        $this->mongo_db->switchDatabase($this->common_db['common_db']);
        $query = $this->mongo_db->get('power_of_ten_district_coordinators');
        $this->mongo_db->switchDatabase($this->common_db['dsn']);

        //$query['district_coordinator_counts'] = count($query);
    ...
    {
        $states = $this->mongo_db->get('power_of_ten_states');
    ...
    {
        $zones = $this->mongo_db->get('power_of_ten_zones');
        return $zones;
    }

    public function get_district_data()
    {
        $district = $this->mongo_db->get('power_of_ten_districts');
        return $district;
    }

    public function get_mandal_data()
    {
        $mandal = $this->mongo_db->get('power_of_ten_mandals');
        return $mandal;
    }

    public function get_village_data()
    {
        $village = $this->mongo_db->get('power_of_ten_villages');
        return $village;
    }

...
class Ttwreis_schools_common_model extends CI_Model {
	/**
	...
	function __construct() {
		...
		$this->screening_app_col_2020_21 = 'ttwreis_screening_report_col_2020-2021';
		$this->screening_app_col_2021_22 = 'ttwreis_screening_report_col_2021-2022';
		$this->absent_app_col    = "healthcare20161015173311279";
		$this->request_app_col   = "healthcare2016108181933756";
		$this->sanitation_infrastructure_app_col = "healthcare201712719524850";
		$this->sanitation_report_app_col         = "healthcare20171211809800";
		$this->bmi_app_col = "healthcare2017619153715384";
		/*$this->bmi_app_col_xl_import = "healthcare2017619153715384_XL_Import";*/
		/*$this->hb_app_col_xl_import = "ttwreis_himglobin_report_col_XL_Import";*/
	...
	public function get_start_end_date($today_date, $request_duration)
	{
		...
		else if ($request_duration == "Yearly") 
		{
			...
			$end_date = date ( "Y-m-d H:i:s", strtotime ( $today_date . "-2 year" ) );
			//$end_date = date ( "Y-m-d H:i:s", strtotime ( $end_date . "1 day" ) );
			//$today_date = date ( "Y-m-d H:i:s", strtotime ( $today_date . "1 day" ) );
			$dates ['today_date'] = $today_date;
			$dates ['end_date'] = $end_date;
	...
	public function classescount($school_code) {
		$count = $this->mongo_db->where('school_code',$school_code)->count ($this->collections['ttwreis_classes']);
		return $count;
	}
	...
	{
		$offset = $per_page * ($page - 1);
		$query = $this->mongo_db->limit ( $per_page )->offset ( $offset )->where('school_code',$school_code)->get ( $this->collections['ttwreis_classes']);
		return $query;
	}
	
	public function sectionscount($school_code) {
		$count = $this->mongo_db->where('school_code',$school_code)->count ($this->collections['ttwreis_sections']);
		return $count;
	}
	public function get_sections($per_page, $page, $school_code) {
		$offset = $per_page * ($page - 1);
		$query = $this->mongo_db->limit ( $per_page )->offset ( $offset )->where('school_code',$school_code)->get ( $this->collections['ttwreis_sections']);
		return $query;
	}
	...
	//"doc_data.widget_data.page2.Personal Information.School Name"=>$school_name,
		$query = $this->mongo_db->select ( array (
				...
				'doc_data.external_attachments',
				'history' 
		) )->where(array("doc_data.widget_data.page1.Personal Information.Hospital Unique ID"=> $uid))->get ( $this->screening_app_col_2020_21 );
		...
		if ($query) {
			...
			if(count($query_request) > 0){
				foreach($query_request as $req_ind => $req){
					...
					$notes_data = $this->mongo_db->where ("req_doc_id", $req['doc_properties']['doc_id'])->get ( 'ttwreis_req_notes' );
	...
	public function get_drilling_absent_students_docs($_id_array) {
		...
		foreach ( $_id_array as $_id ) {
			$query = $this->mongo_db->select ( array ('doc_data.widget_data.page1','doc_data.widget_data.page2') )->where( "doc_data.widget_data.page1.Personal Information.Hospital Unique ID", $_id )->get ( $this->screening_app_col_2021_22 );
			if ($query)
	...
	{
	  $query = $this->mongo_db->select(array('app_template'))->where('_id','healthcare20171211809800')->get($this->collections['records']);
	...
	public function get_sanitation_report_pie_data($date, $search_criteria, $opt) {
	 ...
	 $query = $this->mongo_db->select(array('doc_data.widget_data'),array())->where(array('doc_data.widget_data.page4.Declaration Information.Date:'=>$date,$search_criteria=>$opt))->get('healthcare20171211809800');
	...
	public function get_school_details_for_school_code($school_code)
	{
		...
		$school_data = $this->mongo_db->where(array('school_code'=>$school_code))->select(array('school_name','dt_name'),array())->get('ttwreis_schools');
	...
	public function get_student($per_page,$page,$school_name)
	{
		...
		$query = $this->mongo_db->limit ($per_page)->offset( $offset)->where(array("doc_data.widget_data.page2.Personal Information.School Name"=>$school_name))->get($this->screening_app_col_2021_22); 
	...
	public function get_students_by_class($class, $school_name)
	{
		 if($class == "All")
		{
			$query = $this->mongo_db->where(array("doc_data.widget_data.page2.Personal Information.School Name"=>$school_name))->get($this->screening_app_col_2021_22);
			return $query;
		}
		else
		{ 
		    $query = $this->mongo_db->where(array("doc_data.widget_data.page2.Personal Information.School Name"=>$school_name,"doc_data.widget_data.page2.Personal Information.Class"=>$class))->get($this->screening_app_col_2021_22);  
		   /* $query = $this->mongo_db->select(array('doc_data.widget_data.page1.Personal Information.Hospital Unique ID','doc_data.widget_data.page1.Personal Information.Name','doc_data.widget_data.page1.Personal Information.Mobile.mob_num','doc_data.widget_data.page1.Personal Information.Date of Birth','doc_data.widget_data.page2.Personal Information.Class','doc_data.widget_data.page2.Personal Information.Section'))->where(array('doc_data.widget_data.page2.Personal Information.Class'=>$class,'doc_data.widget_data.page2.Personal Information.School Name'=>$school_name))->get($this->screening_app_col);
		   log_message('debug','class for studentttttt==========12154'.print_r($class,true));
		   log_message('debug','query query studentttttt==========12154'.print_r($query,true)); */
		   return $query;
		 } 
	}
	...
	{
		$count = $this->mongo_db->where(array("doc_data.widget_data.page2.Personal Information.School Name"=>$school_name))->count($this->screening_app_col_2021_22);
		return $count;
	}
	
	public function get_district($district_id)
	{
		$qry = $this->mongo_db->where('_id', new MongoId($district_id))->get('ttwreis_district');
	...
	public function get_all_classes($cls_name = "All",$school_code) 
	{
		if ($cls_name == "All") 
		{
			$query = $this->mongo_db->orderBy ( array (
					'class_name' => 1 
			) )->where(array('school_code'=>$school_code))->get ($this->collections['ttwreis_classes']);
		} 
		else 
		{
	...
	public function get_all_sections($section_name = "All",$school_code) 
	{
		if ($section_name == "All") 
		{
			$query = $this->mongo_db->orderBy ( array (
					'section_name' => 1 
			) )->where(array('school_code'=>$school_code))->get ($this->collections['ttwreis_sections']);
		} 
		else 
		{
	...
	public function generate_new_student_hunique_id($school_code, $dist_code, $school_name)
	{
	  ...
	  $all_uniqueID = $this->mongo_db->where(array('doc_data.widget_data.page2.Personal Information.School Name'=>$school_name))->select(array('doc_data.widget_data.page1.Personal Information.Hospital Unique ID'),array())->get($this->screening_app_col_2021_22);
	...
	public function generate_new_uniqueid_for_new_student_by_checking_all_collection($school_code,$district_code,$school_name)
	{
		...
		$all_uniqueID = $this->mongo_db->where(array('doc_data.widget_data.page2.Personal Information.School Name'=>$school_name))->select(array('doc_data.widget_data.page1.Personal Information.Hospital Unique ID'),array())->orderBy(array('doc_data.widget_data.page1.Personal Information.Hospital Unique ID' => 1))->get('ttwreis_screening_report_col_2021-2022');
		if(!empty($all_uniqueID)){
		...
		$all_uniqueID_frm_pasout = $this->mongo_db->where(array('doc_data.widget_data.page2.Personal Information.School Name'=>$school_name))->select(array('doc_data.widget_data.page1.Personal Information.Hospital Unique ID'),array())->orderBy(array('doc_data.widget_data.page1.Personal Information.Hospital Unique ID' => 1))->get('ttwreis_screening_report_col_2021-2022_passed_out');
		...
		$all_uniqueID_frm_otherscls = $this->mongo_db->where(array('doc_data.widget_data.page2.Personal Information.School Name'=>$school_name))->select(array('doc_data.widget_data.page1.Personal Information.Hospital Unique ID'),array())->orderBy(array('doc_data.widget_data.page1.Personal Information.Hospital Unique ID' => 1))->get('ttwreis_other_classes_screening_data_2020-2021');
	...
	public function create_class($post, $school_code)
    {
    	...
    	$query = $this->mongo_db->insert($this->collections['ttwreis_classes'],$data);
    	return $query;
    }
	
	public function delete_class($class_id, $school_code)
    {
    	$query = $this->mongo_db->where(array("school_code" => $school_code, "_id"=>new MongoId($class_id)))->delete($this->collections['ttwreis_classes']);
		return $query;
    }
    ...
    public function create_section($post, $school_code)
    {
    	...
    	$query = $this->mongo_db->insert($this->collections['ttwreis_sections'],$data);
    	return $query;
    }
    
    public function delete_section($section_id, $school_code)
    {
    	$query = $this->mongo_db->where(array("school_code" => $school_code, "_id"=>new MongoId($section_id)))->delete($this->collections['ttwreis_sections']);
    	return $query;
    }
	...
	public function ttwreis_chronic_cases_count($school_name)
    {
     $query = $this->mongo_db->getWhere($this->collections['ttwreis_chronic_cases'],array('school_name'=>$school_name));
	 return count($query);
    }
	...
	{
	    $this->mongo_db->orderBy(array('_id' => 1));
		$query = $this->mongo_db->select(array('student_unique_id','case_id','scheduled_months'),array())->getWhere($this->collections['ttwreis_chronic_cases'],array('school_name'=>$school_name,'followup_scheduled'=>'true'));
		if ($query)
		{
			return $query;
		}
	...
	{
		$query_request = $this->mongo_db->where ( '_id',  $post ['doc_id'] )->get ( $this->request_app_col );
		...
		if(isset($query_request[0]['doc_data']['notes_data'])){
			array_push($query_request[0]['doc_data']['notes_data'],$notes);
		}
		else{
			$query_request[0]['doc_data']['notes_data'] = [];
			array_push($query_request[0]['doc_data']['notes_data'],$notes);
		}
		
		$token = $this->mongo_db->where ( '_id', new MongoId ( $post ['doc_id'] ) )->set($query_request[0])->update( $this->request_app_col );
   ...
   public function get_update_personal_ehr_uid($uid) {
		$query = $this->mongo_db->select(array ('doc_data.widget_data','doc_data.chart_data','doc_data.external_attachments','history' ) )->whereLike ( "doc_data.widget_data.page1.Personal Information.Hospital Unique ID", $uid )->get ( $this->screening_app_col_2021_22 );
	...
	*/
	...
	public function get_bmi_report_model($current_month, $school_name) {
		...
		$requests = [ ];
				...
				$under_weight = $this->mongo_db->where('doc_data.widget_data.school_details.School Name',$school_name)->whereLt('doc_data.widget_data.page1.Student Details.BMI_values.bmi',18.50) ->whereLike('doc_data.widget_data.page1.Student Details.BMI_values.month',$selected_month)->get('healthcare2017619153715384');
				$request ['label'] = 'UNDER WEIGHT';
				$request ['value'] = count($under_weight);
				array_push ( $requests, $request );
				
				$normal_weight = $this->mongo_db->where('doc_data.widget_data.school_details.School Name',$school_name)->whereBetween('doc_data.widget_data.page1.Student Details.BMI_values.bmi',18.50,24.99)->whereLike('doc_data.widget_data.page1.Student Details.BMI_values.month',$selected_month) ->get('healthcare2017619153715384');
				
				$request ['label'] = 'NORMAL WEIGHT';
				$request ['value'] = count($normal_weight);
				/*echo print_r(count($normal_weight),true);
				exit();*/
				array_push ( $requests, $request );
				
				$over_weight = $this->mongo_db->where('doc_data.widget_data.school_details.School Name',$school_name)->whereBetween('doc_data.widget_data.page1.Student Details.BMI_values.bmi',25.00,29.99)->whereLike('doc_data.widget_data.page1.Student Details.BMI_values.month',$selected_month)->get('healthcare2017619153715384');
				$request ['label'] = 'OVER WEIGHT';
				$request ['value'] = count($over_weight);
				array_push ( $requests, $request );
				
				$obese = $this->mongo_db->where('doc_data.widget_data.school_details.School Name',$school_name)->wheregte('doc_data.widget_data.page1.Student Details.BMI_values.bmi',30.0)->whereLike('doc_data.widget_data.page1.Student Details.BMI_values.month',$selected_month)->get('healthcare2017619153715384');
				$request ['label'] = 'OBESE';
	...
	public function get_drill_down_to_bmi_report($type, $current_month,  $school_name) 
	{
		...
		switch ($type) {
			...
			ini_set ( 'memory_limit', '10G' );
				...
				$query = $this->mongo_db->select ( array (
						"doc_data" 
				) )->whereBetween('doc_data.widget_data.page1.Student Details.BMI_values.bmi',25.00,29.99)->whereLike ( 'doc_data.widget_data.page1.Student Details.BMI_values.month',$selected_month)->where("doc_data.school_details.School Name",$school_name)-> get ( $this->bmi_app_col );
				...
				$query = $this->mongo_db->select ( array (
						"doc_data" 
				) )->wheregte('doc_data.widget_data.page1.Student Details.BMI_values.bmi',30.0) ->whereLike ( 'doc_data.widget_data.page1.Student Details.BMI_values.month',$selected_month)->where("doc_data.school_details.School Name",$school_name)-> get ( $this->bmi_app_col );
	...
	*/ 
	...
	public function export_bmi_reports_monthly_to_excel($date,$school_name){
		...
		$query = $this->mongo_db->select ( array (
				...
				->whereLike("doc_data.widget_data.page1.Student Details.BMI_values.month" , $date)->where(array( "doc_data.school_details.School Name" => $school_name))->get ( $this->bmi_app_col );
	...
	public function fetch_student_info_model($school_name, $unique_id)
	{
		...
		$res = $this->mongo_db->select(array('doc_data.widget_data'))->where(array('doc_data.widget_data.page2.Personal Information.School Name' => $school_name, 'doc_data.widget_data.page1.Personal Information.Hospital Unique ID'=> $unique_id))->get('ttwreis_screening_report_col_2021-2022');
	...
	{
		$is_exists = $this->mongo_db->where(array('doc_data.widget_data.page1.Student Details.Hospital Unique ID'=> $unique_id))->get('healthcare2017619153715384');
	...
	function update_bmi_values($month,$monthly_bmi,$unique_id)
	{
		  ...
		  $after_update = $this->mongo_db->where(array('doc_data.widget_data.page1.Student Details.Hospital Unique ID'=> $unique_id))->push('doc_data.widget_data.page1.Student Details.BMI_values',$monthly_bmi)->update('healthcare2017619153715384');
	...
	public function add_student_BMI_model($doc_data, $doc_properties, $app_properties, $history,$bmi_values_data)
	{
		  ...
		  $query = $this->mongo_db->insert('healthcare2017619153715384',$doc_data);
		  if($query)
			  return TRUE;
		  else
			  return FALSE;
	...
	{
		$is_exists = $this->mongo_db->where(array('doc_data.widget_data.page1.Student Details.Hospital Unique ID'=> $unique_id))->get('ttwreis_himglobin_report_col');
		if($is_exists)
		{
			return TRUE;
		}
	...
	public function add_student_HB_model($monthly_hb,$doc_properties, $app_properties, $history)
	{
		  ...
		  $query = $this->mongo_db->insert('ttwreis_himglobin_report_col',$doc_data);
		  if($query)
			  return TRUE;
		  else
			  return FALSE;
	...
	{
		$query = $this->mongo_db->select(array('doc_data.widget_data.page1.Student Details'))->where(array('doc_data.widget_data.page1.Student Details.Hospital Unique ID' => $unique_id))->get('ttwreis_himglobin_report_col');
	...
	function ttwreis_update_bmi_values($month,$monthly_bmi,$unique_id)
	{
	...
	function ttwreis_update_hb_values($month,$monthly_hb,$unique_id)
	{
	...
	public function fetch_student_information_model($school_name,$unique_id)
	{
		...
		$res = $this->mongo_db->select(array('doc_data.widget_data.page1','doc_data.widget_data.page2'))->where(array('doc_data.widget_data.page2.Personal Information.School Name' => $school_name, 'doc_data.widget_data.page1.Personal Information.Hospital Unique ID'=> $unique_id))->get('healthcare201671115519757');
	...
	{
		$this->mongo_db->switchDatabase($this->_configvalue['common_db']);
		$school_data = $this->mongo_db->where(array('school_code'=>(int)$school_code))->select(array('school_name','dt_name','contact_person_name','school_mob'),array())->get('ttwreis_schools');
		$this->mongo_db->switchDatabase($this->_configvalue['dsn']);
		foreach ( $school_data as $schools => $school ) 
		{
			$dt_name = $this->mongo_db->where ( '_id', new MongoId ( $school ['dt_name'] ) )->get ( 'ttwreis_district' );
	...
	function create_chronic_case_new($student_unique_id,$request_type,$chronic_disease,$disease_desc,$schoolName)
	{
		...
		$query = $this->mongo_db->insert('ttwreis_chronic_cases', $data);
	...
	public function initiate_request_model($doc_data,$doc_properties,$app_properties,$array_history)
	{
		...
		$res = $this->mongo_db->insert('healthcare2016108181933756_static_html',$doc_data);
	...
	public function get_hs_req_normal($usercollection,$unique_id)
    {
        ...
        $pipeline = [ 
           ...
           exit();
            return $result;*/
          $query_normal = array('app_properties.status'=>'new','app_properties.app_id'=>'healthcare2016108181933756','doc_data.widget_data.page2.Review Info.Request Type' => 'Normal','doc_data.widget_data.page1.Student Info.Unique ID' => array(
          				'$regex' => $unique_id),'doc_data.widget_data.page2.Review Info.Status' => array('$ne' => "Cured"));
     ...
     public function get_hs_req_emergency($usercollection,$unique_id)
    {
    	...
    	$query_emergency = array('app_properties.status'=>'new','app_properties.app_id'=>'healthcare2016108181933756','doc_data.widget_data.page2.Review Info.Request Type' => 'Emergency','doc_data.widget_data.page1.Student Info.Unique ID' => array(
          				'$regex' => $unique_id),'doc_data.widget_data.page2.Review Info.Status' => array('$ne' => "Cured"));
        $this->mongo_db->orderBy(array('history.0.time' => -1));
        $query=$this->mongo_db->select(array(),array('_id'))->limit(200)->
    ...
    {
    	$query_chronic = array('app_properties.status'=>'new','app_properties.app_id'=>'healthcare2016108181933756','doc_data.widget_data.page2.Review Info.Request Type' => 'Chronic','doc_data.widget_data.page1.Student Info.Unique ID' => array(
          				'$regex' => $unique_id),'doc_data.widget_data.page2.Review Info.Status' => array('$ne' => "Cured"));
        $this->mongo_db->orderBy(array('history.0.time' => -1));
        $query=$this->mongo_db->select(array(),array('_id'))->limit(200)->
    ...
    {
    	$query_cured = array('app_properties.status'=>'new','app_properties.app_id'=>'healthcare2016108181933756','doc_data.widget_data.page2.Review Info.Status' => 'Cured','doc_data.widget_data.page1.Student Info.Unique ID' => array('$regex' => $unique_id));
        $this->mongo_db->orderBy(array('history.0.time' => -1));
        $query=$this->mongo_db->select(array(),array('_id'))->limit(2000)->where($query_cured)->get($usercollection);
    ...
    {
    	$query_expaired = array('app_properties.status'=>'new','app_properties.app_id'=>'healthcare2016108181933756','doc_data.widget_data.page2.Review Info.Status' => 'Expired','doc_data.widget_data.page1.Student Info.Unique ID' => array('$regex' => $unique_id));
        $this->mongo_db->orderBy(array('history.0.time' => -1));
        $query=$this->mongo_db->select(array(),array('_id'))->limit(200)->where($query_expaired)->get($usercollection);
    ...
    {
    	$query = $this->mongo_db->select(array('history'))->where(array('doc_properties.doc_id'=> $doc_id))->get('healthcare2016108181933756_static_html');
	...
	*/
		...
		public function get_attendance_report_daily_count($school_name,$date)
		{
			...
			))->where(array('doc_data.widget_data.page1.Attendence Details.Select School' => $school_name))->WhereLike('history.last_stage.time', $today_date)->get('healthcare20161015173311279');
			if($query)
			{
	...
	/*  HS Daily Requests Count to show on table ================ By Harish ============   */
		...
		public function get_requests_daily_count($district_school_code,$date)
		{
			...
			))->count('healthcare2016108181933756_static_html');
	...
	$normal_requests_info = $this->mongo_db->select($selected_fields)->whereLike(
				'doc_data.widget_data.page1.Student Info.Unique ID',$district_school_code)
			   ->WhereLike('history.time', $today_date)
			  ->where(array('doc_data.widget_data.page2.Review Info.Request Type' => "Normal"))->get('healthcare2016108181933756_static_html');
	...
	$emergency_requests_info = $this->mongo_db->select($selected_fields)->whereLike('doc_data.widget_data.page1.Student Info.Unique ID',$district_school_code)
			   ->WhereLike('history.time', $today_date)
			   ->where(array('doc_data.widget_data.page2.Review Info.Request Type' => "Emergency"))->get('healthcare2016108181933756_static_html');
	...
	$chronic_requests_info = $this->mongo_db->select($selected_fields)->whereLike('doc_data.widget_data.page1.Student Info.Unique ID',$district_school_code)
			   ->WhereLike('history.time', $today_date)
			   ->where(array('doc_data.widget_data.page2.Review Info.Request Type' => "Chronic"))->get('healthcare2016108181933756_static_html');
	...
	public function get_docs_daily_count($district_school_code, $date)
		{
			...
			$doc_normal_requests = $this->mongo_db->whereLike(
              ...
              ->where(array('doc_data.widget_data.page2.Review Info.Request Type' => "Normal"))->count('healthcare2016108181933756_static_html');
	...
	$doc_chronic_requests_info = $this->mongo_db->select($selected_fields)->whereLike('doc_data.widget_data.page1.Student Info.Unique ID',$district_school_code)
		...
		public function get_sanitation_report_daily_count($school_name,$date)
		{
				...
				))->where(array('doc_data.widget_data.page4.School Information.School Name' => $school_name))->WhereLike('history.last_stage.time', $today_date)->get('healthcare20171211809800');
	...
	public function get_bmi_report_model_count($current_month, $school_name) {
		...
		if ($current_month) {
			...
			$selected_month = $this->selected_month;
				...
				$under_weight = $this->mongo_db->where('doc_data.school_details.School Name',$school_name)->whereLt('doc_data.widget_data.page1.Student Details.BMI_values.bmi',18.50) ->whereLike('doc_data.widget_data.page1.Student Details.BMI_values.month',$selected_month)->count('healthcare2017619153715384');
				
				$normal_weight = $this->mongo_db->where('doc_data.school_details.School Name',$school_name)->whereBetween('doc_data.widget_data.page1.Student Details.BMI_values.bmi',18.50,24.99)->whereLike('doc_data.widget_data.page1.Student Details.BMI_values.month',$selected_month)->count('healthcare2017619153715384');
				
				$over_weight = $this->mongo_db->where('doc_data.school_details.School Name',$school_name)->whereBetween('doc_data.widget_data.page1.Student Details.BMI_values.bmi',25.00,29.99)->whereLike('doc_data.widget_data.page1.Student Details.BMI_values.month',$selected_month)->count('healthcare2017619153715384');

				$obese = $this->mongo_db->where('doc_data.school_details.School Name',$school_name)->wheregte('doc_data.widget_data.page1.Student Details.BMI_values.bmi',30.0)->whereLike('doc_data.widget_data.page1.Student Details.BMI_values.month',$selected_month)->count('healthcare2017619153715384');
	...
	public function get_bmi_report_model_info($current_month, $school_name) {
		...
		if ($current_month) {
			...
			$selected_month = $this->selected_month;
				...
				$under_weight_array = array('doc_data.widget_data.page1.Student Details.BMI_values.month' => $selected_month,'doc_data.school_details.School Name'=> $school_name);
				$under_weight_info = $this->mongo_db->select(array('doc_data.widget_data.page1.Student Details'))->where($under_weight_array)->whereLt('doc_data.widget_data.page1.Student Details.BMI_values.bmi',18.49)->get('healthcare2017619153715384');
				
					
 
				$normal_weight_array = array('doc_data.widget_data.page1.Student Details.BMI_values.month' => $selected_month,'doc_data.school_details.School Name'=> $school_name);

			    $normal_weight_info = $this->mongo_db->select(array('doc_data.widget_data.page1.Student Details'))->where($normal_weight_array)->whereBetween('doc_data.widget_data.page1.Student Details.BMI_values.bmi',18.50,24.99)->get('healthcare2017619153715384');
			   

				
				$over_weight_array = array('doc_data.widget_data.page1.Student Details.BMI_values.month' => $selected_month,'doc_data.school_details.School Name'=> $school_name);
			 
				$over_weight_info = $this->mongo_db->select(array('doc_data.widget_data.page1.Student Details'))->where($normal_weight_array)->whereBetween('doc_data.widget_data.page1.Student Details.BMI_values.bmi',25.00,29.99)->get('healthcare2017619153715384');
			
			

				
				$obese_weight_array = array('doc_data.widget_data.page1.Student Details.BMI_values.month' => $selected_month,'doc_data.school_details.School Name'=> $school_name);

				$obese_info = $this->mongo_db->select(array('doc_data.widget_data.page1.Student Details'))->where($obese_weight_array)->wheregte('doc_data.widget_data.page1.Student Details.BMI_values.bmi',30.0)->get('healthcare2017619153715384');
...
/* Normal Type Anamea */	
	...
	public function get_sanitation_report_data_with_date_version_2($date,$school_name)
	{
		...
		$query = $this->mongo_db->get('healthcare20171211809800_version_2');
		if($query)
			return $query;
		else
			return FALSE;
	...
	public function get_students_load_ehr_doc_model($_id) {
		...
		if ($query) {
			$query_request = $this->mongo_db->where ( "doc_data.widget_data.page1.Student Info.Unique ID", $query [0] ["doc_data"] ['widget_data'] ['page1'] ['Personal Information'] ['Hospital Unique ID'] )->get ( 'healthcare2016108181933756_static_html' );
			
			$bmi_value_table_addto_ehr = $this->mongo_db->where("doc_data.widget_data.page1.Student Details.Hospital Unique ID", $query [0] ["doc_data"] ['widget_data'] ['page1'] ['Personal Information'] ['Hospital Unique ID'])->get('healthcare2017619153715384');
	...
	public function update_principal_hs_profile($data,$schoolCode)
	{
		//echo print_r($data,TRUE);exit();
			...
			$this->mongo_db->switchDatabase($this->common_db['common_db']);
			$result = $this->mongo_db->where('school_code',$schoolCode)->set($update_profile_hs)->update('ttwreis_health_supervisors');
			$this->mongo_db->switchDatabase($this->common_db['dsn']);
			...
			$this->mongo_db->switchDatabase($this->common_db['common_db']);
			$result = $this->mongo_db->where('school_code',$schoolCode)->set($update_profile_principal)->update('ttwreis_schools');
			$this->mongo_db->switchDatabase($this->common_db['dsn']);
	...
	public function get_hb_report_model($current_month, $school_name) {
		...
		$requests = [ ];
				...
				$sevier = $this->mongo_db->where('doc_data.widget_data.school_details.School Name',$school_name)->whereLt('doc_data.widget_data.page1.Student Details.HB_values.hb',8) ->whereLike('doc_data.widget_data.page1.Student Details.HB_values.month',$selected_month)->get('ttwreis_himglobin_report_col');
				$request ['label'] = 'Sevier';
				$request ['value'] = count($sevier);
				array_push ( $requests, $request );
				
				$normal = $this->mongo_db->where('doc_data.widget_data.school_details.School Name',$school_name)->whereBetween('doc_data.widget_data.page1.Student Details.HB_values.hb',12,18)->whereLike('doc_data.widget_data.page1.Student Details.HB_values.month',$selected_month)->get('ttwreis_himglobin_report_col');
				$request ['label'] = 'NORMAL';
				$request ['value'] = count($normal);
				array_push ( $requests, $request );
				
				$moderate = $this->mongo_db->where('doc_data.widget_data.school_details.School Name',$school_name)->whereBetween('doc_data.widget_data.page1.Student Details.HB_values.hb',8.1,10)->whereLike('doc_data.widget_data.page1.Student Details.HB_values.month',$selected_month)->get('ttwreis_himglobin_report_col');
				$request ['label'] = 'Moderate';
				$request ['value'] = count($moderate);
				array_push ( $requests, $request );
				
				$mild = $this->mongo_db->where('doc_data.widget_data.school_details.School Name',$school_name)->whereBetween('doc_data.widget_data.page1.Student Details.HB_values.hb',10.1,12)->whereLike('doc_data.widget_data.page1.Student Details.HB_values.month',$selected_month)->get('ttwreis_himglobin_report_col');
				$request ['label'] = 'Mild';
	...
	public function get_drill_down_to_hb_report($type, $current_month,  $school_name) 
	{
		...
		switch ($type) {
			...
			ini_set ( 'memory_limit', '10G' );
				$query = $this->mongo_db->select ( array (
						"doc_data" 
				) )->whereBetween('doc_data.widget_data.page1.Student Details.HB_values.hb',12,18) ->whereLike ( 'doc_data.widget_data.page1.Student Details.HB_values.month',$selected_month)->where("doc_data.widget_data.school_details.School Name",$school_name)->get ( $this->hb_app_col );
			...
			ini_set ( 'memory_limit', '10G' );
				...
				$query = $this->mongo_db->select ( array (
						"doc_data" 
				) )->whereBetween('doc_data.widget_data.page1.Student Details.HB_values.hb',8.1,10)->whereLike ( 'doc_data.widget_data.page1.Student Details.HB_values.month',$selected_month)->where("doc_data.widget_data.school_details.School Name",$school_name)-> get ( $this->hb_app_col );
				...
				$query = $this->mongo_db->select ( array (
						"doc_data" 
				) )->whereBetween('doc_data.widget_data.page1.Student Details.HB_values.hb',10.1,12)->whereLike ( 'doc_data.widget_data.page1.Student Details.HB_values.month',$selected_month)->where("doc_data.widget_data.school_details.School Name",$school_name)-> get ( $this->hb_app_col );
	...
	function drill_down_to_hb_report_students()
	{
		...
		$students = $this->ttwreis_schools_common_model->get_drilling_hb_students_docs($UI_id);
		...
		$this->_render_page('ttwreis_schools/drill_down_to_hb_report_view',$this->data);
	}
    ...
    {
        $is_exists = $this->mongo_db->where(array('doc_data.widget_data.Student Details.Hospital Unique ID'=> $unique_id))->get('ttwreis_doctor_visiting_reports');
        if($is_exists)
        {
            return TRUE;
        }
		...
		public function get_doctor_visiting_report_date_wise($school_name)
    {
          ...
          $response = $this->mongo_db->command(array('distinct' => "ttwreis_doctor_visiting_reports" ,'key' => "doc_data.widget_data.Student Details.doctor_visiting_date",'query'=>$query));


          foreach ($response['values'] as $dates) {
            
             $getSubmittedDocs = $this->mongo_db->where(array('doc_data.widget_data.Student Details.doctor_visiting_date' =>$dates,"doc_data.widget_data.school_details.School Name"=>$school_name))->count('ttwreis_doctor_visiting_reports');
    ...
    {
        $query = $this->mongo_db->select(array(),array('_id'))->where(array('doc_data.widget_data.Student Details.Hospital Unique ID' => $unique_id,'doc_properties.doc_id' => $doc_id))->get('ttwreis_doctor_visiting_reports');
    ...
    {   
        ...
        $update_query = $this->mongo_db->where(array('doc_data.widget_data.Student Details.Hospital Unique ID' => $uniqueId, 'doc_properties.doc_id' => $doc_id))->set(array('doc_attachments' =>$doc_attachments))->update('ttwreis_doctor_visiting_reports');
    ...
    public function fetch_student_info_for_doctor_visit($school_name, $unique_id)
    {
        ...
        $res = $this->mongo_db->select(array('doc_data.widget_data'))->where(array('doc_data.widget_data.page2.Personal Information.School Name' => $school_name, 'doc_data.widget_data.page1.Personal Information.Hospital Unique ID'=> $unique_id))->get('healthcare201671115519757');
    ...
    public function fetch_student_attachments($school_name, $unique_id)
    {
        ...
        $res  = $this->mongo_db->select(array("doc_data.widget_data.Student Details.Hospital Unique ID","doc_properties.doc_id","doc_attachments"))->where(array('doc_data.widget_data.Student Details.Hospital Unique ID' =>$unique_id,"doc_data.widget_data.school_details.School Name"=>$school_name))->get('ttwreis_doctor_visiting_reports');
    ...
    public function drill_down_to_doctor_treated_list($school_name,$selectedDate)
    {
             ...
             $getSubmittedDocs = $this->mongo_db->where(array('doc_data.widget_data.Student Details.doctor_visiting_date' =>$selectedDate,"doc_data.widget_data.school_details.School Name"=>$school_name))->get('ttwreis_doctor_visiting_reports');
     ...
     public function show_doctor_treated_student(/*$student_id,$doctor_visit_date*/ $doc_id)
    {
             ...
             $getSubmittedDocs = $this->mongo_db->select(array(),array('_id'))->where(array('doc_properties.doc_id'=>$doc_id))->get('ttwreis_doctor_visiting_reports');
...
//		foreach ($final_result as $school_name) 
		{
//			$this->mongo_db->switchDatabase ( $this->common_db ['common_db'] );
			$pc_result = $this->mongo_db->select(array('school_code','school_mob','contact_person_name'))->get($this->collections['ttwreis_schools']);
			$this->mongo_db->switchDatabase ( $this->common_db ['dsn'] );
			...
			foreach ($pc_result as $school_code)
			{
				...
				where(array('school_code' => $school_code['school_code']))->get($this->collections['ttwreis_health_supervisors']);
				$this->mongo_db->switchDatabase ( $this->common_db ['dsn'] );
	...
	public function send_sms_for_not_submited_attendance()
	{
		...
		$this->mongo_db->switchDatabase ( $this->common_db ['common_db'] );
		$res = $this->mongo_db->select(array('school_name'))->get($this->collections['ttwreis_schools']);
		$this->mongo_db->switchDatabase ( $this->common_db ['dsn'] );
		foreach ($res as $school) {
			$school_name_col = $school['school_name'];
			array_push($schools_array_col, $school_name_col);
		...
		{
			$this->mongo_db->switchDatabase ( $this->common_db ['common_db'] );
			$pc_result = $this->mongo_db->select(array('school_code','school_mob','contact_person_name'))->where(array('school_name' => $school_name))->get($this->collections['ttwreis_schools']);
			$this->mongo_db->switchDatabase ( $this->common_db ['dsn'] );
	...
	public function insert_attendance_info($data)
	{
		...
		$get_count  = $this->mongo_db->where(array('Messages_info.school_name' => $data['school_name']))->count('TTWREIS_Attendance_msg_count');
		if($get_count != 0)
		{
			$query = $this->mongo_db->push('Sent_dates', $today_date)->inc(array('Sent_count' => 1))->update('TTWREIS_Attendance_msg_count');
		}else
		{
			
			$today_date  = Date("Y-m-d");
			$insert_data = array('Messages_info' => $data,'Sent_count' => 1,'Sent_dates' => [$today_date],'Type_of_Msg' => "Attendance Not Submitted");
			$query = $this->mongo_db->insert("TTWREIS_Attendance_msg_count", $insert_data);	
		}

		$over_all_count = $this->mongo_db->where(array("$today_date.Attendance_Messages_info.Sent_date" => $today_date))->count('TTWREIS_Over_all_Msg_status');

		if($over_all_count != 0)
		{
			$query = $this->mongo_db->where(array("$today_date.Attendance_Messages_info.Sent_date" => $today_date))->push("$today_date.Attendance_Messages_info.school_name", $data['school_name'])->inc(array("$today_date.Attendance_Messages_info.Sent_count" => 1))->update('TTWREIS_Over_all_Msg_status');
		}else
		{
			$query = $this->mongo_db->set(array("$today_date.Attendance_Messages_info.school_name" => [$data['school_name']], "$today_date.Attendance_Messages_info.Sent_count" => 1, "$today_date.Attendance_Messages_info.Sent_date" => $today_date, "$today_date.Attendance_Messages_info.Type_of_Msg" => "Attendance Not Submitted"))->update('TTWREIS_Over_all_Msg_status');
		}
		...
		}else
		{
			...
			$query = $this->mongo_db->insert('TTWREIS_Over_all_Msg_status', $insert_over_data);
		}*/
	...
	{
		$today_date = Date("Y-m-d");
		$get_count  = $this->mongo_db->where(array('Messages_info.school_name' => $data['school_name']))->count('TTWREIS_Sanitation_msg_count');
		if($get_count != 0)
		{
			$query = $this->mongo_db->push('Sent_dates', $today_date)->inc(array('Sent_count' => 1))->update('TTWREIS_Sanitation_msg_count');
		}else
		{
			$today_date  = Date("Y-m-d");
			$insert_data = array('Messages_info' => $data,'Sent_count' => 1,'Sent_dates' => [$today_date],'Type_of_Msg' => "Sanitation Not Submitted");
			$query = $this->mongo_db->insert("TTWREIS_Sanitation_msg_count", $insert_data);	
		}

		$over_all_count = $this->mongo_db->where(array("$today_date.Sanitation_Messages_info.Sent_date" => $today_date))->count('TTWREIS_Over_all_Msg_status');

		if($over_all_count != 0 )
		{
			$query = $this->mongo_db->where(array("$today_date.Sanitation_Messages_info.Sent_date" => $today_date))->push("$today_date.Sanitation_Messages_info.school_name", $data['school_name'])->inc(array("$today_date.Sanitation_Messages_info.Sent_count" => 1))->update('TTWREIS_Over_all_Msg_status');
		}else
		{
	...
	{
		$query = $this->mongo_db->select(array('doc_data.widget_data.page1.Personal Information.Name'))->where(array("history.last_stage.submitted_by" => str_replace("@", "#", $email)))->get('tt_doctor_personal_profile_report');
		return $query;
	}
	...
	public function get_doc_id_for_check($initate_submit)
	{
		...
		$get_id = $this->mongo_db->select('doc_properties.doc_id')->where ( '_id', $initate_submit )->get('healthcare2016108181933756_static_html');
	...
	public function get_searched_student_sick_requests_model($search_data,$school_name)
	{
		...
		if(preg_match("/_/", $data)){
			...
			$query = $this->mongo_db->where('doc_data.widget_data.page1.Student Info.School Name.field_ref',$school_name)->whereLike('doc_data.widget_data.page1.Student Info.Unique ID', $data)->whereNe(array('doc_data.widget_data.page2.Review Info.Status' => "Cured"))->orderBy(array('history.0.time' => -1))->get('healthcare2016108181933756_static_html');

		}else{

		$query = $this->mongo_db->where('doc_data.widget_data.page1.Student Info.School Name.field_ref',$school_name)->whereLike('doc_data.widget_data.page1.Student Info.Name.field_ref', $data)->whereNe(array('doc_data.widget_data.page2.Review Info.Status' => "Cured"))->orderBy(array('history.0.time' => -1))->get('healthcare2016108181933756_static_html');

...
class Ion_auth_model extends CI_Model
{
	...
	public function login($identity, $password, $remember=FALSE)
	{
		...
		$this->trigger_events('extra_where');
        $this->mongo_db->switchDb($this->switchs['tlstec_common_db']);
		...
		if ($query->num_rows() === 1)
		{
			...
			$config =	array(
                ...
                log_message('debug','config dataaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa at ion auth model'.print_r($config,true));
				$this->mongo_db->setConfigRun($config,true);
				return TRUE;
			}
		}
	...
	public function user($id = NULL)
	{
		...
		$this->limit(1);
		$this->where($this->tables['customers'].'.id', $id);

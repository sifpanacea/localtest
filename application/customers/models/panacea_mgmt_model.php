...
class Panacea_mgmt_model extends CI_Model 
{
    ...
    public function change_doctor_status($status, $doc)
    {
    	$this->mongo_db->switchDatabase($this->common_db['common_db']);
    	$query = $this->mongo_db->where(array("_id"=>new MongoId($doc)))->set(array("active"=>$status))->update($this->collections['panacea_doctors']);
    	$this->mongo_db->switchDatabase($this->common_db['dsn']);
    	return $query;
    }

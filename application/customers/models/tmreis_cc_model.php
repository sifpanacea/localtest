...
class Tmreis_cc_model extends CI_Model
{
	/**
    ...
    {
     $query = $this->mongo_db->get('tmreis_district');
    ...
    {
     $count = $this->mongo_db->count('tmreis_schools');
	 return $count;
    }
	...
	public function get_schools($per_page,$page)
    {
     $offset = $per_page * ( $page - 1) ;
		$query = $this->mongo_db->limit($per_page)->offset($offset)->get('tmreis_schools');
    ...
    {
    	$query = $this->mongo_db->get('tmreis_schools');
    	foreach($query as $schools => $school){
    		$dt_name = $this->mongo_db->where('_id', new MongoId($school['dt_name']))->get('tmreis_district');
    		if(isset($school['dt_name'])){
    			$query[$schools]['dt_name'] = $dt_name[0]['dt_name'];
	...
	public function create_school($post)
    {
     ...
     $query = $this->mongo_db->insert('tmreis_schools',$data);
	 return $query;
    }
	
	public function delete_school($school_id)
    {
     $query = $this->mongo_db->where(array("_id"=>new MongoId($school_id)))->delete('tmreis_schools');
	 return $query;
    }

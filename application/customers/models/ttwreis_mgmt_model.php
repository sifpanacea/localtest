...
class Ttwreis_mgmt_model extends CI_Model 
{
	/**
    ...
    public function create_state($post)
    {
    	...
    	$query = $this->mongo_db->insert('ttwreis_states',$data);
    	return $query;
    }
    
    public function delete_state($st_id)
    {
    	$query = $this->mongo_db->where(array("_id"=>new MongoId($st_id)))->delete('ttwreis_states');
    	return $query;
    }
    ...
    public function create_district($post)
    {
    	...
    	$query = $this->mongo_db->insert('ttwreis_district',$data);
    	return $query;
    }
    
    public function delete_district($dt_id)
    {
    	$query = $this->mongo_db->where(array("_id"=>new MongoId($dt_id)))->delete('ttwreis_district');
    	return $query;
    }
    ...
    public function delete_health_supervisors($hs_id)
    {
    	...
    	$query = $this->mongo_db->where(array("_id"=>new MongoId($hs_id)))->delete($this->collections['ttwreis_health_supervisors']);
    ...
    public function get_cc_users($per_page,$page)
    {
		...
		//==================================================== duplicate document=================================
		// $u_ids = ["TRR_1811"];
		// $query = $this->mongo_db->whereLike("doc_data.widget_data.page1.Personal Information.Hospital Unique ID", "TRR_1811")->get($this->screening_app_col);
		// $no_data = [];
    ...
    {
    	$this->mongo_db->switchDatabase($this->common_db['common_db']);
    	$query = $this->mongo_db->where(array("_id"=>new MongoId($cc_id)))->delete($this->collections['ttwreis_cc']);
    	$this->mongo_db->switchDatabase($this->common_db['dsn']);
    	return $query;
    }
    ...
    public function create_doctor($post)
    {
    	...
    	$this->mongo_db->switchDatabase($this->common_db['common_db']);
    	$query = $this->mongo_db->insert($this->collections['ttwreis_doctors'],$data);
    	$this->mongo_db->switchDatabase($this->common_db['dsn']);
    	// Return new document _id or FALSE on failure
    	return isset($query) ? $query : FALSE;
    }
    ...
    {
    	$this->mongo_db->switchDatabase($this->common_db['common_db']);
    	$query = $this->mongo_db->where(array("_id"=>new MongoId($cc_id)))->delete($this->collections['ttwreis_doctors']);
    	$this->mongo_db->switchDatabase($this->common_db['dsn']);
    	return $query;
    }
    ...
    {
    	$this->mongo_db->switchDatabase($this->common_db['common_db']);
    	$query = $this->mongo_db->where(array("_id"=>new MongoId($school_id)))->delete($this->collections['ttwreis_schools']);
    	$this->mongo_db->switchDatabase($this->common_db['dsn']);
    	return $query;
    }
    ...
    public function school_exists($email = FALSE){
    	...
    	$this->mongo_db->switchDatabase($this->common_db['common_db']);
    	$query = $this->mongo_db->where(array('email'=> $email))->get($this->collections['ttwreis_schools']);
    	$this->mongo_db->switchDatabase($this->common_db['dsn']);
    ...
    public function cc_user_exists($email = FALSE){
    	...
    	$this->mongo_db->switchDatabase($this->common_db['common_db']);
    	$query = $this->mongo_db->where(array('email'=> $email))->get($this->collections['ttwreis_cc']);
    	$this->mongo_db->switchDatabase($this->common_db['dsn']);
    ...
    public function doctor_exists($email = FALSE){
    	...
    	$this->mongo_db->switchDatabase($this->common_db['common_db']);
    	$query = $this->mongo_db->where(array('email'=> $email))->get($this->collections['ttwreis_doctors']);
    	$this->mongo_db->switchDatabase($this->common_db['dsn']);
     ...
     public function create_group($post,$type)
    {
        ...
        $query = $this->mongo_db->insert('ttwreis_chat_groups',$data);
        return $query;
    }
     public function delete_chat_group($_id)
    {
        $query = $this->mongo_db->where(array("_id"=>new MongoId($_id)))->delete('ttwreis_chat_groups');
        return $query;
    }

    public function save_users_to_group($post)
    {
        $group_exists = $this->mongo_db->where("group_name",$post['ed_group_name'])->get('ttwreis_chat_groups_users');
        if($group_exists){
            $data = array(
            ...
            $query = $this->mongo_db->where("group_name",$post['ed_group_name'])->set ( $data )->update ( $this->collections['ttwreis_chat_groups_users'] );
            ...
            $query = $this->mongo_db->insert($this->collections['ttwreis_chat_groups_users'],$data);
